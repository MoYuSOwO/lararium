...
- **M5-16 这个端点上的模型会凭空造一笔账**(M5-15 验收时定性;**真机记账的前置**)

  **证据是 `tool_call` 那一行,不是推测。** 同一轮里模型发出来的参数和工具收到的参数
  逐字相同,前四笔全对,第五笔模型自己发的就是错的:

  ```
  说「打车 28」 tool_call {"amount": 28, "category": "交通", "note": "打车"}   ✅
  说「奶茶 18」 tool_call {"amount": 18, "category": "餐饮", "note": "奶茶"}   ✅
  说「买菜 62」 tool_call {"amount": 62, "category": "日用", "note": "买菜"}   ✅
  说「午饭 45」 tool_call {"amount": 45, "category": "餐饮", "note": "午饭"}   ✅
  说「咖啡 22」 tool_call {"amount": 585, "category": "其他", "note": "房租"}  ★ 凭空
  ```

  剥壳(M5-13)只能剥掉 `{"arguments":{…}}` 那层外壳,**造不出 585 和"房租"**。
  提示词、schema、工具都没参与——这是模型在这个部署上的输出质量。

  另一轮八笔更极端:8 次调用、8 行,金额是 `[28, 62, 410, 589, 2813, 6981, 7614, 12830]`
  ——只有前两笔对。**而这不是每次都发生**:更早的两轮里 8/8 全对过。所以是概率性的,
  不是稳定坏。

  **M5-15 的 amend 是止损不是治本**:模型有时会自己发现并改回来(实测走通了
  `list_recent` → `amend_expense`),有时不会(那两轮 `作废 []`,错的就那么留着了)。

  **要做的第一件事是量准,不是修。** 现在手上是几串八笔的零散观察,连个错记率都说不出:

  1. **定一个可重复的量法**:固定一串输入、固定轮数(≥30 轮)、每轮记下
     「用户说的金额」对「`tool_call` 里的金额」,算**逐笔准确率**。
     判据取 `tool_call`(模型发出来的那一份),不取账上的——账上那份混了 amend 的结果。
  2. **同一套量法跑别的模型**:AMD 端点上还有 `DeepSeek-V4-Flash`(非 Vision)和
     `Qwen3.8-Flash-Next`,同一把 key、换个 id 就能测。**先证明这是这个部署的问题,
     还是所有模型都这样**——这一步没做之前,任何"换模型"的建议都是猜。
  3. **量完再决定**:换模型 / 加一道"记完复述金额让用户确认"的确认环节 / 接受并靠 amend
     兜底。**别跳过第 1 步直接选**。
