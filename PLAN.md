  ...
  - **`search_recipes`**:菜名命中、内容命中**各有一条测试**,且结果里**分得出是哪一种**;
    ...
    (这一条钉的是"没有索引会变味");
  - **前缀影响(A1)**:新 bundle = 目录行 + 6 个工具 schema,前缀重建一次。
...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿

  用户一开始没选它,后来补的:

  > 「其实我发现学习 bundle 也要。」
  > 「**作业考试也是待办啊,为什么不能用待办系统来装呢?** 我要的是类似
  > **一个课程建立一个目录 + 知识库**这种,不知道能不能搞定。」

  ### 两件事因此归位了

  **① 作业 / 考试 → 待办(M6-6),不在这一条。** 复核方上一版把它们放进了学习 bundle,
  并问"它们和待办的边界在哪"。**用户直接答了:没有边界,它们就是待办。**
  有到期时间的一次性事项,没理由另造一套——**另造一套就是第二份真相**,
  而这正是 M6-6 那一条自己在防的东西。

  **② 这一条就是做菜那套文档库,加一层课程。** 而那个两层命名空间
  (`list_categories` + `list_under_category`)**正是复核方给做菜写过、被用户否掉的那个**:

  ```
  做菜   番茄炒鸡蛋属于什么类?煮面水属于什么类?  → 分类是硬塞的 → 扁平
  学习   这份笔记属于哪门课?                      → 天然就有答案 → 两层
  ```

  **那个设计不是错的,是放错了地方。** 记下来:同一个形状在一个领域是过度设计,
  在另一个领域是正好——**判据是"这个维度是数据自带的,还是我们替用户编的"**。

  ### 形状:`(课程, 笔记名)` 两层,路径仍然不出现在接口上

  存成 markdown 文件(理由同 M6-4:用户要能在自己电脑上打开、能 cat、能复制文件夹备份)。
  **模型手里只有两个名字**,底下怎么落盘它不知道。
  ...
  replace_in_note(course, old, new)           定点改(匹配不上就拒绝,同 M6-4)
  search_notes(query, course=None, page=1)    搜内容;不给 course 就全搜
  ```

  ### **一门课一个笔记本**,不是一门课下面多份笔记

  用户:「一门课给一个笔记本应该大概也够了吧?」**采纳,而且理由比"够用"更硬:**

  **多份笔记会逼模型编文件名。** 每次写都得给个名字,而它会产出
  「第一章笔记」「chapter1」「线代笔记1」这种互不一致的东西——**和做菜那个 category
  是同一个毛病:别让模型发明一个维度。** 一门课一个本子,这个决定整个消失。

  **所以 `write_note`(整篇覆盖)也去掉了**:一学期的笔记整篇覆盖太危险,
  `append` + `replace` 够用;真要重构,**用户直接编辑那个 `.md`**
  ——那正是选文件而不是 SQLite 的理由。第一次 `append` 时隐式创建。

  **代价**:一个本子会长。所以 `read_note` **必须分页**,而 `search_notes` 从
  "锦上添花"变成"主要入口"——找一段的正常姿势是搜,不是从头读。

  ### RAG 知识库:**不建;我们现在这套正好是取代它的形状**

  用户问「RAG 知识库现在主流还用吗?」查了(2026-09-12):

  > 「chunk → embed → 塞进向量库」那种 2023 版 vanilla RAG **确实被判死了**;
  > 活下来的是**把检索决定交给模型自己**的形态(agentic retrieval)。
  > 而且:**知识库在约 20 万 token 以下时,带缓存的全上下文可以直接打败自建检索基础设施。**

  **这个项目已经是后者了:**

  ```
  search_history   词法(FTS5 trigram)     人名 / 数字 / 店名这类精确词
  recall_similar   语义(model2vec + sqlite-vec) 不记得原话、只记得意思
  前缀第 2 层       档案(事实)每轮全量在场,靠缓存不额外付钱
  ```

  **模型自己决定什么时候调哪个——那就是 agentic retrieval。**
  给课程笔记再搭一套向量 pipeline,**恰恰是去做那个已经被判死的 2023 版本**。

  **真需要语义搜笔记的那天**(「这门课讲过啥关于并发的」而字面搜不到):
  `embed()` 和 sqlite-vec **都已经在了**,那是把现有那条路多接一个数据源,不是新建一套。
  **而那一天要有证据才算到**:先上 `search_notes` 的字面搜,撞到"明明写过却搜不出来"再说。

  **课件(PDF)那一支是只读的,单独两个工具**(见下面那节的实测和理由):

  ```
  list_materials(course, page=1)               这门课有哪些课件(共几页也要给)
  read_material(course, name, page)            **一页给两样:文字 + 那一页的图**
  ```

  **没有 `write_material`,也不该有。** 课件是老师给的,你没法 write 一个 PDF。
  课件怎么进来:用户在微信里发文件,走 M5-4 那条媒体入站
  ——**归到哪门课要用户或模型说,别猜文件名。**

  **课程名校验、落点兜底、`replace` 的"匹配不上就拒绝"、不建索引直接扫文件
  ——全部照 M6-4,一个字都不许另写一套。** 见下。
  (笔记本没有第二层名字要校验了——一门课一个本子,见下节。)

  ### ★ 所以 M6-4 要为这一条留好接缝

  **做菜和学习是同一个文档库的两个消费者。** 而 `.importlinter` 禁止 bundle 互相依赖,
  所以只有两条路:**要么在 `lararium` 侧放一份共用的文档库**(bundle 都能 import 它,
  finance 已经这么 import `lararium.db` 了),**要么抄一份**。

  **抄一份是不行的**:名字校验、落点兜底、`replace` 的匹配纪律、搜索的"不建索引"——
  这些每一条都是安全或正确性攸关的,**抄一份就是两处维护同一个事实,
  而漂了之后的表现是"其中一个 bundle 的路径校验松了"**。

  **这一条要写进 M6-4 的任务书**(已加),让做菜那一轮就知道后面有第二个消费者。
  **但 M6-4 不要为此先建抽象**——先把做菜做对、做具体,
  抽象在 M6-5 动手时按真实的第二个用例提出来。**先抽象再用是猜,用两次再抽象是归纳。**

  ### ★ 课件是 PDF——**不用 OCR,也不用 subagent**,但要分两支

  用户:「课程不同,很多东西都是 PDF 的,能建立知识库吗?或者得先让一个 subagent 去 OCR?
  **而且你也不能什么 read_note 之类的了,write 你也没法 write。**」

  **最后那半句是对的,而且它指出了一个框架错误**:笔记和课件是两种东西。

  ```
  笔记(markdown)   你写的、她写的  → 可读可写:read/write/append/replace/search
  课件(PDF 等)      老师给的       → **只读**。你没法 write 一个 PDF,也不该能
  ```

  所以工具面要分开,**课件那一支没有任何写入口**。

  ### 我实测过的四件事(2026-09-12,真 key、真模型)

  ```
  ① 模型自己吃不吃 PDF?  不吃。
     `image_url` 送 PDF → 400「unsupported image」
     content part 只认 `text` / `image_url` / `file` 三种(错误信息里 expected one of 给的)
     而 `file` 那一种试到最后:400「unsupported file … formats: webp, png, jp…」
     **它只收图片。** 这是打出来的,不是猜的。
  ② 有文字层的 PDF 抽得出来吗?  抽得出。`pypdf`(**纯 Python,零 C 依赖**)
     抽三页讲义,逐行正确。
  ③ 渲染成图行不行?  行。`pypdfium2`(许可宽松,PDFium)把第 2 页渲成
     1191x1684 的 PNG,**53 KB**。
  ④ 渲出来的图模型读不读得准?  准。问「这一页的 key number 是多少?标题是什么?」
     → 「Key number 是 274,标题是 Lecture 2: Concurrency in SQLite」——**全对**。
  ```

  ### 于是:**一页给两样——文字 + 那一页的图**,不做分支

  复核方第一版写的是"有文字层就抽文字,没有才渲染成图"。**用户否了两次:**

  > 「v4.1f 又不是截不了图片……你让他看图不就行了。」
  > 「**你不要算经济账,效果不好你 1000 token 能抵消??**
  > 按我说最好就是**可以读单页的图片和文字,都弄出来搞一搞**。」

  **他对,而且我那个"分支"的框是错的。** 我的理由是 token 便宜、一次能读更多页
  ——**那是经济账,而效果不好的话省下来的 token 一文不值。**

  更要紧的是:**"有文字层就只用文字"会真的丢信息。** 讲义的文字层会把
  公式、表格、图整坏——一张幻灯片抽出来可能是 `f(x) = P n i=1 x i`,
  而图上是正常的求和符号;图表根本没有文字层。

  **所以一页给两样:**

  ```
  read_material(course, name, page)  →  ① 这一页抽出来的文字(没有就说没有)
                                        ② 这一页渲染出来的图
  ```

  ### 这个形状顺带给了一个免费的好处

  两样的生命期不一样,而**那正好是我们要的**:

  ```
  文字   是工具结果 → 留在起居注和 L0 里 → 这一轮之后她还能回头引用
  图     只在到达轮进模型(M5-5 的机制) → 看完就没了
  ```

  所以她**当下**看得见版式、公式、图,而**之后**手里仍然有那一页的文字。
  **这不用设计,是两条已有的规矩叠出来的。** 但要写进 docstring
  ——否则下一个人会以为图也能回头看,然后为"让图留下来"去动 M5-5 那条机制。

  ### 一次一页,页数上限由图片那条机制定

  `MAX_IMAGES_PER_TURN = 4`(M5-5,**不许为这个放宽**:注入面不随轮次累积
  就是靠它)。所以一次最多 4 页,**而工具要说实话**:要读第 7 到第 20 页,
  它得回「一次最多 4 页,先给你 7-10」,**不许悄悄只送前 4 页然后装作读完了**
  ——那正是 M5-5 的 `load_images` docstring 里钉的那条(静默截断读起来和"就这些"一样)。

  ### 文字从 **OCR** 来,不从 PDF 的文字层来——用户否掉了复核方的默认

  用户:「**OCR 肯定比文字层好啊,就算有。OCR 能识别表格,还有公式之类的……
  为什么一定要文字?文字就是不准确啊,对很多东西来说。表格也抽不出来啊。**」

  **造了一份带表格和公式的 PDF 两边对打,他对,而且赢的地方正是表格:**

  ```
  文字层(pypdf)        Level / Dirty Read / Phantom / Cost / Read Uncommitted /
                        yes / yes / 1 / Read Committed / no / yes / 2 / ...
                        ——**每个格子自己一行,列结构整个没了**,哪个 yes 属于哪一行只能猜

  OCR + 坐标            Level | Dirty Read | Phantom | Cost
                        Read Uncommitted | yes | yes | 1
                        Read Committed | no | yes | 2
                        Serializable | no | no | 9
                        ——**行结构还在**(5 行里 4 行完整,断开那一行是复核方的 y 分桶
                        写得糙,不是 OCR 的问题)
  ```

  **OCR 赢是因为它带坐标**:文字层给的是阅读顺序,坐标给的是版面。
  而**表格的信息一半在版面里**。

  ### 公式两边都输——**那正是图的那一半在兜的事**

  ```
  文字层  T ( n ) = ■ n i=1 w i / t i     ← Σ 变成 ■,每个字符一行
  OCR     Throughput: T (n) = | wi /t     ← Σ 直接丢了
  ```

  **所以公式只有看图才行。** 这一条要写进 docstring——否则下一个人看到"我们有 OCR 文字了"
  就会想把图省掉,而公式那一栏立刻变成空的。

  ### 于是:**每一页都跑 OCR,不管它有没有文字层**;`pypdf` 不要了

  复核方原来的默认是"有文字层就抽、没有才 OCR",**那个默认错了**:
  文字层唯一的优势是便宜,而**便宜这条用户已经否过一次**(「效果不好你 1000 token 能抵消??」)。

  - **渲染**:`pypdfium2`(许可宽松,PDFium)——**这个本来就要**,图那一半靠它;
  - **文字**:RapidOCR(PP-OCRv6-small,百度的模型转 ONNX)+ **保留坐标**,
    按坐标聚成行再交给模型;
  - **`pypdf` 去掉**:真要文字层做交叉校验,`pypdfium2` 自己就能抽,不用第二个依赖。

  ### 实测数字(2026-09-12)

  ```
  准确率  中文截图 / 英文讲义两个样本,PP-OCRv6-small 全对,连空格都对
          —— 和视觉模型打平
  体积    模型 30 MB(det 9.4 + rec 20.2 + cls 0.5)+ 包 32 MB + onnxruntime 76 MB ≈ 110 MB
  速度    冷启 1.1s + 识别 0.9s/页
  峰值内存 773 MB(全新 python 进程,含 numpy/onnxruntime)
  ```

  **复核方在这里判错过一次,错法要记下来**:第一次拿 `rapidocr-onnxruntime`
  (默认带 **v4**)打,得到 `lndexError丶InterfaceError`、`thestatement`、
  `Keynumberonpage2:274`,据此报了"读不如视觉模型准"。换新版(默认拉 **v6-small**)
  重打就全对了。**我测了 v4 就报成"RapidOCR 如何如何"**
  ——这一天里第五次"测了某一个、报成那个东西"(前四次:编的 URL 测反爬、
  信文档说没有 `title` 字段、单样本定 `country`、拿一篇答不了的文章测 `chunks_per_source`)。
  **纪律:报一个库或模型的结论之前,先打印它实际加载的是哪个版本的哪个文件。**

  ### 773 MB 决定了它怎么跑:**独立进程、一次性、缓存**

  真机 3.9 GB 总量、约 2 GB 可用,Lararium 自己占 ~800 MB,`MemoryMax=2500M`。
  **不能常驻在 Steward 进程里**;跑一个用完就退的子进程,那 773 MB 是瞬时的。

  所以 OCR 不是"一轮里调的工具",是**入库时的一次性作业**:

  ```
  一份 PDF 进来 → 逐页渲染 + OCR(约 2s/页,50 页约 100 秒)→ 文字 + 坐标缓存起来
  之后 read_material / search_materials 都只读缓存,不再跑 OCR
  ```

  **顺带**:`search_materials` 因此对扫描件也有效——而这不是额外做的,是同一份缓存。

  ### 明确不做

  - **不装 tesseract**(v6-small 更小更准);
  - **不起 subagent 编排 OCR**(一条顺序的作业,不是需要决策的任务);
  - **不常驻在 Steward 进程**(见那 773 MB);
  - **不用 OCR 替代看图**(公式和图表只有看图才有);
  - **不上 PP-Structure 的表格/公式专用模型**——它们确实存在(表格转 HTML、公式转 LaTeX),
    但那是又一批模型、又一份内存,而**图那一半已经把公式兜住了**。
    真撞上"表格行拼错了导致答错"再说。

  ### 不需要 OCR 栈,也不需要 subagent

  没有文字层的页(扫描教材、拍的板书)**照样走同一条路**——图本来就给,
  而模型读图就是 OCR:三天前实测它抄一页中文一字不差。
  **别装 tesseract**(2C2G 上再养一个东西),**别起 subagent**(为一件顺序的事加一层编排)。
  ...
  **现在收到 PDF 会被认成 `application/octet-stream`、存成 `<hash>.bin`。**
  ...
  这一条**可以并进 M6-1**(那一轮正在改同一个文件的同一批表),或者在这里做。
  **注意 M5-5 那条教训**:`_sniff` 当初"两个方向都在说谎"——
  漏掉一种格式会让它**掉进 octet-stream 然后一声不响地不进模型**,
  而「把一次响亮的失败换成一次静默的失败不是修复」。

  ### 课程表:**就是一份笔记,不是一张表**

  用户原来要的是"一条条录入课程表"。有了知识库之后**它不需要任何结构**:
  在那门课下写一份叫「课程表」的笔记,或者全局一份,里面就是
  「周三 10:00 高数 / 周三 14:00 线代」。

  「我明天有什么课」她读那份笔记 + `current_time` 就能答——**而这两样都已经有了**。
  **不要为周期性建规则引擎、不要展开成每一次。**
  真到"她答错了"那天再说,而现在连那份笔记都还没有。

  ### 明确不做

  不做成绩、不做 GPA、不做课程评价、不做选课、**不做课程表的结构化**、
  **不做作业和考试**(那是 M6-6)。
