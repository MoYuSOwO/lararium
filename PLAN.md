...
- **M6-1 语音听不懂——不是缺 ASR,是微信转好的文字我们没读**
...
- **M6-2 附件改成「报个 id,要看自己调」**——**这一条把 M5-5 的一个决定翻过来了**
  ...
  **不要现在就给每个 bundle 都加一个**(M5-11 的形状:先猜再用)。
  ...
  - **不给通用的"读任意文件"工具**。每种类型该怎么读是这一类自己的事
    ...
    一个 `read_file(id)` 会把这些混成一个"看情况"的黑盒,而**那正是 M5-5 栽过的地方**
...
- **M6-6 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**
  ...
  **而"把一页转成文字"要调模型。** 所以那件事**不可能住在学习 bundle 里**
  ...
  ```
  Steward 侧(内置工具)   read_file(id, page)  渲染 + 调模型转 + 缓存   ← **唯一一份实现**
  bundle 侧(学习)        add_file / list_materials                   ← 只存"哪个 id 属于
                                                                        哪门课、叫什么名"
  ```

  **跨这条边界的唯一把手就是 id。** 所以用户两条消息前提的 `read_file(id)`
  ...
  **对,三件事各就各位:**
  ...
  读       read_file(id, page) 一套接口,所有类型、所有 bundle 共用
  缓存     键是 (media_id, page),跟着池子
  ```

  于是模型的动作链是:`list_materials(course)` 拿到**名字 + id** →
  `read_file(id, page)` 读。**名字给人看,id 给机器用**,两边都不需要知道对方的事。
  ...
  **所以:**
  ...
  图片    **不转**。read_file 给图,她看完自己决定要不要写进笔记
  ```
  ...
  **真到"我拍的板书太多了找不着"那天再说**——那时候机制现成(和 PDF 那条同一套),
  ...
  - **两样都键在 `(media_id, page)`**,所以**同一份 PDF 被两门课引用时共用**
    ——不是优化,是"一个事实一个出处"。
  - **图每次 `read_file` 仍然要重新送进上下文**(M5-5:图只在到达轮进模型),
    **缓存省的是渲染,不是发送**。这一句要写进注释,否则下一个人会以为缓存了就不用再送。

  ### `look_at_image` 怎么办

  它现在只管图片,而 `read_file` 覆盖了它。**二选一,别两个都留**
  ——两个入口做同一件事,迟早有一个漏掉新规则(M4-4、M5-5、M5-21 各栽一次)。
  **倾向:保留 `look_at_image` 的名字当图片那一支的别名,或者干脆合并进 `read_file`。
  要在回报里说清选了哪个、老测试怎么改。**
...
- **健康 bundle / 学习 bundle**(原 M6-3/M6-4):用户明确没选。
  契约的反复验证由 M6-1 的做菜 bundle 承担。
