...
- **M6-6 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**
  ...
  **归属表放哪**:学习 bundle 自己的库。按上面那条判据
  ...
  这是上一问带出来的,复核方原来没写清。判据一句话:
  **转换的存在是为了"不用看也找得到";她正在看的时候,转换是多余的。**

  ```
  走 add_file 归档的(PDF 的每一页、拍下来的板书照片)
      → **转**。因为一份 50 页讲义你不会每页都看,而搜不到 = 和没写过一样
  只是在对话里发过来、没归档的图(收银条、截图)
      → **不转**。她正在看,而她看完说的那句话本来就会留在 L0 里
        —— 那句话比转出来的文字更有用,因为它是**答你的问题**,不是通用转录
  ```

  所以:

  - **转换发生在 `add_file` 时**,对任何类型都一样(PDF 转 N 页、图片转 1 页);
  - **`read_file` 不触发转换**,它只给"缓存里有的文字 + 那一页的图";
  - 没归档过的图 `read_file` 就只有图,**而这不是缺陷**——写进 docstring,
    免得下一个人以为漏了一半。
  ...
  **前三次是同一件事:先想"怎么把它做对",而不是先问"用户会怎么用它"。**
  ...
  - **`read_file` 不触发转换**:读一张没归档过的图 → 只有图、没有文字,
    且**模型调用计数为 0**(拿计数钉死,别让它偷偷转);
