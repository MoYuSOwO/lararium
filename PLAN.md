...
- **M6-1 做菜 bundle**——**用户重新设计过:它是文档,不是表**
  ...
  **我原来那版(做法表 + 尝试表)是错的。** 我查了 schema.org/Recipe,砍掉了它九成的字段,
  ...
  ### 存成 markdown 文件,**不是 SQLite**——复核方判错过一次,记下来

  我第一版判的是"存 SQLite 里,名字是键不是路径",理由是**让路径穿越成为一类不存在的
  bug**,并类比 M5-22 的「SSRF 面归零不是防住了,是根本没有出站」。

  **用户否了,而且他对:那个类比不成立。** M5-22 那次"不出站"是**零成本**消掉一整个
  攻击面;这次"不落盘"要付的代价是**用户没法在自己电脑上打开自己的菜谱、没法 cat、
  没法复制一个文件夹备份**——而菜谱恰恰是那种你会想在灶台边用别的东西打开的文本。
  我把这个成本当成了零。

  > 「你存在 sqlite 也就没必要了……存几个 markdown 文件又不会咋样啊,
  > 为什么要吃那么大的开销呢?还不方便。」

  **所以:`data/recipes/<名字>.md`,一份做法一个文件。**

  ### 于是路径穿越回来了,而它必须用**机制**挡住,不是靠小心

  **只准有一个地方把菜名变成路径。** 那个函数是唯一入口,别处一律不许拼路径
  ——这条和 M5-4 的教训同源:`Attachment` 上**根本没有可写的 path 字段**,
  「能自报路径的附件就是路径穿越的入口,而它指向 `prompts/character.default.md`
  的那天人设就成了对话可改的」。

  那个函数里要有两道,**两道都要**:

  1. **名字白名单**:拒 `/`、`\`、`..`、控制字符、前导 `.`、空名;长度封顶。
     归一化(折内部空白、去首尾)在校验**之前**做,否则「`..` + 空格」能绕过。
  2. **算完之后再验一次落点**:`resolve()` 之后断言它在 `data/recipes/` 里面
     (`is_relative_to`)。**这道是兜底**——白名单是"我想到的那些",这道是"不管你怎么绕,
     出了这个目录就不行"。M5-22 那五步防护里 `getpeername()` 复查是同一个位置的同一个角色。

  **验收里这几条必须有,而且现在它们是真的路径校验测试**(不是"没有路径这回事"):
  `../../etc/passwd`、`..`、`/etc/passwd`、`a/b`、`.hidden`、空名、超长名、
  `紫菜蛋花面/../../x`——全部拒绝且回人话。
  再加一条:**把白名单那一道注释掉,第 2 道兜底必须仍然挡住**(反过来也要成立)。

  ### 工具:五个

  ```
  read_recipe(name)                        读一份
  write_recipe(name, content, category=)   写/整篇覆盖
  list_recipes(page, category=)            列出来,分页
  ...
  **`replace` 我第一版没给,理由是"模型整篇重打可能悄悄漏一步"。用户指出那是臆想:**
  ...
  > 「coding agent 都能 edit,为什么你臆想中它连 edit 个菜谱都做不到?给个 replace 吧,
  > append 也给,都可以给。」

  **他对,而且我漏掉的正是让它安全的那个机制。** coding agent 的 edit 安全不是因为模型
  聪明,是因为**匹配不上就拒绝执行**。所以 `replace_in_recipe` 照抄那套纪律:

  - `old` 在文件里**出现恰好一次**才改;
  - **零次** → 拒绝,并说"没找到这段,可能已经改过了或者你记错了原文";
  - **多次** → 拒绝,并说出现了几次、要求给更长的上下文。
    **绝不允许"改第一处"**——那正是会悄悄改错地方的那个行为;
  - 改完把**改动前后那一小段**回给模型,让它能自己核对(而不是只回"改好了")。

  这三条一写下来,"模型改坏"就从一个担心变成一个被机制挡住的事。
  `append` 照旧给——它是最常发生的动作,而且不可能弄丢东西。

  **★ 一个要你判的**:`write` 整篇覆盖要不要留上一版?
  有了 `replace` 之后整篇覆盖会少很多,所以这个问题的分量比我第一版估的小。
  M5-26 刚删掉一整套"留痕但没人读"的机制,按 G6 先问谁会读到它。
  **我的倾向:不留**,`write` 的 docstring 写明"同名覆盖,旧的那版不留"
  ——而且现在有文件了,用户自己 `cp` 一份就是备份。要在回报里说清理由。
  ...
  **别在这一步发明新守卫**(M5-11)。
  ...
  - **路径穿越那一组**(见上,八个用例)全部拒绝且回人话;把白名单注释掉之后
    落点兜底仍然挡住,反之亦然;
