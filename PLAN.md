...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿
  ...
  **课件是外部内容**(用户下载的 PDF 可能是任何东西),而现在我们**把它喂给模型、
  ...
  - **不装任何本地 OCR**(tesseract / rapidocr 都不装)——实测表格赢一点、
    公式和图全输,而代价是 110 MB 模型 + 773 MB 峰值 + 一套子进程;
  - **不起 subagent 编排转换**(一条顺序的作业,不是需要决策的任务);
  - **不上 PP-Structure 的表格/公式专用模型**(又一批模型又一份内存,
    而"模型 + 指令"已经把表格和公式都拿下了);
  - **不用转换出来的文字替代看图**——她当下看版式、看图、看板书仍然要那张图;
    转出来的文字是给"之后还能引用"和"能搜"用的,两样各管一头(用户定的:一页给两样)。
