...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿
  ...
  **课件(PDF)那一支是只读的,单独两个工具**(见下面那节的实测和理由):
  ...
  ```
  list_materials(course, page=1)               这门课有哪些课件
  read_material(course, name, pages="1-5")     按页抽文字;没有文字层就说清楚
  ```
  ...
  **最后那半句是对的,而且它指出了一个框架错误**:笔记和课件是两种东西。
  ...
  ### 于是两支,**不需要 OCR 栈,也不需要 subagent**

  1. **有文字层** → `pypdf` 抽文字。绝大多数课件是 LaTeX / PPT 导出的,都有文字层。
  2. **没有文字层**(扫描的教材、拍的板书)→ **渲染成图,走已有的视觉路**。
     **那条路就是 OCR** ——M5-5 建的 `look_at_image` 加上视觉模型,
     而三天前实测它抄一页中文一字不差。

  **所以别装 tesseract、别起 subagent。** 加一个 OCR 栈是在 2C2G 上再养一个东西,
  而我们已经有一个会读图的模型在环里了;起 subagent 是为一件顺序的事加一层编排。

  ### 但有三条硬约束,不写清楚这一支会撑爆上下文

  1. **按页读,不许整份读。** 一份 50 页讲义的文字可能十几万字——
     `read_material(name, pages="1-5")`,**页数和字数都要封顶**,截断要说清少了多少
     (M5-22 立的形状:「静默截断读起来和'就这些'一模一样」)。
  2. **渲染成图那一支受 `MAX_IMAGES_PER_TURN = 4` 管**(M5-5 的机制,不许为这个放宽)。
     所以"看第 7 到第 20 页"是办不到的,**工具要说实话**:一次最多几页,
     让模型自己分轮,而不是悄悄只送前 4 页。
  3. **图只在到达轮进模型**(M5-5)。所以渲染出来的页**不会留在上下文里**——
     她读完得当场把要点写进笔记,否则下一轮就没了。**这一点要写进 docstring**,
     它是这条路能不能用起来的关键,而不是一个实现细节。
