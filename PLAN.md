...
- **M5-15 流水记错了没法改**(M5-14 验收时查清)

  `finance` 只有 `record_expense` / `query_spending` / `list_recent`——**只能加,不能改、
  不能删**。而 `memory` 的 `propose_fact` 有 `kind: add|amend|…`:
  **账本(很少变、还有门控)有更正路径,流水(模型天天写、偶尔手抖)反而只进不出。**

  后果不是"错一条",是**一次填错必然长成两三行垃圾**。真机实测两轮:

  ```
  第 0 轮  买菜 62 → 记成 5.0;下一轮补记 62.0/买菜 + 22.0/咖啡   → 多一条 5.0
  第 1 轮  咖啡 22 → 记成 49.0
                   → 试 amount=-27.0 note「咖啡金额更正(49→22)」 ← 被工具拒(须 >0)
                   → 又记 49.0 note「咖啡(金额有误待处理)」
           → 多两条 49.0,而 22 一直没进去
  ```

  **模型的行为是对的**:它发现记错了、想抵消。是我们没给路,它才拿仅有的工具去凑。

  **要做的**:给 finance 一条更正路径。设计上要先定几件事,别一上来就加个 `delete`:

  1. **改还是冲?** 直接改那一行,历史上就没发生过错记;记一条冲正,账目可审但读起来吵。
     账本那边用的是 amend(改),但账本有快照可回滚,流水没有。
  2. **拿什么定位?** `list_recent` 现在不返回 id。要改就得能指名道姓,而"最近那一笔"
     这种相对定位在并发/多轮下会指错。
  3. **要不要门控?** 记一笔不需要用户点头(流水不是账本),但**改和删**是另一回事
     ——尤其"删"可以被用来抹掉痕迹。倾向:改可以自助,删要么不给、要么走命令端点
     (和 `/approve` 同一条路,由用户来)。
  4. **别顺手放宽 `amount > 0`。** 那条挡住的是"记一笔负数当更正"这种取巧,而它正是
     这次模型试过的路。要更正就给正经工具,不是把校验拆了。

  **验收口径**:构造一次"记错 → 更正"的完整来回,断言**账上恰好一条、金额是对的那个**;
  以及反向,更正工具不许能凭空造出一笔(它只能作用于已存在的行)。
