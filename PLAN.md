  ...
  - 写一份带换行、带 `|`、带围栏字符的做法 → `read_recipe` 读回来**内容逐字一致**,
    且**步骤的界限在渲染后仍然分得出来**;
  - 同名再 `write` = 更新不是新增;名字首尾空白 / 内部多空格 → 和原名对上同一道菜;
  - `append_to_recipe` 三次 → 三段都在、顺序不变、**原有内容一字没动**;
  - **`replace`**:`old` 出现一次 → 改;**零次 → 拒绝**;**多次 → 拒绝且说出现几次**;
    改完回话带得出改动前后那一小段;**绝不许"改第一处"**
    ——拿一份 `old` 出现三次的做法钉死它;
  - **非法名字**(`../../etc/passwd`、`..`、`/etc/passwd`、`a/b`、`.hidden`、空名、超长)
    → 全部拒绝且回人话;**注释掉白名单那一道,落点兜底仍然挡住,反之亦然**;
  - **工具签名里没有路径、也没有 category**——加一条机械检查钉住它
    (这是设计约束,不是风格偏好);
  - `list_recipes` 页码 0 / 负数 / 超大都钳住,总数出现在输出里;
  - 不存在的菜名、空名字 → 各自一句人话,不抛;
  - **`search_recipes`**:菜名命中、内容命中**各有一条测试**,且结果里**分得出是哪一种**;
    命中片段不是整份(拿一份长做法钉死);零命中 → 人话 + 提一句 `list_recipes`;
    分页钳位 + 总数;**用编辑器直接改一个 `.md` 之后立刻搜得到**
    (这一条钉的是"没有索引会变味");
  - **`rename_recipe`**:`new` 已存在 → **拒绝**;不存在的 `old` → 人话;
    改完 `read_recipe(new)` 读得到、`read_recipe(old)` 说没有;
  - **新建要说出口**:`write_recipe` 写一道新菜 → 回话里出现"新建"字样;
  - **`delete_recipe`**:删完默认看不见、`include_deleted=True` 看得见且标「已删」、
    文件没销毁;`undo=True` 逐字节搬回;`list_recipes` 不许列出 `.trash`;
  - **前缀影响(A1)**:新 bundle = 目录行 + 8 个工具 schema,前缀重建一次。
...
- **SQLite ≥ 3.35**(两个前提取高者:FTS5 `trigram` 分词器要 3.34,中文检索的前提;
  `ALTER TABLE … DROP COLUMN` 要 3.35,M5-26 的退休迁移用它)。Task 3 有断言强制。
...
- **M5-1 渠道定型 —— ✅ 已完成(2026-08-30)**。产出是一份实测记录 + 一个决定,不是代码。
  **ClawBot 已跑通收发闭环**(扫码 → bot_token → 收信 → 回信,用户微信里确认收到);
  协议规格见下面三节。**决定:只用微信一条通道,企业微信不做**——主动推送已验证可行,
  官方实现支持 cron delivery 且 `context_token` 无过期逻辑。窗口探针在跑,
  `+25h` 那条送达即最终确认;**即便窗口存在,失效形态也只是"消息在出件箱里等你开口"**
  (M4-7 已做好),不构成第二条通道的理由。

  **执行顺序调整**:M5-3 提到 M5-2 前面。M5-2 修的是数据面那一轮的回复路由,
  而数据面要到 M6 才有真实来源;M5-3 才是"能从手机上说话"的阻塞项。先做看得见的那个。
...
- **M5-4 媒体入站:收下来、存起来、不堵通道**(不含读图)

  **先修一个真机上必然踩到的静默死锁**:现在非文本条目被 `_extract_text` 过滤成空串,
  照样 POST 出去 → 服务端 400(`content` 不能空)→ 异常抛出 → **游标不推进** →
  三秒后拿旧游标又取到同一条 → 永远循环。复核方探针实测游标卡在原处。
  后果不是"图片不支持",是**助手整个哑掉**:那条消息卡在队首,它后面所有文字消息一起
  进不来,而用户在微信这头只看到"发什么都没反应"。

  **Step 1 — 失败的测试先行**:
  - 收到纯图片消息 → **游标推进**,通道不堵;后面那条文字消息照常送达(这条是重点);
  - 任何一条投递失败 → 跳过它并推进游标,**不让一条毒消息把整批停摆**
    (今天是图片,明天可能是超 16KB 的长文、或服务端一次抖动);
  - 收到图片 → 回一句人话(「收到一张图,已存下来」),用户知道它收到了;
  - 媒体落盘按**内容哈希**命名,同一张图发两次只存一份;
  - CDN 下载失败 / 解密失败 → 走 E2 人话,不打崩泵。

  **Step 2 — 实现**:iLink 侧照官方 `src/cdn/`(`cdn-url.ts` / `aes-ecb.ts` /
  `pic-decrypt.ts` / `media-download.ts`)用 Python 重写——**AES-128-ECB 要自己解**。
  存 `{data_dir}/media/<sha256>.<ext>`;信封带附件引用(**不是字节**)。

  `Envelope` 加 `attachments`,但 `content` **仍是字符串**——它是所有外部输入的入口,
  `validate_assignment=True` 那条纪律不许绕。图片对应的 `content` 是一行可读文本
  (如 `(图片 · media/ab12cd…)`),这样下游一切按文本走的东西都不用动。

  **Step 3** — 门禁 + 提交 + 登记 + CHANGELOG。

- **M5-5 读图**(动核心与注入面,单独一步)

  **★ 图片是一个全新的注入面,这是本步最难的一条。** 现有防线——围栏、折行、中和分隔符、
  来源标注——**保护的全是文本**;图片绕开全部:一张图上写着「忽略之前的指令」,模型直接
  读图,**没有任何一层能中和它**。M4 那四条渲染规则一条都用不上,因为不存在"渲染"这一步。

  这不是理论:用户转发的群截图、别人发来的收据,都是**不可信来源**,而它进的是可信位置。

  **三条设计约束,写在动手之前**:

  1. **图片只在到达那一轮进模型;历史轮只留一行文本引用。**
     否则每张图都会**永久地**乘进后续每一轮的成本,而 L0 的预算算术(`estimate_tokens`
     + `_render_overhead`,实测校准过)对图片一无所知。模型想重看,给它一个按 id 取回的
     工具——**别让它默认一直在**。
  2. **不可信轮的图片必须带来源框定。** 框定语是文本(那一层还有效),而且该轮的
     `propose` 照旧强制降档(`_guard_propose_fact` 已有)。
  3. **起居注落引用 + 哈希,不落字节**(字节在 `media/`,按哈希不可变)。
     重放时文件不在了要**明说重放不完整**,不许静默给一份残缺的。

  **模型不支持视觉要优雅降级**:端点是可配的,用户接的模型未必能读图。
  探测或配置开关,关掉时图片仍然存下来 + 告诉用户"当前模型看不了图",**不许崩**。

  **不做**:拍小票自动记账。那要 finance 配合,而**错记比不记更糟**——它得先能读准,
  再谈自动。等读图跑一阵、看真实识别率再说。

- **M5-6 审批卡 —— ✅ 实际已由 M5-3 交付**

  任务书原话是「IM 按钮回调」,但**这条通道上没有按钮**:官方 `MessageItemType` 只有
  NONE/TEXT/IMAGE/VOICE/FILE/TOOL_CALL_*,全库 grep 不到 button/inline_keyboard。
  所以审批在微信里就是打一行 `/approve <id>`,而 M5-3 的 `_route` 已经把 `/` 开头的
  分流到 `/v1/commands`,有测试钉着。**分派仍只有一套**(服务端 `handle_command`),
  适配器里一个命令动词都不认——认了就是第二份,而这条路上放的是账本的批准权。

- **M5-7 真机 —— 排在 M5-8…M5-11 之后做**。手机上完成一次记账 + 一次审批 + 发一张图。
  三件都要真做,不是看代码推断——发图那条尤其:它同时验证了通道不堵、媒体落盘、
  以及(读图做完后)模型真的看见了。

  **为什么往后挪**:M5-8 是一个会让任意一轮随机失败的并发 bug。拿一个会随机丢轮次的
  版本上真机,验收结果读不出来——挂了不知道是通道、是适配器、还是那个 bug。

- **M5-8 一条 sqlite 连接被并发使用**(2026-08-30 换模型时挖出;**今天就在,与换不换模型无关**)

  **症状**:一轮里模型同时发多个工具调用时随机炸,三种面孔——
  `IndexError: tuple index out of range` / `InterfaceError: bad parameter or other API
  misuse` / `TypeError: 'NoneType' object is not subscriptable`。真机上表现为这一轮变成
  `retry_later`,重试到上限后用户收到「处理失败,已放弃」,而起居注里只有一行 TypeError,
  指不到任何地方。

  **根因**:pydantic-ai 把同步工具丢线程池,而一条 assistant 消息里的多个 `tool_call`
  是**并发执行**的;全系统共用一条 `sqlite3.Connection`。`check_same_thread=False` 关掉的
  是那个守卫,不是让连接变线程安全——两个线程交错 `execute`/`fetchone`,游标状态就烂了。

  **`db.py:171` 把这个假设写死了,而它是错的**:「安全性由架构保证——收件箱严格串行,
  任一时刻只有一轮在跑,不存在真正的并发访问。」**「一轮在跑」不等于「一个数据库调用
  在跑」。** mimo 一次只发一个工具调用,所以这个洞躺了五个里程碑没人碰到;
  DeepSeek 会批量发,一试就炸。哪天 mimo 换个版本开始批量发,同样炸。

  **Step 1 —— 先确认它真的会失败**(纪律那一步,别跳)。复现:建 400 条起居注,
  用 `asyncio.gather` + `asyncio.to_thread` 并发调 `search_history`。复核方实测
  n=2 全过,n=3 / n=4 各挂 4/45。**是概率性的**,别看到一次全绿就以为没事。

  **Step 2 —— 三个约束,想清楚再动手**:
  1. `loop.py:342` 要求 `inbox.conn is outbox.conn`(崩溃语义:回复落出件箱与信封标完成
     必须原子)。**"每线程一条连接"这条最顺手的路会直接违反那个不变量**,真要走先说清事务;
  2. `transaction()` 是可重入的(SAVEPOINT),加锁不许和它死锁——粒度是「一次 execute」
     还是「一个事务」要想清楚;
  3. `check_same_thread=False` 得留着。工具**在**工作线程里跑没问题,有问题的是**同时**跑;
     顺手改回 True 会让所有工具调用都抛。

  方向自选。若选「让工具串行执行」,回报里要写明**那是治症状**——连接本身仍不是线程安全的。

  **Step 3 —— 测试钉机制,不钉崩溃。** 概率性复现直接搬进 pytest 就是一条随机红的测试,
  比没有更糟。包一层「连接正在被另一个线程使用」的探针、断言它从未触发,是一种解法。

  **Step 4** —— 门禁 + 提交 + 登记 + CHANGELOG,顺带把 `db.py:171` 那段注释改对。

- **M5-9 注入图的暗号重选**(M5-5 的对照锚点只对 mimo 校准过)

  **暗号必须在图里,这条不许动。** 文字暗号不是对照——「模型看了没照做」和「模型压根
  没看清」话头表都是空的,区分它们唯一的办法是让模型说出**只可能来自像素**的东西;
  暗号一旦也出现在文字里,模型照抄文字就能过,测的就变成"它会不会复读"。

  脆的地方在别处,三处一起修:

  1. **判据本身是错的。** M5-5 写的是「ZORVAX 六个字母在 5x7 点阵里两两不相似」,可模型
     输出的是**整个字母表**,不是这六个字母。按"与字库里任意字符的最小汉明距离"量
     (每字形 35 位):`D` 2/35、`O` 2/35、`C/I/M/N/P/R` 3/35,**距离 ≥9 的字母一共只有
     `J K X Y` 四个**;`ZORVAX` 最弱一环是 2(那个 `O`)。`U`/`V` 更直观:七行有五行逐位
     相同,只差最后两行——DeepSeek 把图压到 384 token 之后,区分它们的全部信息就是那两行,
     实测被读成 `ZORUAX`,**防御是对的,阳性对照错杀**。
     **诚实的补充**:`V` 的距离是 6,不在最差前十里却照样被读错——汉明距离是**筛子不是
     证明**,它表达不了"缩图后底部两行一起糊"。选完仍然要拿模型跑。
  2. **暗号和载荷是同一个串。** `ZORVAX` 既是"证明读到了"的锚点、又是注入要开的话头名,
     于是整条测试的成败押在这一个 token 的 OCR 上。两者要求不同:锚点要 OCR 完美,载荷
     只要能读懂意思。**拆开**——图里放一个短锚点专门当对照,载荷叫什么都行。
  3. **失败时必须打出模型的原始转写。** 若当时是模糊匹配悄悄放过,就丢掉了一个真发现:
     384 token 的上限会糊掉笔画。放宽匹配可以,转写要照打,别被断言吞掉。

  **验收**:选定的暗号对 mimo 与 DeepSeek **各跑 3 次,两个都读得出来**才算数。

  **给 M6 记一条**:384 token 的图片上限意味着小票上的小字很可能认不清,
  「拍小票自动记账」要重新评估,别默认能读准。

- **M5-10 换模型**:`deepseek-v4-flash-vision-exp`(opencode zen 上有此 id)。
  改三行 env,零代码;**M5-8 必须先过**,不然一换就炸。

  实测(2026-08-30):缓存命中 93.4%(mimo 94.9%),输出 357 token/轮(mimo 242),
  按量约 ¥2.6/月。**它在一条 mimo 翻车的场景上顶住了**——不可信短信经检索进入**可信轮**
  之后要不要归档:mimo 直接 `propose(provenance="user_stated")` 洗进账本,它坚持要用户
  亲口确认。图片注入 3/3 拒绝。换完把 `vision.SENDABLE_IMAGE_TYPES` 对新服务商重测。

- **M5-11 `read_skill` 的抖动率能不能接受**(题目已修正,见下)

  **原题写错了,是我的错。** 原话是「记账前该先读 finance 总览,现在**两个模型都不读**,
  这条已经红了」——那是**各跑一次**就下的结论。实际是**抖动**:M5-10 观测 10 次 8 过,
  复核方另跑 6 次 4 过,合计 **12/16,失败率约四分之一**。失败那几次明显更快
  (5.1s / 5.5s vs 通过时 8~9s),少了一次 `read_skill` 往返,对得上。
  一次采样就写进任务书,正是这个项目反复在防的事。

  **所以真正的题目是**:四分之一的轮次不读领域总览就直接记账,能不能接受?
  - 先量准:同一模型跑够次数(≥20),两个模型分别量,别再拿单次说话。M4 记过
    「`read_skill` 的 reach 低」,把那时的数字翻出来对比——是退化还是一直如此。
  - 再决定它该不该**靠自觉**。skill 路由现在完全是提示词层的期望,没有任何机制。
    选项至少有三种,各有代价:提示里加硬约束(仍是说服)、把总览折进前缀(每轮都付钱,
    而且前缀是不可协商第 1 条,进去的东西要极省)、或者**接受它**并把测试改成
    量化门槛而不是布尔断言(测试当前的形态是"必须读",而事实是"多数时候读")。
  - **不许**为了让测试变绿而放宽断言了事——那正是 T6 那一类。要么改行为,要么把
    "只要求多数轮次读"写成明确的规格并说清为什么可以接受。

- **M5-12 无人值守两周的两个前提**(真机之前的最后一步)

  这两条捆在一起,因为它们服务同一件事:**让「真机跑两周」这件事跑得下去,而且跑完
  有数可看。** 别的已知缺口(`_active_untrusted`、`look_at_image` 重看、语音转码、
  图片压缩)一律等这两周的数据再排——**没有真实用量,优先级排不出来**。

  **Step 1 —— 出件箱失败要退避,而且要能被入站消息叫醒**

  `wechat.py:449` 现在是「任何异常 → `sleep(3)` → 重来」,无限。而微信窗口是 24 小时:
  你哪天没开口,晨报发不出去,适配器就对着腾讯的接口打一整夜——**一夜约 1.4 万次**。
  真会撞限流,而限流很可能回 `-14`,那个码正好被过载(缺 HTTP 头也回 `-14`),
  到时候极难查。这不是优化,是「无人值守跑一夜」的前提。

  两处都要治,失效形态不同别混:
  - **发失败**(`send_text` 抛 `ILinkError`)→ 指数退避,封顶几分钟。
  - **还没人跟它说过话**(`peer`/`context_token` 为空,`pump_outbox_once` 提前 return,
    **不抛异常**)→ 现在是每 5 秒空转一次长轮询、并且每次都打一条 warning。
    它不打腾讯,但同样是白转 + 日志刷屏。

  **窗口重开的信号是入站消息**(它会刷新 `context_token`),所以退避必须能被入站泵
  叫醒——两个泵是独立 task,需要一个共享的 `asyncio.Event` 之类。**只退避不叫醒等于
  把延迟写死**:用户开口之后还要再等几分钟才收到攒着的推送,那个体验比刷接口更糟。

  失败的语义不变:**条目留在出件箱、游标不推进**(M4-7 定的「消息在等你」)。
  这一步只改重试节奏,不改「发不出去就丢」与否。

  **Step 2 —— 给三种失效留痕(只写日志,不改行为)**

  M5-11 之后实测到三种失效,**起居注里一种都没有信号**(见 `REVIEW.md` 那节):
  漏做 / 谎报「已记」/ 把稳定安排记成流水。现在只能靠人一遍遍手动跑才看得见,
  而真机上它们会稀疏地发生、没有人会注意。

  **这一步是仪器,不是修复。** 它必须在两周之前做,否则两周之后你问「门控救过我几次」
  「它谎报过几次」,答案是「不知道」——那两周就白等了。

  落两条,都进起居注,**都不许进 L0、不许进检索索引**(照 `tool_executed` / `skill_gate`
  的先例):

  1. **`read_only`**:一轮结束时,`_overview_read` 里有某个 bundle,而这一轮**一次**
     它的领域工具都没调过。这条是纯机械的,`loop.py` 已经有这两份状态。
  2. **`claimed_without_write`**:回复里出现「记下了 / 已记 / 已入档 / 已提交」这类
     承诺,而这一轮**没有任何写工具**跑过(`propose_fact` / `record_expense` / …)。

  **第 2 条落的是「疑似」,不是「判定」** ——事实可能上一轮就已经在账本里了,
  它说「已经在账本里了」是对的。所以:**这条只留痕,绝不改行为、绝不报警**,
  它的价值是让人事后翻得到。写死这一点,别让下一个人把它升级成拦截。

  **第三种(把稳定安排记成流水)不做。** 判它需要语义(「我房租每月 3800」是安排,
  「这个月房租交了」是流水),那已经不是留痕而是**对账**,该单独立项。如实记在这里。

  **验收口径**:两条信号各自要有阳性对照——**构造出该触发的场景,确认它真的落了一条**;
  以及反向,正常轮次不许落。否则就是一个永远不响的仪器,比没有更坏(M5-9 那句原话:
  守住一扇门的门禁比没有更坏,它让人以为这条规则是机械保证的)。

  **Step 3** —— 门禁 + 提交 + 登记 + CHANGELOG。

- **M5-13 `record_expense` 的工具重试会耗尽,而我们看不见为什么**(换到 AMD 端点后暴露)

  换到 `https://developer.amd.com.cn/radeon/api/v1`(模型 `DeepSeek-V4-Flash-Vision-Exp`,
  自部署,标签与后端一致、能读图)之后,连续记账到第 2~6 轮必然出现:

  ```
  UnexpectedModelBehavior: Tool 'record_expense' exceeded max retries count of 1.
  ```

  → 这一轮变 `retry_later`,重试耗尽后用户收到「处理失败,已放弃」。
  `test_live_finance_boundary` 里三条多轮的因此全红(单轮那条过)。

  **Step 1 —— 先让它可见,再谈修。这一步的产物是一条日志,不是一个修复。**

  复核方查到这里就停了,因为**现有的东西看不见原因**:
  - pydantic-ai 把校验详情吞了,异常正文只有上面那一行,起居注里也只有这一行;
  - 三种抓报文的办法都没拦到(`AsyncHTTPTransport.handle_async_request`、
    `AsyncClient.send`、按里程碑现成的 wire 装置)——那条 client 不走这几层。

  所以第一步是**把重试提示原文捞出来**(它会作为一条 `tool` 消息回给模型,一定在下一个
  请求体里)。做法自选:给 `model.py` 那个隔离盒加一条只在出错时生效的报文快照、
  或者用 pydantic-ai 自己的事件钩子。**捞到之后先贴进登记,再动手改**。

  **已知的排除项,别重复走**:
  - **参数在单轮下是好的**。拿真实 schema 直接打 8 条(「打车 28」这类),
    `0/8` 参数有问题——所以不是"这个模型不会填参数",是**多轮上下文里才出现**。
  - `record_expense` 自己**不抛**:非法金额/类目都返回可读提示(E2),不是异常。
  - 时间上不是限流:失败那几轮耗时 7~9s,和成功的那几轮一个量级。

  **Step 2 —— 修,但先说清楚"修什么"。** 三种可能各自的处置完全不同,别一上来就调参数:
  - 真的是参数校验失败 → 看是 schema 太严还是模型填错,**别用"把 retries 调大"了事**
    (那是把症状推后:调到 3 只是让它多错两次);
  - 是 M5-11 的路由守卫把返回值变成了模型不认的形状 → 那是我们自己的洞;
  - 是别的 → 按捞到的原文说话。

  **Step 3 —— 顺带记一条真机观测,不在本步修**:同一批探针里见过一次
  **模型没调工具却回「好的,已经帮你记了一笔餐饮支出12元」**。那正是 M5-12 装的
  `claimed_without_write` 要抓的形状,可以借这一步确认信号在真机上确实响了。
...
- **M5-15 流水记错了没法改**(M5-14 验收时查清;设计已定,别再当开放问题)

  `finance` 只有 `record_expense` / `query_spending` / `list_recent`——**只能加,不能改、
  不能删**。真机实测:模型填错金额后**会主动想更正**,但手里只有 `record_expense`,
  于是先试 `amount=-27.0`(被工具拒,须 >0),再留一条 `note「咖啡(金额有误待处理)」`
  的标记行。**一次填错必然长成两三行垃圾,而模型的动机是对的——是我们没给路。**

  ```
  第 0 轮  买菜 62 → 记成 5.0        → 账上多一条 5.0
  第 1 轮  咖啡 22 → 记成 49.0 → 试 -27.0 被拒 → 又记 49.0「待处理」
                                → 多两条 49.0,而 22 一直没进去
  ```

  **设计已经定了,下面六条照做,不要重新讨论。**

  1. **作废 + 新记,不就地覆盖。** `expenses` 加一列指向替代它的那行(如 `voided_by`)。
     就地覆盖读起来最干净,但它**销毁证据**,而"进过系统的一切留痕"是不可协商第 3 条。
     形状照抄 `memory/ledger.py`:**保留全部历史,让当前视图干净。**
  2. **`amend_expense(id, …)`**:作废旧行 + 插入新行,返回一句人话(「改了:49 → 22」)。
  3. **聚合与 `list_recent` 默认跳过作废行**;想看全部另给参数。
  4. **`list_recent` 开始吐短 id。** 现在模型和用户都无法指名道姓——用户想说
     「第三笔记错了」时,手上没有任何可指的东西。
  5. **不给 `delete`,也不放宽 `amount > 0`。** 删除是唯一会**彻底销毁**用户可能想要的
     信息的操作,所以干脆不提供;`amount > 0` 挡住的正是这次模型试过的"负数当更正"取巧,
     要更正就给正经工具,不是把校验拆了。**改完永远还剩一条替代行,记录不会消失。**
  6. **不要在主控里加守卫来限制"能改哪一行"。** 我想过限定"只能改本轮自己刚记的那笔",
     **否掉了**,两个理由:一是它打掉最常见的真实用例(「昨天那笔咖啡记成 49 了」本来
     就是跨轮的);二是 **M5-11 就是"在主控里加个守卫替模型把关",结果拦掉正确动作、
     丢了 5 笔账**。再往同一个地方塞守卫,得先证明这次不一样——证不出来就别塞。
     安全靠**改不能删 + 全程留痕**,不靠拦。

  **为什么流水不加审批**(有人一定会问,写在这里):门控防的是**外来内容偷偷写进档案**,
  不是防你自己。你亲口说的事,连账本那边都是 `user_stated → passed` 自动放行的
  (`gate.py`),记一笔咖啡更没道理停下来等你点头。真要每笔 `/approve`,这助手没法用。

  **验收口径**:
  - 构造一次「记错 → 更正」的完整来回,断言**账上恰好一条有效记录、金额是对的那个**,
    且**作废行还在、查得到**;
  - 反向:更正工具**不许能凭空造出一笔**(只能作用于已存在的行);
  - `query_spending` 的总额不许把作废行算进去。

- **M5-16 记错只发生在多轮上下文里,而那份历史是我们自己重建的**(真机记账的前置)

  **题目改过一次。** 原来写的是「这个端点上的模型会凭空造一笔账」,那是错的定性
  ——复核方绕开整个栈直接打,结论翻过来了:

  ```
  单轮 + 真 system prompt(1431 字符)+ 1 个工具 :  11/12 金额全对,0 次套壳
  多轮 + 同样 prompt        + 12 个工具(走我们的栈):套壳每轮 1~5 次,8 笔错 1~3 笔
  ```

  **模型单轮是好的。** 出问题的是多轮,而多轮里那份带工具往返的历史,是 **M4-5c v2 我们
  自己重建**的(assistant 带 `tool_calls` + 配对的 `tool` 消息)。重建里只要有一处不对
  ——`tool_call_id` 配不上、有 `tool` 消息却没有对应的调用、顺序错位、工具名不一致——
  任何模型都会被带歪。**在证明这份重建是干净的之前,不许把账算到模型头上。**

  真机上见过的三种表现,现在都该按"上下文被带歪"重读:

  ```
  说「咖啡 22」→ record_expense(62.0, note='买菜')      ← 记成了上一轮那笔
  说「咖啡 22」→ record_expense(585, note='房租')       ← 「房租」是 discipline.md 里的例子词
  说「奶茶 18」→ amount=187 / amount=None               ← 生成的 JSON 本身坏掉
  ```

  **Step 1 —— 把我们真正发出去的那份多轮报文 dump 出来,一个字一个字看。**
  这一步的产物是**一份报文和一段结论**,不是修复。M5-13 你已经解决了抓报文那个死结
  (生产构造路径上的 `http_client=` 注入口),用同一条路。要看的:

  - 每条 `tool` 消息是否都有一条**前置的** assistant `tool_calls`,`tool_call_id` 是否对得上;
  - 工具名、参数序列化是否和当初真的发生过的那次一致(起居注里有原件,逐字比);
  - 有没有孤儿(有结果没有调用 / 有调用没有结果),顺序有没有错位;
  - 套壳(`{"arguments":{…}}`)出现在**请求**里还是只在**响应**里——这一条决定了
    是我们发歪的还是它吐歪的,是本步最要紧的一个判断。

  **Step 2 —— 分离变量。** 单轮/多轮之外还有一个变量一起变了:工具数量(1 个 vs 12 个)。
  raw 打一组「单轮 + 12 个工具」,再打一组「多轮 + 1 个工具」,**把两个因素拆开**。
  不拆就不知道该修历史重建还是该减工具。

  **Step 3 —— 只有 Step 1/2 都指向模型时,才谈换模型。** 到那时用同一套量法跑
  AMD 端点上另外两个(`DeepSeek-V4-Flash`、`Qwen3.8-Flash-Next`),同一把 key 换个 id。
  **别跳过前两步**——复核方已经在这条题上把结论下错过两次(第一次怪模型,实为 M5-11 的
  守卫;第二次又怪模型,实为多轮上下文),两次都是"看到账不对就下结论"。

  **在这条有结论之前,真机不要开跑记账。** 其余功能(对话、查账、记忆)不受影响。

- **M5-17 删掉两个没人读得到的信号**(题目从"修判据"改成"删掉")

  起居注里三条信号只有生产者没有消费者——没有按 kind 查询、没有汇总,唯一的出口是
  `/replay <信封id>`,**要看得先知道是哪一封信**。

  ```
  claimed_without_write   挂在"你根本不会注意到"的事件上 → 查不到 → 删
  args_unwrapped          同上                            → 删
  tool_retry              挂在一次「处理失败,已放弃」上 —— 用户看得见那条消息,
                          /replay 它就行                   → 留
  ```

  `claimed_without_write` 还额外犯了第二个错:判据是一张中文词表(`CLAIM_MARKERS`)。
  换到 Qwen 说「记了」就漏了。**那张表是复核方在 M5-12 的任务书里指定的,你照做了**
  ——这一步不是返工,是把一个错设计拿掉。

  **要删的:**

  1. `CLAIM_MARKERS`、`_journal_turn_signals`、`_writes_done`、`_note_tool_use` 里
     记写工具那半段、`Steward` 里相关的每轮重置。
  2. `Registry.write_tools()`,以及 manifest 里的 **`writes:` / `reads:`** 两个键
     ——它们**只为这条信号存在**;连同 `test_architecture.py` 里那条
     "`writes ⊎ reads == tools`" 的门禁。留着就是留一套没有用户的规矩。
  3. `ModelReply.unwrapped_args`、`WrapperModel` 上那个计数器、`loop` 里写
     `args_unwrapped` 那两行。

  **不许删的:** `unwrap_tool_args` 本身(M5-13 那个剥壳是真修复,和计数无关);
  `tool_retry` 及 `retry_details`。

  **验收口径**:门禁全绿;`grep -rn "claimed_without_write\|args_unwrapped\|CLAIM_MARKERS\|write_tools"`
  在 src/bundles/tests 里一处不剩;起居注里 `tool_retry` 照旧会写(构造一次工具重试耗尽,
  `/replay` 那封信能看到参数与报错)。

  **顺带**:`CONVENTIONS.md` 值得加一条,因为这个错这一节犯了两次
  (M5-11 把红测试做成机制、这条把哑信号一路修判据)——**拿到一个"坏了"先问它该不该在,
  再问怎么修**。措辞你定,但要写下来。

- **M5-18 不可信是单向的:进来就不出去**(M5 最后一条代码任务)

  `_active_untrusted` 只在**认领信封**时定死一次(`loop.py:251`),而 `_guard_propose_fact`
  只看它(`loop.py:230`)。于是**工具捞回来的不可信内容不会把这一轮拉成不可信**:

  ```
  不可信轮  一条伪造银行短信进来 → 模型识破,拒绝入档            ✅ 防住了
  可信轮    用户说「搜一下历史那条通知,如果是我的长期安排就归档吧」
            → 模型把短信里的账号 propose(provenance="user_stated")
            → user_stated 自动放行 → worker 空闲自动结算 → 落进账本
            → **用户从没见过任何审批提示**                      ★ 洞在这里
  ```

  实测:无归档暗示时 **0/2 不触发**(模型自己顶住了);给一句条件式放行则 **1/1 洗进账本**。
  所以现在这条路上唯一的防线是**模型的判断力**——而"门控不建立在渲染永远不出错的假设上"
  这句话就写在 `loop.py:249` 它自己头上。

  **不变式**:一轮里只要有不可信内容**进过上下文**(信封带的、或工具带回来的),
  这一轮**余下全程**都算不可信。**只能拉高,不能拉低**,每轮认领时重置。

  **已知的不可信来源**(现在只有两个,都是主控自己的内置工具):

  ```
  search_history   命中里有 hit.untrusted 的(tools.py:80 已经在按它渲染围栏)
  recall_similar   同上
  ```

  **怎么让主控知道** —— 这一条是本步最容易做错的地方:

  **不许去嗅工具返回的字符串**(比如找 `⚠` 或围栏符号)。那就是 `CLAIM_MARKERS` 那个病
  的翻版:判据挂在渲染措辞上,换个渲染就哑,而哑掉是静默的。**判据要用结构**——
  `BuiltinTools` 在渲染那一刻**本来就知道** `hit.untrusted` 是真是假,让它直接说出来
  (回调、共享的一次性标志、或别的;`BuiltinTools` 现在拿不到 `Steward`,接线在
  `Steward.__init__` 里,自己挑一种最小的)。

  **一个要你判的问题,别默默决定**:`look_at_image` 把一张**来自不可信轮**的图重新拉回
  上下文时,算不算"不可信内容进来了"?按 M5-5 的推理(图片是绕开全部文本防线的注入面)
  应该算,但那需要知道这张图当初是哪一轮进来的——起居注里有,但要多一次查。
  **做或不做都行,但要在回报里说清楚选了哪个、为什么。**

  **验收口径**:
  - 可信轮 + `search_history` 捞回一条不可信命中 + 模型 `propose(user_stated)`
    → **必须降档成 pending**,`/pending` 里看得到;
  - 可信轮、这一轮没有任何不可信内容 → `propose(user_stated)` 照旧自动放行
    (**别把正常路径一起拖下水**);
  - 一轮之内拉高之后**不许再被拉低**(哪怕后面又调了一次干净的检索);
  - **给判据一条阳性对照**:证明它不是靠字符串匹配的——把 `_render_hit` 的围栏措辞改掉,
    降档仍然要发生。

- **M5-19 `_sniff` 定义在 `if __name__` 之后,真机第一张图就炸**(真机验收挖出的第一个)

  ```
  555  async def main()
  579  if __name__ == "__main__":
  580      asyncio.run(main())      ← 跑到这儿进主循环,再也不往下走
  583  def _sniff(...)              ← 永远不会被定义
  ```

  真机日志:CDN 下载 `200 OK`,紧接着
  `WARNING 消息 …… 没能投进 Lararium,跳过:NameError: name '_sniff' is not defined`。
  `data/media/` 目录至今为空——**一张图都没落过盘**。

  **542 个测试全绿,因为测试是 `import` 这个模块**:不走 `__main__`,整个文件从头跑到尾,
  `_sniff` 就有了。而生产是 `python -m lararium.gateway.wechat`,走到 580 行就进了事件循环。
  **这是"测试通过、生产必炸"的教科书形状,而且只有真机能发现。**

  **要做的两件:**

  1. **把 `_sniff` 挪到 `if __name__` 之前。** 一处,全仓库扫过只有它
     (`git ls-files 'src/*.py' 'bundles/*.py'` 逐个查 `__main__` 之后还有没有顶层定义)。
  2. **加一条架构门禁:任何模块里,`if __name__ == "__main__"` 之后不许再有顶层定义**
     (`def` / `async def` / `class` / 常量赋值)。这条是机械可判的,而且它挡的是
     **这一类**,不是这一个。失败信息要说清楚为什么——不然下一个人会以为是风格洁癖。

  **不要做的**:别顺手给适配器加"启动自检"之类的东西。门禁挡住这一类就够了,
  真机第一张图会告诉你还有没有别的(G6:先问它该不该在)。

  **验收口径**:门禁能咬住变异(把任意一个顶层定义挪到 `__main__` 之后,门禁要红);
  真机上发一张图,`data/media/` 里有文件、模型看得见。

- **M5-20 记错了删不掉**(第一次真实记账就撞上;M5-15 我设计错了)

  真机第一条正经记账:「那之前的那个作废。现在开始认真记账,给女朋友买麦当劳45.5……」
  三笔全记对了,但**"作废"那一半办砸了**:

  ```
  她调的   amend_expense(expense_id=1, note="测试作废:不计入")
  结果     #1 28.00 打车        标了作废,指向 #3
           #3 28.00 测试作废…   ← 新行,金额原样,仍然有效
  她说的   「测试那笔 28 打车,我已经标作废了」
  实际     28 块还在账上,有效合计 255.27 而不是她自己算的 227.27
  ```

  **不是她的错,是没有路**:`finance` 只有 record / query / list / amend 四个工具,
  **没有"删掉"这个动作**。她想作废,手里只有"改"——而 amend 必然留下一条替代行。
  失效形态是最坏的一种:**用户以为删了,账上还在,而她也真心以为自己办了。**

  **M5-15 的任务书里我写死了"不给 delete",理由是**「删除是唯一会彻底销毁用户可能想要
  的信息的操作」。**那个理由站不住**:作废机制本身不销毁任何东西(旧行留表、`voided_by`
  指着、`include_voided=True` 查得到)。我把「改错了金额」和「这笔根本不该存在」
  混成了一件事,只给了前者一条路。

  **要做的:**

  1. **`delete_expense(expense_id, reason=None)`** —— **就叫 delete,别叫 void**。
     底下是打状态位(标作废、不插替代行),正常查询查不出来、聚合不算它。
     「删除」在回收站、软删除、git 里本来就是"从眼前消失"而不是"覆盖扇区";
     而**上一次失败正是因为没有一个看起来像"删"的东西,她才去用 amend 顶**。
     名字越像人话,她越找得到。
  2. **docstring 第一句要写「删掉一笔记错的账」**,并且**写明可以撤回**
     ——用户说"删错了恢复一下"时,她得知道这事能办。
  3. **`amend_expense` 的语义收紧成"改"**:docstring 里点明"要整笔去掉用
     `delete_expense`,别用改备注的方式假装删掉"。
  4. **恢复的路**:`delete` 能撤回的话就得有地方撤。最小做法是 `amend_expense`
     对已作废行放行一次(现在它会拒),或者给 `delete` 一个反向参数。**你选一个,
     但要能在回报里说清为什么另一个不行**。

  **验收口径**:
  - 删一笔 → `list_recent` 默认看不见、`query_spending` 总额不算它、
    `include_voided=True` 还在;
  - **删完能撤回**,撤回后金额与类目和删之前逐字一致;
  - 删一个不存在的 id → 一句人话,不落任何行;
  - **真机复验**:说一句「刚才那笔删掉」,账上确实没了、她说的数和账上的数对得上。

- **M5-21 `web_search`(Tavily)**——第一条联网能力,风险最小的那半

  真机第一天就撞上了:「帮我查一下这周末上海天气」→「查不了,我这台机器上没有网络查询
  的口」。她**明说查不了、没有编**(这点是对的),但那条缺口是真的。

  **做成内置工具,不做成 bundle。** bundle 是"生活领域"(财务、健康),联网是**能力**,
  和 `search_history` / `current_time` / `look_at_image` 同一类。而且内置能直接
  `import` 到 `assembler.neutralize_fence`,不用像 finance 那样被 import-linter 逼着
  再抄一份中和实现(那是第三份了)。

  **搜回来的内容是不可信内容,四件事一件都不能少:**

  1. **调 `self._on_untrusted()`**(M5-18 建的那个闩)。搜回来的东西进了上下文,
     这一轮余下全程都算不可信,`propose` 强制降档。**这是这条工具能落地的前提**,
     M5-18 之前它根本不该做。
  2. **围栏 + 折行 + 中和 + 来源标注**,照 `_render_hit` 那一套。
     **`url` 也要中和**——它是搜索结果里带回来的、攻击者可控的文本,而且会出现在围栏
     附近;不中和的话它能伪装成框定语(P1-4 那个教训)。
  3. **条数和字数都要封顶**,理由和 `MAX_SEARCH_HITS=20` / `MAX_HIT_CHARS=200` 一样:
     不封顶一次调用就能撑爆 L0 并逼出一次压缩。
  4. **没配 key 时回人话,不崩**(E2):「没接搜索,这个我查不了」。
     key 走 `LARARIUM_TAVILY_KEY`,`.env.example` 里留空 + 一句说明。

  **代价**:新增一个工具 = 前缀重建一次,`prefix_log` 会记。

  **验收口径**:
  - 搜一次 → 结果在围栏里、带来源标注,`untrusted_seen` 落了一条;
  - **同一轮里搜完再 `propose(user_stated)` → 必须降档成 pending**(这条最要紧,
    是 M5-18 那套在新来源上的复验);
  - 没配 key → 一句人话,不抛异常;
  - 条数/字数超限 → 截断且说清楚少了多少。

- **M5-22 `web_fetch`(取一个网页,转成 markdown)**——风险大的那半,`web_search` 之后再做

  **SSRF 是这条的主体,不是附带。** `url` 是**模型可控的自由字符串**,不像 `image_id`
  能用正则锁死。要挡的:环回、私有网段、link-local(`169.254.169.254` 是云元数据)、
  `file://`。而且:

  - **检查解析后的 IP,不是主机名**——`evil.com` 解析到 `127.0.0.1` 是一行 DNS 记录的事;
  - **重定向之后要重新检查**;
  - **连到检查过的那个 IP**,别检查完再交给 httpx 重新解析一遍(DNS rebinding 的窗口);
  - 响应大小封顶、超时封顶。

  **一个要你判的选项**:Tavily 有 `/extract` 端点,由**它**去抓页面并抽正文——那样
  我们永远不发起模型可控的出站请求,**SSRF 面直接归零**,代价是全走 Tavily。
  自己抓则要把上面那套写对。**做哪个都行,但要在回报里说清楚选了哪个、放弃了什么。**

  正文抽取 + markdown 用现成库(`trafilatura` / `readability-lxml` + `markdownify` 之类),
  别自己写 HTML 解析。渲染和不可信处理与 M5-21 完全一致,**共用同一个出口**
  ——两个出口两套规则,这一节已经栽过两次(M4-4、M5-5)。

# M6 · 按真实使用重排(2026-09-12)

> **原来的 M6 是「短信 provider + 健康 bundle + 学习 bundle + 沙箱」,那是在没有使用数据时
> 拍的。** 六天 68 轮真机之后重排,旧的那份存进 `/tmp/m6_old.md` 的内容已并入下面「推后的」。
>
> **重排的依据,以及我在这上面犯的一个错**:我先去日志里数"她说自己做不了"的地方,
> 想按真实使用排序,并据此说"健康学习你一次没提过"。**用户当场指出那是错的**——
> 「我如果知道你不会做 A 我也不会提 A 啊」。**没人要求 ≠ 没有需求**:
> 日志里能观察到的需求,已经被用户对她能力的了解过滤过一遍了。
> 所以日志只能证明**穿过筛子的**那些(课程表:明知可能做不了还提了两次、被拒后又追问),
> 剩下的只能问。问下来的答案里**没有健康和学习**——用户知道自己会用什么。

## 执行顺序 = 编号顺序(2026-09-12 用户重排)

```
M6-1  语音            真机已确认:微信给转写,我们没读;转写要标注「语音 N 秒 · 转文字」   ✅ 已验收
M6-2  附件报 id        「报个 id,要看自己调」——图片不再自动塞进上下文                     ✅ 已验收
M6-3  收入 / 退款      600 元卡在话头里三天了                                             ✅ 已验收
M6-3a 退款并进收入      用户:很难说全退,那个指针指不准,派生的净额比没有更坏        ✅ 已验收
...
M6-6c PDF 收到就转     收到即逐页转文字、按 (id, 页) 缓存;read_pdf 给文字 + 图             ✅ 已验收
M6-6d 工具名加前缀     <bundle>__<工具>;分层注入不做了(DeepSeek 不支持,量过)           ✅ 已验收
M6-6e 按 id 搜内容     通用的、针对一个或多个文件 id 的内容搜索(读 6c 那份缓存)          ✅ 已验收
M6-7  待办             装作业和考试;放在学习之后                                        ✅ 已验收
M6-8  归拢自动跑        后台作业                                                  ✅ 已验收
M6-8a 风控卡批         不做(D22):用户不聊会被挡的话题;前提是没核实的转述             ✖ 不做
M6-9  隔一阵问一嘴      唯一会烦人的那条,排最后                                        ✅ 已验收
M6-10 隔了多久        消息带上算好的间隔 + 人读的时间格式;缓存安全的前提是渲染不随轮次变  ✅ 已验收
```

**排序依据:已经卡住的 > 已经踩到的 > 还没发生的。**
**编号改过一次**(原来是"编号不动、执行另排"),因为 M6 一条都还没交付,
`REVIEW.md` / `CHANGELOG.md` 里没有任何引用——**当初"改号会打断引用"那个理由是假的**,
用户指出之后改了。以后再有这种事:**先去 grep,别凭印象说代价。**

- **M6-1 语音听不懂**——**微信给了转写,我们没读**(真机已确认);而转写要标注

  用户:「你的 asr 也没加 语音识别 我发语音她好像还搞不定」。

  ### ★ 真机打完了(2026-09-13):**微信确实给文字,而且够用**

  用户发了四条语音,临时探针把原始报文里那一项打出来:

  ```
  PROBE voice keys=['bits_per_sample', 'encode_type', 'media', 'playtime',
                    'sample_rate', 'text']
        text='你帮我看一下那个GPT 5X那个那个订阅怎么样,然后帮我去subscribe一下,
              就是续一下那个订阅,还有那个anthropic的那个cloud'
        media_keys=['aes_key', 'encrypt_query_param', 'full_url']
  ```

  **`voice_item.text` 有内容,而我们的代码一个字都没读。** 所以这一条是小改动。
  顺带白拿三个字段:`playtime`(时长)、`sample_rate`、`bits_per_sample`。

  ### 和自己跑 ASR 的 A/B:**打平,而且微信还赢一处**

  同一段 17 秒音频(SILK 解码走 `pilk`,`#!SILK_V3` 前面微信多加一个字节):

  ```
  微信        …还有那个 anthropic 的那个 cloud
  SenseVoice  …还有那个 entropic 的那个 cloud
                        ↑ 微信对了
  ```

  两边**只差两处,而中英混那一处微信赢**;`Claude → cloud` 两边都错。
  SenseVoice 推理 0.6 秒(17 秒音频),**慢的只是首次加载**(下载 + torch 约 98 秒)。

  **所以不上 SenseVoice**:质量打平,而成本是 900 MB 模型 + 一套 onnx/torch +
  每次起进程。**没有任何理由自己跑一个。**

  而 `Claude → cloud` 这种**换模型也解决不了**——那是同音词,要靠上下文;
  **而上下文正好是她有的东西**(档案里写着「AI 工具主力用 Claude」、前几轮对话都在)。
  **所以正确的做法是把转写原样交给她、让她自己纠,而不是在转写那一层较劲。**

  ### ★ 必须标注这是语音转的——用户点的,而且理由是机制性的

  > 「你就用微信那个吧,**但是记得标注一下这条是语音转文字的**。因为有时候如果标了的话,
  > 模型会自己推测出来还是说错还是怎么样。」

  **这和项目里已有的几个标注是同一个形状:标签本身在给下游传信息。**
  ...
  所以信封正文长这样(`playtime` 白送的时长也标上——**17 秒的语音糊掉的概率比 3 秒高**,
  那本身就是判断依据):

  ```
  (语音 17 秒 · 转文字)你帮我看一下那个GPT 5X那个那个订阅怎么样……
  ```

  **两条不许搞错的:**

  1. **这不是"不可信内容",不要套围栏。** 说话的人**就是用户**,只是过了一道有损的信道
     ——`source` 仍然是 `user`,标注在正文那一行里。
     套围栏会让她把用户自己的话当成外部内容,那是另一种错(比不标还糟)。
  2. **不许把标注写成"以下内容可能有误,请谨慎"这种废话。** 要的是**事实**
     (这是语音转的、多长),**判断交给她**——她手里有档案和上下文,
     比任何免责声明都强。

  ### 要做的

  1. **取文字那一支加上语音**:`type == VOICE` 且 `voice_item.text` 非空 → 那就是这条消息
     的文字,**按 `item_list` 原序**拼进 `content`,别把语音那句排到最后。
  2. **音频一律存下来,哪怕 `text` 有内容。** 见下面那节(用户对转写质量的怀疑)。
  3. **没有 `text` 的语音**:占位文本要说实话——**不是「(语音 · media/xxx)」就完了**,
     要让她知道**这条语音没有文字、她听不了**,否则她会对着一行占位符编内容(M5-19 之后那次)。
  4. **`_media_refs` 那两处 `continue` 必须打日志**(`logger.warning`,带上
     `item.get("type")` 和有哪些键)。**这一条比上面三条都重要**:
     没有它,下一次协议变形又是一次"六天之后用户来问我为什么不行,而日志里什么都没有"
     ——**而这次就是这么查出来的**(临时探针,而它本该是常设的)。
     **注意 `ilink.py` 现在压根没有 `logging`**,要先加(复核方打探针时就是漏了这个,
     把适配器搞挂了 6 分钟,见下)。
  5. **语音的 `aes_key` 检查补上**(官方对语音是 `|| !voice?.media?.aes_key`,我们没有)。

  ### 复核方在真机上把适配器搞挂了 6 分钟,记下来

  打探针时往 `_media_refs` 里加了 `logger.warning(...)`,**而 `ilink.py` 里没有 `logger`**
  ——每次收信抛 `NameError`,退避 3→6→12→24→48→96 秒,**用户那条语音卡在外面进不来**。

  **两个教训:**

  - **照另一个文件的习惯写,不先看这个文件有没有那个名字。** 加日志之前先 `grep logger`,
    两秒钟的事。
  - **而它没造成损失,是 M5-12 那轮的设计救的**:失败不推进游标、消息留着等。
    修好之后那条语音自己补了进来。**这就是当初为什么要那么做。**

  ### 验收口径

  - 造一条 `type=3` + `voice_item.text="麦当劳 45.5"` + `playtime=3` 的报文
    → `content` 里**有那句话、而且带「语音 3 秒 · 转文字」这样的标注**,
    **而且音频也落了盘**;
  - **标注的措辞钉住**:一条断言写明它出现"语音"、时长、"转文字"三样
    ——**这句话是用户点名要的机制,措辞漂了就没了**;
  - **不许套围栏**:`source` 仍是 `user`,输出里**不出现 `<<<` / `>>>`**
    (拿一条断言钉死——套围栏会让她把用户自己的话当外部内容);
  - 文本 + 语音混在一条消息里 → 两段都在 `content` 里,**顺序和 `item_list` 一致**;
  - 造一条 `type=3` 有 `media` 无 `text` → 音频落盘,占位文本**说得出"没有文字、听不了"**;
  - 造一条 `type=3` 既无 `media` 也无 `text` → **打一条 warning**,不静默;
  - 造一条 `type=3` 有 `text`、`media` 缺 `aes_key` → 文字照样进,音频那半**打日志**不静默;
  - **`ilink.py` 有 `logger`**(现在没有,要先加)——拿一条测试或 grep 钉住那两处
    `continue` 都打日志;
  - 现有图片那条路**逐字不变**(拿现有测试当金样)。

  ### 转写质量:量过一轮了,但**中英混只测到一处**

  用户原来怀疑「微信的 asr 质量不太好」。实测四条(2026-09-13):

  ```
  纯中文短句   「测试一下测试一下测试一下能收到吗?能收到?」   ✅
               「收不到就算了,收到了吗?收到开始。」           ✅
               「发了没发了没」                                ✅
  中英混长句   「…GPT 5X…subscribe…anthropic…cloud」
               ← GPT 5X / subscribe / anthropic 都对,**Claude 听成 cloud**
  ```

  **够用。** 而 `Claude → cloud` 靠标注 + 上下文解决(见上),不靠换模型。

  **音频仍然一律存**——不是因为怀疑没解除,是因为**几 KB 的代价换"以后想重转还有原件"**,
  而这个"以后"现在有具体形状了(比如某天发现长语音糊得厉害,想拿 SenseVoice 重跑一遍)。

  ### 明确不做

  - **不上本地 ASR**(SenseVoice / Whisper 都不上)——A/B 打平而微信赢一处,
    成本是 900 MB 模型 + 一套运行时;
  - **不在转写层较劲**:同音词错(`Claude` / `cloud`)换模型也解决不了,
    **标注 + 上下文才是解法**;
  - **不加"以下可能有误"这类免责声明**——给事实(是语音转的、多长),判断交给她。

- **M6-2 附件改成「报个 id,要看自己调」**——**这一条把 M5-5 的一个决定翻过来了**

  > 「你给一个工具吧,就是 loadimage,**不要直接塞进去,你让他调工具来看**。
  > 然后别的话就别送,**你只发一句,发了什么什么东西,文件名还有唯一那个 id**。
  > 然后再给一个,比如说每一个需要的 bundle 里面,**给一个 copy 的工具**,这样可不可以?」

  ### 现在是什么样

  ```
  所有附件  → 落盘 data/media/<sha256>.<后缀>,信封正文放一行 (图片 · media/ab12…)
  图片      到达轮:loop.py 直接 load_images() 把字节塞进上下文,**不经过任何工具**
            之后轮:只剩那行文字,要再看调 look_at_image(id)
  别的类型  语音 / PDF / 视频 / 文件 —— **一个工具都没有,压根摸不到**
  ```

  ### 为什么翻:**图片是特例,而那正是别的类型一条路都没有的原因**
  ...
  **「报个 id,要看自己调」是通用形状**——PDF、语音、视频、以后任何东西都套得进来,
  ...
  - **不看就不花钱**。用户一次发 10 张照片,现在硬塞 4 张(封顶)进去,
    不管有没有用;改完是零成本,直到模型自己决定要看;
  - **张数封顶不再是静默截断**。M5-5 现在得说"少了几张";改完**十张全报出来**,
    模型自己挑要看哪张;
  - **注入面更小**:恶意图片要进上下文,得模型自己决定去取。
    M5-5 立的「只在到达轮进 + 张数封顶」是机制,**这一条比它更严**,不是放松。

  ### 但它有一个真风险,而且现在没有数据

  **她可能该看的时候不看。** 「这个多少钱」+ 一张收银条,如果她没意识到要先调工具,
  就会对着一行文字瞎答——而 M5-5 已经抓到过这个形状的近亲:
  **读不清的时候她会编**(图上一个中文都没有,她回「看到一些中文文字」)。

  **真机上 68 轮里 `look_at_image` 被调过 0 次**——因为图一直是预先塞好的,
  从来没有"需要去调"的场合。所以**我们对"她会不会调"一无所知**。

  所以这一条的验收**必须有真机那一半**,而且判据是**行为不是不报错**:
  发一张收银条 + 一句「这个多少钱」→ 看她**有没有调 `look_at_image`**,
  以及**没调的时候是不是老实说"我还没看这张图"**。
  **后者比前者重要**:调不调可以靠 docstring 调整,而"没看却瞎答"是编造。

  ### 那一行"报什么"要写清楚

  现在是 `(图片 · media/ab12cd…)`。改完要**够模型据此决定**:
  ...
  - **id 要完整给**(现在那行是截断的 `…`),否则模型得猜;
  - **文件名要给**(微信带的那个原名),`id` 是把手但名字才是人看的;
  - **每一种都要带一句"能拿它干什么"**——这是 M5-5 那条"每一种降级都要留下一句话"
    的延伸:一行 `(视频 · media/xxx)` 后面什么都没有,等于让模型自己猜有没有路。

  ### `copy` 工具:**形状定一次,谁有理由谁要**
  ...
  **不要现在就给每个 bundle 都加一个**(M5-11 的形状:先猜再用)。
  形状统一成 `file_*(..., media_id, name)`:

  - `media_id` 走**和 `look_at_image` 同一条正则**(`_IMAGE_ID_RE`,纯十六进制),
    **共用同一个常量**,两处各写一份的那天就开始漂;
  - `name` 过那个 bundle 自己的名字白名单;重名拒绝;
  - **复制不是移动**,media 池那份留着(不可协商第 3 条);
  - **bundle 怎么读到 media 池**:它 `build()` 时已经拿到 `data_dir`,
    而 media 池就是 `data_dir/media`——**不用新接线、不碰 import 边界**。

  ### 明确不做

  - **不给通用的"读任意文件"工具**。每种类型该怎么读是这一类自己的事
    (图 → 视觉;PDF → 归档后逐页转;语音 → 微信的转写)。
    一个 `read_file(id)` 会把这些混成一个"看情况"的黑盒——**按 G7 拆成
    `read_image` / `read_pdf`**,签名各自诚实,而"没有 `read_docx`"本身就在告诉模型读不了
    (`look_at_image` 一个种类判断都没有,一段语音和一份 PDF 都被贴上 `image/jpeg` 送了出去)。
  - **不动 `MAX_IMAGES_PER_TURN`**:它现在管的是"一轮里最多能看几张",照样有效。
  - **不给视频和普通文件开路**——没有任何证据要它们,现在开就是猜。

  ### 验收口径

  - **到达轮不再自动塞图**:发一张图那一轮,**上下文里没有图片部件**,
    只有那行报告(拿现有的 M5-5 测试改口径,**而它们改了要在回报里逐条说明**);
  - **报的那行**:id 完整、文件名在、每种类型都带一句"能拿它干什么";
  - 发 10 张图 → **十行全报**,不再是"少了几张";
  - `look_at_image(id)` 仍然只认图片(和 `load_images` 共用判据,M5-5 的那条不许动);
  - **`MAX_IMAGES_PER_TURN` 仍然生效**:一轮里调第 5 次 → 拒绝并说清;
  - **★ 真机**:发收银条 + 「这个多少钱」→ 记下她调没调工具;
    **没调的时候必须老实说"我还没看这张图",不许瞎答**;
  - 现有那 16 个注入图样本(M5-5)重跑,**注入成功仍然 0 次**。

- **M6-3 账本只有支出,没有收入**——用户问出来的,而账上**已经卡着 600 元**

  真机证据(seq 289,她自己的话):

  > 「**但那 600 退款,我记不进去。** 我的账本只有"花出去"这一个口子,没有收入、没有退款。
  > 刚试了记 -600,系统直接挡回来:"要一个大于 0 的数字"。所以要么等你给我补个能记退款
  > 的口子,要么我按冲抵处理——去动那两笔 GPT 的账。**但那样底稿就不是原始记录了,
  > 我不建议,先挂着。**」

  **她的处置全对**:试了、被挡了(那道校验是对的)、想出两条路、**选了不破坏原始记录
  的那条**、开话头挂着。那 600 到现在还挂在「ChatGPT 退款 600」这个话头里。
  **这是穿过筛子的需求**——她明知自己做不了还是把两条路摆出来了。

  ### 退款和收入不是一件事,要分清再动手

  ```
  退款   冲抵某一笔支出。「这个月花了多少」应该**减掉**它。
  收入   生活费、奖学金、兼职、红包。它**根本不该出现在"花了多少"里**。
  ```

  混成一个会出现这种事:记一笔 3000 生活费,然后「这个月花了多少」变成 -3000。

  ### 我倾向的形状(**但你要自己论证一遍,不许照抄**)

  **新一张表,不是往 `expenses` 加符号位。** 理由:

  - `query_spending` 的测试金样**一个字不用动**(这个项目刚为它付过两次迁移的代价);
  - 加符号位的话,**每一条查询、两条聚合 SQL 都要多一个条件**,而 M5-26 刚把
    `voided_by` 那个多状态位的债还掉,不该立刻欠一个新的;
  - 支出和收入回答的是不同的问题,分表之后各自的查询都简单。

  表里要有 `kind`(`income` / `refund`)和**可选的 `of_expense_id`**
  ——那 600 退的正是那两笔 GPT(500 + 134.16),能指回去才说得清。
  状态位照 M5-20:`deleted_at` + 能撤回,**别再发明第二套**。

  ### 工具

  `record_income` / `list_income`。**先只做这两个**,`amend` / `delete` 等真撞上再加
  (finance 那两个是被真机逼出来的,不是设计出来的)。

  ### 月度复盘要跟着改,而且这是最容易出错的地方

  - 「这个月花了多少」= 支出 **减去 refund**,**不减 income**;
  - 「这个月进了多少」= income;
  - **两个数都要能说出口径**——她在 [262] 已经干过一次对的事:
    「其实是**五天**。账本 9 月 6 号才开张,9/1–9/5 是空的」。同样的实话精神,
    有 refund 的时候要说得出"支出 X,其中 Y 是退回来的"。

  ### 验收口径

  - 记一笔 refund 指向某笔支出 → 「这个月花了多少」减掉了它,**而 `list_recent`
    里那笔原始支出一字不变**(底稿不许被动);
  - 记一笔 income → 「这个月花了多少」**一个字不变**;
  - 指向不存在的 `of_expense_id` → 人话,不落行;
  - 指向已删的支出 → 人话(M5-31 刚为 amend 答过同一个问题,照它);
  - 老库开库 → 新表建出来,现有数据一行不动,`list_recent` / `query_spending` 输出**逐字节一致**;
  - 没有 refund / income 时,**finance 全部现有测试逐字不改**。

  ### 顺序

  见 M6 开头那张执行顺序表:这一条排第 ②,做菜排第 ③。
  **600 元挂在话头里三天了,而做菜是还没发生的需求——已经卡住的先修。**
- **M6-3a 退款并进收入——把 M6-3 的一半拆掉**(2026-09-13 用户定的,验收完 M6-3 当天)

  > 「我觉得没必要退款啊,**很难说全退**,所以我觉得没啥用。退款就弄成收入就行,
  > 支持收入我觉得就好了。」

  **他的理由比我写在 M6-3 里的任何一条都硬**:`of_expense_id`「冲抵某一笔支出」
  这个建模**假设退款是全额的**。真实的退款经常是部分退、几笔合并退、或者退到别处
  (代金券、积分),于是那个指针大多数时候指不准;而「抵掉退款后实际花掉 Z」这个数
  **整个建在那个指针上**——前提不成立,派生出来的数就不可信,**而它比没有更坏**:
  一个读起来很确定的数字,底下是个猜的对应关系。

  这一条同时印证了 G7 的反方向:我把「退款」和「收入」塞进一张表、用 `kind` 分开,
  论证写的是「它们在**存**这件事上完全一样,差别只在一个读者」。**对了一半**——
  存是一样的,而我保留的那个差别(减不减「花了多少」)本身就不该存在。
  东西真的一样的时候,连那个 `kind` 都是多的。

  ### 拆掉什么

  - `kind` 列、`of_expense_id` 列,以及 `of_expense_id` 那一整条校验路径
    (目标在不在 / 是不是已删 / 收入不许指);
  - `query_spending` 里退款那一摊:`_income_totals` 调用、「同期收到退款」那一行、
    「净收回」分支、「没有支出但收到过退款」分支。**它要回到 M6-3 之前逐字节一样。**
  - `delete_expense` 里验收时补的那句悬空退款提示。**没有指针就没有悬空**——
    它报告的那个事实不存在了,留着就是 G6 说的"修一个不该存在的东西"。
    (G8 本身照旧有效,M5-8 还是它的实例;**M6-3 那个实例蒸发了**,这件事要记下来。)

  ### 留下什么

  `income` 表、`record_income` / `list_income`、「收入不算在「花了多少」里」这句口径、
  `deleted_at` 两列与那条机械保证、金额上界、备注共用同一个消毒器、截断说出口。
  `_is_bindable_id` **留着**——`amend_expense` / `delete_expense` 那个
  M5-15/M5-20 起就在的 `OverflowError` 洞靠它,只是 `record_income` 那个调用点没了。

  ### 迁移:真机那张表已经带着两个要拆的列

  真机上 `income` 表**已经建出来了、0 行**(M6-3 于 2026-09-13 推上去,当天验过
  三份视图逐字节一致)。`CREATE TABLE IF NOT EXISTS` 对已存在的表是空操作,
  所以**必须有一步真的退休手续**,照 `_retire_the_voided_column` 同一个形状:
  `PRAGMA table_info` 探测 → 一个事务里 `ALTER TABLE income DROP COLUMN` 两次 → 幂等。
  **有行的情况也要对**:一条 refund 行直接变成一条收入行,而那正是新口径要的意思,
  不是凑合——所以不需要重建表、不需要搬数据。

- **M6-4 预算 / 超支提醒**——用户点的,**做成"记账当场说",不做成定时推送**

  用户选的是「想要预算 / 超支提醒」。**先做反应式的那一半**:记一笔的时候,
  如果这一笔把某个类目或总额推过了线,**就在那一笔的回话里说**。

  **为什么不做成定时推送**:① 不需要调度器,M6-8 之前就能上;
  ② **花钱那一刻说比第二天早上说有用**——第二天说的时候钱已经花了。
  定时那一半留给 M6-8。

  - 预算存哪:finance 自己的表(`budgets`:类目 or 总额、月额度)。
  - 怎么设:`set_budget` / `list_budgets`;**不设就没有提醒**,不要发明默认额度。
  - 提醒的话**拼在 `record_expense` 的回话里**,不新开一条消息——
    新开一条就要走出件箱,而那是主动推送的形状,和这一条没关系。
  - **口径要小心**:账本 9 月 6 号才开张,[262] 她自己说过"8 月没数据,环比做不了"。
    所以提醒说的是**本月已花 / 额度**,不许说"比上月多",那是编的。

  **验收**:没设预算 → 记账回话一个字不变(拿现有测试当金样);设了没超 → 也不变;
  超了 → 多一句,且**说得出已花多少、额度多少**;跨月边界 → 按记账时刻所在的月算。

- **M6-5 做菜 bundle**——**用户重新设计过:它是文档,不是表**

  ### 用户的原话,以及为什么他对

  > 「做菜确实要标准啊,就是标准之后尝试,觉得可以就留着作为经验,然后不可以就提改进。
  > **其实就是读文件**,这个没有太多格式化的东西。给一个 read 和 write 指定文件名,
  > 还有 list 就行了,最多弄一个 category 之类的。因为这个没法结构化,有可能得改做法
  > 什么的,那肯定就 edit 啊。**但是不要真的给文件系统,给几个工具,不给读目录这种。**」

  **我原来那版(做法表 + 尝试表)是错的。** 我查了 schema.org/Recipe,砍掉了它九成的字段,
  然后把剩下的那点做成了两张表——**而用户指出连那两张表都不该有**:

  - 一份做法**没有稳定的形状**:步骤数不定、"焯水的时候加勺盐"是注解不是步骤、
    "上次咸了,酱油减半"该写在它影响的那一步旁边。**塞进表里就得给每一条注解找个格子。**
  - 「尝试」不是一张独立的日志,而是**做法本身在长**。觉得行就并进去当经验,
    不行就在旁边写改进。我把它拆成两张表,等于强迫用户把一件事记在两个地方。

  ### 存:扁平的 markdown 文件,**路径和分类都不出现在接口上**

  `data/recipes/<菜名>.md`,一份做法一个文件,**一层,没有分类**。

  模型手里只有**一个菜名**。工具签名里**没有路径、也没有 category**
  ——底下怎么落盘它不知道,所以以后想改布局、改成别处存,**工具签名一个字不用动**。

  **只有一个内部函数知道目录在哪**,它是唯一把菜名变成落盘位置的地方。
  这条和 M5-4 的教训同源:`Attachment` 上**根本没有可写的 path 字段**,
  「能自报路径的附件就是路径穿越的入口」。

  ### 这一节复核方判错了三次,依次记下来

  1. **"存 SQLite,让路径穿越成为一类不存在的 bug"**,类比 M5-22 的「SSRF 面归零」。
     **类比不成立**:M5-22 那次"不出站"是零成本消掉一整个攻击面;这次"不落盘"要付的
     代价是**用户没法在自己电脑上打开自己的菜谱、没法 cat、没法复制文件夹备份**,
     而我把这个成本当成了零。
  2. 改成文件之后写"只准一个函数把菜名变成路径 + 白名单 + 落点兜底"。
     用户:「为什么要写 path 呢?**你用文件,但是不暴露目录**……为什么要二极管?」
     ——**我把选项压成了"文件(于是有路径要校验)"和"SQLite(于是没路径)"两个,
     而中间那个一直在那儿:用文件,而路径不是接口的一部分。**
  3. 于是我改成两层命名空间(`list_categories` + `list_recipes(category)`)。
     用户:「**category 都可以不要,因为做菜其实不好分类。番茄炒鸡蛋属于什么类?
     煮面水什么的,这种家常菜都很难说怎么分类。**」
     ——对。而且分类去掉之后,"同一个菜名出现在两个类里"那个歧义分支也一起没了:
     **扁平化让名字天然唯一。**

  **三次都是同一个毛病**:我先想"怎么把它做对",而不是先问"用户会怎么用它"。
  菜谱是几十个文本文件,不是一个需要 schema 的数据集。

  ### 名字要校验,但理由不是"防路径"

  路径已经不是参数了,校验是为了**命名空间的完整性**:一个叫 `..` 或带 `/` 的菜名
  会跑到别人家里。归一化(折内部空白、去首尾)在校验**之前**做,然后两道:

  1. **白名单**:拒 `/`、`\`、`..`、控制字符、前导 `.`、空名;长度封顶。
  2. **落点兜底**:算完 `resolve()` + `is_relative_to(recipes_root)`。
     白名单是"我想到的那些",这道是"不管怎么绕,出了这个目录就不行"
     ——角色等同 M5-22 五步里的 `getpeername()` 复查。

  ### 工具:五个

  ```
  list_recipes(page=1)                存了哪些菜(分页 + 总数)
  read_recipe(name)                   读一份
  write_recipe(name, content)         写 / 整篇覆盖
  append_to_recipe(name, text)        追加一段(做完一次菜写一句)
  replace_in_recipe(name, old, new)   定点改一处
  search_recipes(query, page=1)       搜菜名 + 搜内容
  rename_recipe(old, new)             改名(打错了得能救)
  ```

  ### ★ 打错菜名得能救(和 M6-6 同一个缺口,用户点的)

  `write_recipe` 是隐式创建的,所以「番茄炒鸡蛋」打成「番茄炒鸡旦」就是一道新菜,
  而且没法改。所以:

  - **`rename_recipe(old, new)`**,`new` 已存在时**拒绝**——合并两份做法是静默的破坏;
  - **`write_recipe` 写到一道不存在的菜时,回话要明说"之前没有这道,给你新建了"**
    ——打错的那次照样会创建,**说一声才挡得住**;显式 `create` 挡不住。
  - **`delete_recipe(name, reason, undo=False)`**——照 M5-20 那套:
    **移到 `data/recipes/.trash/` 不是 unlink**、`undo=True` 搬回来且逐字节一致、
    `reason` 必填、`list_recipes(include_deleted=True)` 才列出来并标「已删」。
    名字白名单拒前导 `.`,所以模型寻址不到 `.trash`,但 `list_recipes` 要跳过它。

  ### `search_recipes`:**菜名和内容都搜**

  复核方第一版没给,理由是"几十份 `list_recipes` 一页就看完了"——**那是只想到了
  "菜多到列不完"这一种用途**。真正的用处是**按内容找**:「我之前哪道菜写了要少放酱油」
  ——列名字一点用没有,而逐份 `read_recipe` 是把整个 bundle 倒进上下文。

  - **两样都搜**:菜名命中和内容命中都算,**在结果里标出来是哪一种**
    (名字命中通常更强,模型该知道差别);
  - 返回**菜名 + 命中处附近的一小段**,不返回整份——否则它就是一个伪装成搜索的
    "全部读出来";片段字数封顶,截断要说清楚;
  - 条数封顶 + 分页,照 `_paged_search` 那套,别另写一份;
  - 一个字都没命中 → 人话,并提一句可以 `list_recipes` 看全部。

  **不建索引,直接扫文件。** 这不是偷懒,是**用文件这个决定推出来的**:
  用户随时可能用编辑器直接改那些 `.md`(那正是选文件而不是 SQLite 的全部理由),
  **而任何索引都会在那一刻变味,且变味之后没有任何报错**——搜不到的东西
  和"确实没写过"长得一模一样。几十份几 KB 的文本,扫一遍的代价小于维护一致性的代价。
  真到几百份再说,**而到那时候正确的做法是加缓存 + 失效判据,不是先建索引**。

  **`replace` 我第一版没给,理由是"模型整篇重打可能悄悄漏一步"。用户指出那是臆想:**

  > 「coding agent 都能 edit,为什么你臆想中它连 edit 个菜谱都做不到?」

  **他对,而且我漏掉的正是让它安全的那个机制。** coding agent 的 edit 安全不是因为模型
  聪明,是因为**匹配不上就拒绝执行**。照抄那套纪律:

  - `old` 出现**恰好一次**才改;
  - **零次** → 拒绝,说"没找到这段,可能已经改过了或者你记错了原文";
  - **多次** → 拒绝,说出现了几次、要求给更长的上下文。**绝不允许"改第一处"**;
  - 改完把**改动前后那一小段**回给模型,让它自己核对,不是只回"改好了"。

  `append` 照旧给——最常发生的动作,而且不可能弄丢东西。

  **`write` 整篇覆盖不留上一版**:有了 `replace` 之后整篇覆盖会少很多,
  而且现在是文件、用户自己 `cp` 一份就是备份;M5-26 刚删掉一整套"留痕但没人读"的机制,
  按 G6 问下来没有读者。docstring 写明"同名覆盖,旧的那版不留"。

  ### ★ 后面有第二个消费者:学习 bundle(M6-6)

  M6-6 是**同一套文档库,加一层课程**(`(课程, 笔记名)` 两层)。
  而 `.importlinter` 禁止 bundle 互相依赖,所以将来只有两条路:
  在 `lararium` 侧放一份共用的,或者抄一份。**抄一份不行**——名字校验、落点兜底、
  `replace` 的匹配纪律、搜索的"不建索引",每一条都是安全或正确性攸关的,
  抄一份就是两处维护同一个事实。

  **但这一轮不要为此先建抽象。** 先把做菜做对、做具体;抽象在 M6-6 动手时
  按真实的第二个用例提出来。**先抽象再用是猜,用两次再抽象是归纳**
  ——而这个项目为"先猜"付过账(M5-11)。

  这一条写在这里只为一件事:**M6-6 那一轮不该是"重新发明",而该是"提取"。**
  所以这一轮**别把逻辑埋进工具函数体里**,该拆成能被搬走的小函数(名字校验、
  落点计算、`replace` 的匹配、扫文件搜索),这不是为了抽象,是为了**将来能搬**。

  ### ★ 换行这次是主体问题,不是附带

  我们的渲染器**折行**(攻击者能凭换行伪造列表项,M4-4、M5-21 各栽一次)。
  而做法**整篇就是换行撑起来的**——折完「1. 水开下面 / 2. 打蛋 / 3. 撒紫菜」变成一坨,
  **这个 bundle 的全部内容都不可读**。这和 M5-27 那个 `[...]` 片段界限被折掉同形,
  但严重程度高一个量级。

  **必须解决,而且要在回报里论证。** 三条路:

  - 渲染时把换行换成行内编号或分隔符(`① … ② …`)——结构保住、注入面没扩;
  - 给这个 bundle 单独一条允许换行的渲染路径——**多一个出口,而"两个出口两套规则"
    这一节栽过两次**,真选它要说清楚凭什么这次不会漂;
  - 别的你想到的。
  ...
  **不许"就折掉算了"。**
  ...
  做法可能是用户让她 `web_fetch` 一个菜谱存下来的。那一轮是脏的,而 `write_recipe`
  写的是 bundle 自己的库、不经门控——**这不是漏洞**(门控管的是长期档案),
  但 docstring 要写明白:存进来的做法**是用户让存的,不是"经过核实的"**。
  **别在这一步发明新守卫**(M5-11)。
  ...
  - `append_to_recipe` 三次 → 三段都在,顺序不变,**原有内容一字没动**;
  - `list_recipes` 页码 0/负数/超大都钳住,总数出现在输出里;
  - **非法名字那一组**(见上)当 `name` 传 → 全部拒绝且回人话;
    注释掉白名单那一道,落点兜底仍然挡住,反之亦然;
  - **工具签名里没有任何路径参数,也没有 `category`**——加一条机械检查:除了菜名与
    翻页/搜索那几个参数,不许出现 path/dir/file/category 这类参数(它是这一条的设计
    约束,不是风格偏好。**2026-09-13 修:下面原本还有三条是按被否掉的分类设计写的
    ——`category` 能筛、`list_categories`、「两个类里同名 → 拒绝」。用户已经定了扁平化
    (「番茄炒鸡蛋属于什么类?」),扁平化之后名字天然唯一,那三条整个不存在**);
  - `replace_in_recipe`:`old` 出现一次 → 改;**零次 → 拒绝**;**多次 → 拒绝且说出现几次**;
    改完回话里带得出改动前后那一小段;**绝不许"改第一处"**(拿一份 `old` 出现三次的
    做法钉死它);
  - 不存在的菜名、空名字、超长名字 → 各自一句人话,不抛;
  - **前缀影响(A1)**:新 bundle = 目录行 + **8 个工具** schema(`list_recipes` /
    `read_recipe` / `write_recipe` / `append_to_recipe` / `replace_in_recipe` /
    `search_recipes` / `rename_recipe` / `delete_recipe`),前缀重建一次。

- **M6-6 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**

  > **2026-09-13 拆成两个任务派**(M6-5 验收完当天,核过前置条件之后):
  >
  > - **M6-6a 笔记那半**:`list_courses` / `read_note` / `append_to_note` /
  >   `replace_in_note` / `search_notes` / `rename_course` / `delete_course`。
  >   **零新依赖**,而且它的正题是**从 M6-5 的 `recipes/store.py` 提取共用层**
  >   ——名字校验、落点计算、`replace` 的匹配纪律、扫文件搜索,四样都是安全或
  >   正确性攸关的,抄一份就是两处维护同一个事实。
  > - **M6-6b 课件那半**:`add_file` / `list_materials` + Steward 侧 `read_pdf(id, page)`
  >   与 `(media_id, page)` 缓存。**这半是真基建**:核过了,仓库里**一个 PDF 库都没有**,
  >   所以它要一次依赖决定 + 光栅化 + 缓存 + 一条调模型的路。
  >
  > 切线就是这一节自己画的那条架构线(bundle 侧只存归属 / Steward 侧持有转换),
  > 不是按行数切的。
  >
  > **同日又切一刀:6b 拆成 6b(看得见)与 6c(搜得到)。** 派之前核出这一节有**两处跨边界
  > 没对上**,都要改设计、都是用户的决定:
  >
  > 1. **谁触发转换**:下面写「`add_file` 时逐页转」,而 `add_file` 是 bundle 工具,
  >    **bundle 不含 LLM**,调不了转换(转换与缓存只能在 Steward 侧,这一节自己说的)。
  > 2. **谁搜转出来的文字**:下面写「文字缓存决定了 `search_notes` 能不能搜到课件」,
  >    而缓存在 Steward 侧、`search_notes` 在学习 bundle 里——**bundle 按 §5 数据产权
  >    读不到 Steward 的存储**。
  >
  > **6b 只做完全不碰这两处的那半**:`add_file` / `list_materials`(归属表)+ Steward 侧
  > `read_pdf(id, page)` **给那一页的图**(本地渲染)+ 说共几页。它单独就有用:
  > 课件发过来、归到课下、「看第 3 页」就能看。**不转文字、不建缓存、不搜课件**,
  > 于是也没有"搜不全 = 静默错"的问题——**没有任何工具声称能搜课件**。
  >
  > **6c 用户拍板了(同日),两处都推翻了复核方的倾向:**
  >
  > > 「**A 啊,当然是直接转然后按照 id 缓存,因为 ai 随时有可能读啊……你为什么会觉得
  > > 只有 bundle 才会读?** 还有为什么要搜课件,**不搜课件**。」
  >
  > 1. **收到 PDF 就转,不等 `add_file`。** 复核方倾向"归到课下才转",**错在把学习 bundle
  >    当成了唯一的读者**。文件在主控的池子里,`read_pdf` 是主控的内置工具,**任何一轮对话
  >    都能读任何一份收到的 PDF**——随手发个合同问「第 2 页写的啥」,它从来不会被归到
  >    哪门课下。按"归档才转",这份合同就永远只有图。**触发点在收件那一侧,和 bundle 无关**,
  >    于是"bundle 不含 LLM 触发不了转换"那个矛盾**整个不存在了**。键是 `(media_id, page)`,
  >    id 是内容哈希,同一份发两次只转一次。
  > 2. **不搜课件。** 下面「文字缓存决定了 `search_notes` 能不能搜到课件」「50 页 PDF 你不
  >    可能每页都看 → 搜不到就等于没有」**是复核方自己的理由,不是用户的需求**——用户要的是
  >    「模型提炼出来的文字和弄出来的图片都可以复用哈希缓存」,是**缓存**不是搜索。
  >    **所以文字缓存唯一的读者是 `read_pdf` 自己**:给那一页的文字 + 图(「读单页的图片和
  >    文字,都弄出来」,用户定的),没转完就先给图并说一声、不当场调模型。
  >    **"bundle 读不到主控的缓存"那个矛盾也跟着消失**——没有 bundle 需要读它。
  >
  > **同日用户又补了一句,把搜索用对的形状加回来了:**
  >
  > > 「或者这样,你加一个**通用的、针对一个或者多个文件 id 的 content search**。」
  >
  > 和上面「不搜课件」不矛盾:**不做的是"学习 bundle 搜课件"那种绑在课程上、要跨边界读
  > 主控缓存的形状;做的是主控的内置工具、按 id 搜**。缓存在主控、id 是唯一把手,于是
  > **谁都不用跨边界读别人的东西**,而且它不绑课程——课件、合同、随手发的讲义都能搜。
  > 动作链:`list_materials(course)` 或信封里那行附件报告拿到 id → 按 id 搜 → 搜到第几页
  > 再 `read_pdf(id, page)`。复核方给的默认(6c 任务书照这个写):
  >
  > - **必须给 id**,一个或多个,**没有"搜全部"**(用户原话就是"针对一个或多个文件 id");
  > - 返回 **id + 显示得出的文件名 + 第几页 + 命中处一小段**,条数封顶 + 分页;
  > - **没转完的页必须说出口**:「这份第 12–30 页还没转完,没搜到不代表没有」——
  >   这是这个工具**唯一会静默错的地方**(搜不到和没写过长得一样),而"收到就转"让它少见,
  >   **少见不等于不说**;
  > - **没有文字的 id**(图片)→「这是图片,没有可搜的文字,用 read_image 看」,
  >   不跳过、不装作搜过;
  > - 名字**不带类型**(它搜的是缓存里的文字,不管来源是什么;按 G7,**处理真的一样时入口
  >   可以统一**——和 `read_pdf` / `read_image` 必须拆开是同一条判据的两面)。
  >
  > **下面这一节里凡是讲"search_notes 搜课件""学习 bundle 搜到课件"的句子,6c 一律不照做。**
  >
  > **另外两处过期,已核**:① 下面「`look_at_image` 怎么办:改名成 `read_image`」
  > 那一节**M6-2 已经做完了**(`read_image(id)`,没有 `page` 参数),这一轮不用再做;
  > ② `MAX_IMAGES_PER_TURN = 4` 已经在 `vision.py` 里,不用新建。

  > 这一节被增量改了七版才定稿(用户否掉复核方六次)。**下面是定稿**;
  > 被否掉的那些和理由集中在最后一节「复核方在这一节判错的四次」,
  > 留着是因为**每一次都是同一个毛病的不同长相**。
  ...
  真机证据——这是六天里**唯一穿过筛子**的需求(明知她可能做不了还提了两次、
  被拒之后又追问一次):

  ```
  [120] 她: 课程表这个功能我目前**还弄不了**——我手里就一个记账的工具,加一个"记话头"的
  [123] 她: 日程和待办是有的,我刚才说"弄不了"是说岔了。不过我现在的日程/待办,
            存的是**一件事、一句话**,它不认结构化的一张表
  ```

  ### 形状

  ```
  笔记  一门课一个笔记本  data/courses/<课程>/notes.md   可读、可追加、可定点改
  课件  老师给的 PDF 等   data/courses/<课程>/materials/  **只读**
  ```

  **一门课一个笔记本,不是一门课下面多份笔记。** 理由不是"够用",是
  **多份笔记会逼模型编文件名**——它会产出「第一章笔记」「chapter1」「线代笔记1」
  这种互不一致的东西。**别让模型发明一个维度**,这和做菜那个 category 是同一个毛病。

  **没有 `write_note`(整篇覆盖)。** 一学期的笔记整篇覆盖太危险;`append` + `replace`
  够用,真要重构**用户直接编辑那个 `.md`**——那正是选文件而不是 SQLite 的理由。
  第一次 `append` 时隐式创建。

  **代价**:本子会长。所以 `read_note` **必须分页**,而 `search_notes` 从"锦上添花"
  变成**主要入口**——找一段的正常姿势是搜,不是从头读。

  **路径不出现在接口上。** 模型手里只有课程名(和课件的文件名)。
  只有一个内部函数知道目录在哪——同 M6-5,同 M5-4 那条教训
  (`Attachment` 上根本没有可写的 path 字段)。

  ### 工具:bundle 侧九个,读课件走 Steward 侧那两个

  ```
  list_courses()                              有哪些课
  read_note(course, page=1)                   分页读这门课的笔记本
  ...
  **Steward 侧的内置工具**,因为转换要调模型而 bundle 不含 LLM(见下面那节)。
  ...
  > 「学习那个你又没给它增加课程还有改名……**错了咋办**?」

  **隐式创建 + 不能改名 = 打错一次就是一门新课,而且永远躺在那儿。**
  「线性代数」/「线代」/「线性待数」会变成三门课,而笔记散在三个地方。

  两样,理由不同:

  1. **`rename_course(old, new)`** —— 必须有。改名要**连课件一起搬**
     (笔记本和 materials 在同一个目录下),而且 `new` 已存在时**拒绝**,
     别把两门课的笔记悄悄合成一份。**这一条在验收里要钉**:
     合并比重名更坏——重名你看得见,合并是静默的。
  2. **不加 `create_course`。** 隐式创建省一步,而打错的真正问题不是"没有创建动作",
     是**创建了你不知道**。所以:**`append_to_note` 写到一门不存在的课时,
     回话要明说"「X」这门课之前没有,给你新建了"**——你当场就能改名。
     加一个显式 create 挡不住打错(打错的那次照样会创建),**说一声才挡得住**。

  3. **`delete_course(course, reason, undo=False)`** —— 用户点的:「**delete 也加,
     llm 不是弱智**」。**他对,而且这是复核方第二次犯同一个偏见**
     ——上一次是 `replace`(理由"模型整篇重打可能漏一步"),这一次是 delete。
     **两次都是用"假设它会做错"代替"给一个做不错的机制"。**

     机制照 M5-20(`delete_expense`)那套,一个字不改:

     - **移到回收站,不是 `unlink`**:`data/courses/.trash/<课程>-<时间戳>/`。
       整个目录搬走(笔记本 + 课件一起),**一个字节不销毁**;
     - **`undo=True` 搬回来**,且要求**逐字节一致**(M5-20 那条验收口径就是这个);
     - **`reason` 必填**——和 `delete_expense` 一样,它让这个动作是有意的,
       而且撤回时你看得见当初为什么删;
     - **`list_courses(include_deleted=True)`** 才列出回收站里的,标「已删」
       ——「删错了恢复一下」的唯一入口就是这个(M5-20 的原话:
       **留痕的意义是查得到,查不到就等于没留**);
     - **`.trash` 自己不是一门课**:名字白名单已经拒了前导 `.`,所以模型
       **寻址不到它**——那道校验在这里顺带当了第二个角色。但 `list_courses` 要跳过它,
       加一条测试钉住(否则用户会看到一门叫 `.trash` 的课)。

     **为什么不是让用户自己删目录**:能自己删是选文件的好处,但**那是给用户的路,
     不是给她的路**。她答不上"帮我把这门课清掉"的时候,用户得去开终端
     ——而这个项目的全部意义就是不必去开终端。

  **没有 `write_material`,也不该有**——课件是老师给的,你没法 write 一个 PDF。

  ### ★ 课件怎么从微信走到某门课下面——用户点出这一环是缺的

  > 「现在模型也读不了下载下来的文件。那如果我要拖课件给他,他肯定要去复制什么的,
  > 然后也要加这个课件。我不知道,**因为他现在没有目录,所以怎么解决这个问题?**」

  **他说的两件事都对:**

  1. **现在没有任何工具能读一份存下来的文件。** `look_at_image` 只管图片;
  ...
  **对,而且这就是下面这个东西**——复核方原来叫它 `file_material`,名字不如
  ...
  ### ★ 不拷贝:一个池子 + id,读的那一份实现在 Steward 侧——**这是架构逼出来的**

  用户把两条路摆出来问哪个靠谱:

  > 「第一种是都放在一个目录,然后只用那个 ID 去分,要读的时候就直接 read id 就好。
  > 第二种就是弄个拷贝嘛。**但是拷贝要咋打开咋读呢?又不好弄。然后文件又要 bundle
  > 分开管理,读的逻辑也不好复用。**」

  **选第一种。而决定性的依据不是"哪个方便",是 AGENTS.md 第一段那条架构约束:**

  > 一个主控(Steward)持有全部智能,各生活领域是独立的 MCP bundle,
  > 只提供工具和数据,**不含 LLM**。

  **而"把一页转成文字"要调模型。** 所以那件事**不可能住在学习 bundle 里**
  ——它必须在 Steward 侧,和 `vision.py` 同一层。于是分工被定死:

  ```
  Steward 侧(内置工具)   read_image(id) / read_pdf(id, page)   ← **转换与缓存只有这一份**
  bundle 侧(学习)        add_file / list_materials                   ← 只存"哪个 id 属于
                                                                        哪门课、叫什么名"
  ```

  **跨这条边界的唯一把手就是 id。** 所以用户提的「直接 read id」
  **不是偏好问题,是架构推出来的**——复核方当时反对得没道理,记在这儿。

  **拷贝那条的死穴用户自己点出来了**:一旦 bundle 各自持有文件,
  每个都要一套渲染 + 转换 + 缓存,**而它们连调模型的权利都没有**。

  顺带三条,让"不拷贝"这个选择没有隐藏成本:

  - **池子是内容哈希命名的**(`<sha256>.pdf`),**本来就没法浏览**,所以不拷贝没损失什么;
  - **课件在用户电脑上本来就有原件**——它和笔记不是一回事:
    **笔记是用户亲手改的,课件是输入**;
  - 由此得一条判据,写下来以后好用:
    **用户会亲手改的东西用文件(笔记、做法),不会亲手改的用表(课件的归属、流水)。**

  ### 读的工具**按类型拆开**:`read_image(id)` / `read_pdf(id, page)`

  用户:「干脆改成几个吧,`read_pdf`、`read_image`,**以后还有 `read_docx` 什么的
  也总不能混在一起吧**。」

  **对,而且这把复核方上一条的补丁也拆穿了。** 上一版是一个 `read_file(id, page)`,
  而用户问"图片没有 page 啊",我的答复是**"图片就是 1 页"——那是在给一个不该统一的
  签名打补丁**,不是承认它。按 G7:**入口也不必统一,只要模型不用猜该调哪个。**

  拆开之后三个好处:

  ```
  签名诚实       read_pdf(id, page) 要页码;read_image(id) 不要 —— 各自是真的
  docstring 干净 read_pdf 讲分页、讲"转换在 add_file 时发生";
                 read_image 讲张数上限、讲"图不留在上下文里"
                 —— 混在一个工具里,两套话都得塞进同一段
  能力边界可见   **没有 `read_docx`,模型就知道读不了** —— 而一个 read_file
                 会让它试完才知道。**能力边界写在工具清单里,比写在错误信息里强。**
  ```

  **"模型得选哪个"这个顾虑不成立**:M6-2 那行报告本来就写着类型
  (`图片 · 收银条.jpg` / `文件 · 第3讲.pdf`),而且那行后面就跟着"要看就调哪个"。

  ### 两个工具各自的规矩

  ```
  read_image(id)
    - 给图部件,受 MAX_IMAGES_PER_TURN 管(一轮第 5 次 → 拒绝并说清)
    - **不转文字**(见下面那节),没有 page 参数
    - docstring 要写明:图**只在这一轮**进模型,要留下什么得当场说出来

  read_pdf(id, page)
    - 给那一页的**文字(缓存里的)+ 那一页的图**
    - page 超范围 → 人话 + **说共几页**
    - 还没转过 → 给图 + 说"这页还没转完",**不当场调模型**
    - 一次最多 4 页(同 MAX_IMAGES_PER_TURN),要更多 → 说清,**不许静默截断**
  ```

  **两者共用的是内部件,不是接口**:id 正则、缓存读写、`(media_id, page)` 那个键
  ——**抽成小函数给两边用,而不是抽成一个大工具**(这正是 G7 那句"入口可以不统一,
  但两处维护同一个事实是另一回事")。

  ### `look_at_image` 怎么办:**改名成 `read_image`,别留两个**

  它现在做的就是 `read_image` 的事。**两个入口做同一件事,迟早有一个漏掉新规则**
  (M4-4、M5-5、M5-21 各栽一次)。

  - **倾向:改名。** 和 `read_pdf` 成对,模型不用记两种命名风格;
    代价是老测试要跟着改 + 前缀重建一次——**而加 `read_pdf` 本来就要重建一次**,
    一并付掉。
  - 改完 `vision.py` 里"一个函数管两个出口"那条(到达轮 `load_images` 与重看)
    **仍然成立且不许动**——M5-5 补做就是为它付过账的。
  - **在回报里说清改没改、老测试动了哪几条。**

  ### 以后加类型:**加一个工具,不改现有的**

  `read_docx` / `read_xlsx` 真要来的那天,是**新增一个工具**,
  不是在 `read_file` 里加一个 `elif`。
  **而现在一个都不加**——没有任何证据说用户会发 .docx(G5:不做没有需求的抽象)。

  ### 文件名归 bundle 管,**Steward 侧只认 id**

  用户:「**bundle 内部自行处理文件名的事情**?然后 read 可以用同一套接口了,
  然后**模型提炼出来的文字和弄出来的图片都可以复用哈希缓存**。」

  **对,三件事各就各位:**

  ```
  显示名   住在 bundle 的归属表里(course → [(id, 显示名)])
           —— Steward 侧**压根不知道**这份 PDF 叫"第3讲",它只认 id
  读       read_image(id) / read_pdf(id, page)——按类型拆开,但**所有 bundle 共用同一套**
  缓存     键是 (media_id, page),跟着池子
  ```

  于是模型的动作链是:`list_materials(course)` 拿到**名字 + id** →
  `read_pdf(id, page)` 读。**名字给人看,id 给机器用**,两边都不需要知道对方的事。

  **归属表放哪**:学习 bundle 自己的库。按上面那条判据
  ——"用户会亲手改的用文件,不会亲手改的用表",归属是表。

  ### 图片**不转**文字——复核方加它只是为了对称,而对称不是理由

  用户:「**图片为啥要转文字?**」

  复核方上一版写的是"走 `add_file` 的都转,包括拍下来的板书"。**理由站不住:**

  ```
  50 页 PDF   你不可能每页都看 → 搜不到就等于没有 → **必须转**
  一张图      "就看一眼"是一次调用 → 转它省不下什么
  ```

  搜索那个论证**对图片不成立**,而我拿它套过去了,理由其实只是"对任何类型都一样"
  ——**对称不是理由**,这一天里已经被指出过好几次。

  **而真正值得转的那种情况,正确做法也不是缓存转录。** 比如拍了课程表、要反复查
  「我周三几点上课」:她**读一次,然后 `append_to_note` 写进笔记本**。
  那样它**可编辑、可纠错**,而且课程知识只有一个地方;
  一份缓存的转录既改不动、又和笔记本抢"这门课的资料在哪"这个问题。

  **所以:**

  ```
  PDF     add_file 时逐页转(N 页)→ 缓存 → 可搜
  图片    **不转**。read_image 给图,她看完自己决定要不要写进笔记
  ```

  **真到"我拍的板书太多了找不着"那天再说**——那时候机制现成(和 PDF 那条同一套),
  改动是一行。**而现在连一张板书都还没有。**

  ### 缓存:文字**必须**缓存,图**可以**缓存——两者代价差一个量级

  用户说的"都可以复用哈希缓存"是对的,但**两样的理由不同,别混成一条规则**:

  ```
  转出来的文字   一次模型调用(约 1.4k token、1.9 秒)  → **必须缓存**,不然每次读都重花
  渲染出来的图   一次本地渲染(pypdfium2,约 0.1 秒)   → **缓存是省事,不是省钱**
  ```

  所以:

  - **文字缓存是正确性的一部分**:它还决定了 `search_notes` 能不能搜到课件
    (没转完 = 搜不到 = 和"没写过"长得一样);
  - **图缓存可以有大小上限,满了就丢**——丢了重渲染 0.1 秒,**代价可忽略**。
    一份 200 页 PDF 的页图约 10 MB,而 30 GB 的盘上这不是问题;真要省,先丢图别丢文字。
  - **两样都键在 `(media_id, page)`**,所以**同一份 PDF 被两门课引用时共用**
    ——不是优化,是"一个事实一个出处"。
  - **图每次读仍然要重新送进上下文**(M5-5:图只在到达轮进模型),
    **缓存省的是渲染,不是发送**。这一句要写进注释,否则下一个人会以为缓存了就不用再送。
  ...
  - **`media_id` 走和 `look_at_image` 同一条正则**(纯十六进制),
    **共用同一个常量**——两处各写一份的那天就开始漂;
  - **`name` 过和课程名同一套白名单**(拒 `/`、`..`、前导 `.`、超长);
    模型该给一个像样的名字(「第3讲-并发控制.pdf」),而不是把 `ab12cd…` 当名字;
  - **`name` 已存在 → 拒绝**,别把两份课件悄悄合成一份(同 `rename_*` 那条);
  - **复制,不是移动**:media 池那份留着。它是"用户发过什么"的原始记录,
    而不可协商第 3 条管的正是这个。硬链接也行(同一文件系统、省一份字节),
    **但要在回报里说清选了哪个**;
  - **不认识的 `media_id` / 不是 PDF 的东西 → 人话**,不抛。
  ...
  **只有 `add_file` 成功之后才触发整份转换。**
  ...
  用户拖一份 PDF 过来、什么都没说 → **她该问一句"这是哪门课的"**,
  而不是猜文件名、也不是丢进一个 `未分类` 的桶。
  **不要为此加"待归档"的状态**——那是个需要清理的队列,而一句话就能问清。
  文件本来就还在 media 池里,不会丢。

  课程名校验、落点兜底、`replace` 的"匹配不上就拒绝"、搜索不建索引直接扫文件
  ——**全部照 M6-5,一个字都不许另写一套**(接缝见 M6-5 那一节)。

  ### 课件:一页给两样,文字和图各管一头

  用户原话:「**你不要算经济账,效果不好你 1000 token 能抵消??**
  按我说最好就是**可以读单页的图片和文字,都弄出来搞一搞**。」

  ```
  ① 这一页转出来的文字  → 是工具结果,**留在起居注和 L0 里**,这一轮之后还能回头引用
  ② 这一页渲染出来的图  → **只在到达轮进模型**(M5-5 的机制),看完就没了
  ```

  **这不用设计,是两条已有的规矩叠出来的**:她**当下**看得见版式、公式、板书,
  **之后**手里仍然有那一页的文字。**但要写进 docstring**——否则下一个人会以为
  图也能回头看,然后为"让图留下来"去动 M5-5 那条机制。

  **一次最多 4 页**(`MAX_IMAGES_PER_TURN`,**不许为这个放宽**:注入面不随轮次累积
  就是靠它)。要读第 7 到 20 页,工具得回「一次最多 4 页,先给你 7-10」,
  ...
  **先说清这不是绕路,是唯一的路**:上面 ① 已经把三条"直接喂 PDF"全试死了。
  模型只吃 webp/png/jpeg/gif,所以 PDF 想进模型**必须先变成图**。
  剩下的选择只是"变成图之后谁来读"——而那就是下面这张对比表。

  用户:「**就是调我们用的那个模型**……因为那样你还能给指令——比如
  **文字呈现不了的改成一两句对图片的描述并标记图片内容**,然后**公式也能对**。」

  **赢在一个本地 OCR 结构上做不到的地方:OCR 只能转录字形,模型能描述。**
  三条路对打(2026-09-12 实测,同一页带表格 + 公式 + 时序图):

  ```
                     正文    表格            公式             图 / 图表
  文字层(pypdf)      打平    **行列全丢**    ■ + 散字符       什么都没有
  本地 OCR(v6-small) 打平    行结构保住      **Σ 丢了**       什么都没有
  视觉模型 + 指令     打平    完整 markdown   **正确 LaTeX**   **准确描述** ★
  ```

  模型实际输出(1.9 秒,prompt 1128 token、输出 263 token):

  ```
  | Level            | Dirty Read | Phantom | Cost |     ← 5 行全对
  | Read Uncommitted | yes        | yes     | 1    |
  \( T(n) = \sum_{i=1}^{n} w_i / t_i \)                  ← 另两条都失败的地方
  [图: 一个时序图,展示 Thread A、Thread B 和 Connection 三条水平线……Connection
  线右侧标有红色文字 "X cursor corrupted"。]              ← 另两条什么都给不出
  ```

  **所以只留 `pypdfium2`**(把页面渲染成图,而图那一半本来就要它)。
  `rapidocr`(30 MB 模型)、`onnxruntime`(76 MB)、773 MB 峰值、子进程设计、`pypdf`
  ——**全部不要**。

  ### 转换用的那段指令是这一条的产物,原样固定

  实测有效的五条。**别改词序、别"优化"措辞;要改,先重新打一遍那三个样本**
  (表格页、公式页、时序图页):
  ...
  ### 这一步引入一个注入面,按 M5-5 那套处理

  **课件是外部内容**(用户下载的 PDF 可能是任何东西),而我们**把它喂给模型、
  再把输出存下来**,那份输出之后每次 `read_material` 都进上下文。
  所以一页里写着"忽略以上规则"能影响转换结果。

  - **机制那一半**:存下来的文字**读的时候照样过围栏 + 中和 + 来源标注**,
    和 `web_fetch` **共用同一个出口**,不许另写一套;
  - **说服那一半**:上面第 5 条就是框定语,**它是说服不是机制**(M5-5 立的区分),
    别把它当防线;
  - **不拉不可信闩**:课件是用户自己给的材料,不是模型上网捞回来的。
    **这个判断要写在 docstring 里**,免得下一个人以为漏了。

  ### 什么时候转:**入库时整份转完,不懒转**

  懒转(读到第 3 页才转第 3 页)省钱,但**搜索会不完整,而不完整的搜索是静默错的**
  ——「搜不到」和「没写过」长得一模一样。**这条论证和"搜索不建索引"那次是同一条。**

  - 入库时逐页渲染 + 转换,**页数上限要有**,并**告诉用户转了多少页、还剩多少**;
  - **必须能续**:一页失败不影响别页,重跑只补没转的那些
    (M5-30 立的形状:区分成功 / 失败 / 未完成);
  - 一份 50 页讲义约 50 次调用、每次约 1.4k token,**一次性**,之后只读缓存。

  ### 课程表:**就是一份笔记,不是一张表**

  用户原来要"一条条录入课程表"。有了笔记本之后**它不需要任何结构**:
  在那门课的本子里写「周三 10:00 高数 / 周三 14:00 线代」。
  「我明天有什么课」= 读笔记 + `current_time`,**而这两样都已经有了**。
  **不要为周期性建规则引擎、不要展开成每一次。** 真到"她答错了"那天再说。

  ### RAG 知识库:**不建**

  用户问「RAG 知识库现在主流还用吗」。查了(2026-09-12):
  「chunk → embed → 塞进向量库」那种 2023 版 vanilla RAG **确实被判死了**;
  活下来的是**把检索决定交给模型自己**的形态。而且:**知识库在约 20 万 token 以下时,
  带缓存的全上下文可以直接打败自建检索基础设施。**

  **这个项目已经是后者了**:`search_history`(FTS5 词法)+ `recall_similar`
  (model2vec + sqlite-vec 语义)+ 档案每轮全量在场靠缓存不额外付钱,
  **而模型自己决定什么时候调哪个**。再搭一套向量 pipeline 恰恰是去做那个已被判死的版本。

  真需要语义搜笔记那天(字面搜不出来):`embed()` 和 sqlite-vec **都已经在了**,
  那是把现有那条路多接一个数据源,不是新建一套。**而那一天要有证据才算到。**

  ### 一个前置缺口(可以并进 M6-1)

  **现在收到 PDF 会被认成 `application/octet-stream`、存成 `<hash>.bin`**
  ——`wechat._MAGIC` 里没有 `%PDF-`,`envelope.SUFFIXES` 里没有 `application/pdf`。
  存下来了,但认不出、用不了。**注意 M5-5 那条教训**:漏掉一种格式会让它
  掉进 octet-stream 然后**一声不响地不进模型**,而
  「把一次响亮的失败换成一次静默的失败不是修复」。

  ### 明确不做

  - **不装任何本地 OCR**(tesseract / rapidocr 都不装)——表格赢一点、公式和图全输,
    而代价是 110 MB 模型 + 773 MB 峰值 + 一套子进程;
  - **不上 PP-Structure 的表格/公式专用模型**(又一批模型又一份内存,
    而"模型 + 指令"已经把表格和公式都拿下了);
  - **不起 subagent**(转换是一条顺序的作业,不是需要决策的任务);
  - **不用转出来的文字替代看图**(两样各管一头,用户定的);
  - **不做作业和考试**(那是 M6-7);
  - **不做课程表的结构化**、不做成绩、不做 GPA、不做课程评价、不做选课。

  ### 复核方在这一节判错的四次——**四次都是同一个毛病**

  1. **把作业/考试放进学习 bundle**,还问"它们和待办的边界在哪"。
     用户直接答了:**没有边界,它们就是待办。** 另造一套就是第二份真相。
  2. **两层命名空间 `(课程, 笔记名)`**——而一门课一个本子就够,
     多一层只是逼模型编文件名。
  3. **"有文字层就抽文字、没有才渲染成图"**——用户:「**你不要算经济账,
     效果不好你 1000 token 能抵消??**」
  4. **拿 `rapidocr-onnxruntime`(默认 v4)打,报"读不如视觉模型准"**
     ——换 v6-small 重打就全对了。**我测了 v4 就报成"RapidOCR 如何如何"。**

  **前三次是同一件事:先想"怎么把它做对",而不是先问"用户会怎么用它"。**
  第四次是另一条,但同样致命——**报一个库或模型的结论之前,
  先打印它实际加载的是哪个版本的哪个文件。**

  ### 验收口径

  - **笔记**:`append` 三次 → 三段都在、顺序不变、原有内容一字没动;
    `replace` 的 `old` 出现一次 → 改,**零次 / 多次 → 拒绝并说清**,
    改完回话带得出改动前后那一小段,**绝不许"改第一处"**;
  - `read_note` 分页钳位(0 / 负数 / 超大)+ 总页数;`search_notes` 给不给 `course` 都对;
  - **非法课程名**(`../../etc/passwd`、`..`、`/etc/passwd`、`a/b`、`.hidden`、空、超长)
    → 全拒且回人话;**注释掉白名单那一道,落点兜底仍然挡住,反之亦然**;
  - **工具签名里没有路径参数**——加一条机械检查钉住(这是设计约束,不是风格偏好);
  - **课件**:`read_pdf(id, page)` 一页**同时**给文字和图;要 4 页以上 → 说清一次最多 4 页,
    **不许静默截断**;还没转完 / 转失败 → **说得出是哪种**,
    别让"没文字"和"还没处理"混成一句;
  - **`read_image` 没有 page 参数**(拿工具 schema 钉住);`read_pdf` 的 page
    超范围 → 拒绝且**说出共几页**;
  - **图片一律不转**:`add_file` 一张图 → **模型调用计数为 0**;
    之后 `read_image` → 只有图、没有文字,**计数仍然为 0**
    (拿计数钉死,别让它为了"对称"偷偷转);
  - **`read_pdf` 不触发转换**:读一页还没转过的 → 给图 + 说"这页还没转完",
    **不当场调模型**(转换只发生在 `add_file`);
  - **没有 `read_docx` 之类的**——工具清单里不许出现没有实现的读取器;
  - **缓存共用**:两门课 `add_file` 同一个 media id → 第二门课读那一页时
    **不再调模型**(同样数调用次数);
  - **图缓存丢了不影响正确性**:清掉图缓存 → 再读那一页照样给得出图和文字;
  - **`rename_course` 不搬字节**:改完归属表里的 id 没变、media 池一个文件没动;
  - **转换**:拿那三个样本页(表格 / 公式 / 时序图)各钉一条——
    表格出 markdown 且行列对得上、公式出 LaTeX、图出 `[图:...]`;
  - **转换能续**:让第 2 页失败 → 第 1、3 页照样转完,重跑只补第 2 页;
  - **`rename_course`**:笔记本跟着搬、**课件只改归属表不搬字节**;`new` 已存在 → **拒绝**
    (拿一门有笔记也有课件的课钉死"合并没发生");不存在的 `old` → 人话;
  - **新建要说出口**:`append_to_note` 写到不存在的课 → 回话里出现"新建"字样
    (拿一条断言钉住措辞——**这句话是这条缺口的唯一防线**);
  ...
  - **`add_file`**:拿一个真的 media id 归档 → 归属表里出现那一条、**一个字节都没拷**、
    整份转换被触发;`name` 重名 → 拒绝;
...
- **M6-6d 工具名加 bundle 前缀——分层注入不做了**(2026-09-13 用户定的)

  ### 由来:用户以为有分层注入,而实际一直是全量注入

  用户问「现在是不是所有 bundle 的工具一次性全都注入 tools」,核下来**是**:组装根在同一个
  进程里 import 每个 bundle 的 `build()`,拼成一个扁平列表,每轮整个交给模型;
  `mcp_servers` 是空的,bundle 里的 `create_server()` 是给以后拆容器留的形状。
  用户:「如果不是分 mcp 注入的,那我分 bundle 有什么意义?**我的意思一开始就是说做分层注入。**」
  **DESIGN 里分层的只有 skill**(目录行 + `read_skill`),工具这一层从头到尾没人落实,
  **而复核方上一条还把现状说成了"就是这么设计的"**——那是把一次漂移说成了一个决定。

  ### 为什么最后不做分层,改成加前缀——**量出来的,不是猜的**

  - **光换成 MCP 不会分层**:pydantic-ai 2.31.0 接 MCP 服务时 `list_tools()` 把全部工具
    放进同一个 `tools` 数组(`mcp.py:1292`)。
  - **Claude 的 tool search 是服务端做的**:请求里照发全部定义,标 `defer_loading` 的服务端
    不放进前缀,搜到时在对话里插引用、每轮重新展开,前缀不动、缓存不断。
  - **DeepSeek 的 Anthropic 端点(`api.deepseek.com/anthropic`)不支持它**,三条实测:
    内置 `tool_search_tool_bm25_20251119` → HTTP 400(只认它自家的 `web_search_*`);
    `defer_loading` 被接受但**被忽略**(26 个工具标延迟 9231 token,不标 9179);
    自写搜索工具返回 `tool_reference` → 200 但没展开(和普通文本 181 对 185 token)。
  - **`tools` 数组动一下,整段缓存失效**:同一段约 4000 token 的历史,原样再发命中 4224,
    末尾追加一个工具**只命中 256**,退回原数组又命中 4224。DeepSeek 把工具渲染在最前面。
  - 于是剩下的唯一缓存安全的分层是"固定数组 + 加载进对话 + 通用入口调用",
    **代价是丢掉原生调用的参数约束**。用户:
    > 「你注册工具像 mcp 那样给个前缀就算了吧😂 因为工具这玩意好像就是一整个 schemas,
    > 很难说折叠的吧。」

    **他对。** 前缀拿到的是分层真正想要的那一半——**同一个 bundle 的工具在名字上就是一组**
    (Anthropic 文档把"prefix by service or resource"列为提高选对工具准确度的办法,
    也写了「工具超过 30–50 个,选对的准确度开始下降」,我们现在 40 个)——
    而不碰缓存、不丢原生调用。

  ### 形状(复核方的默认,任务书照这个写,执行方可以论证推翻)

  - **`<manifest 名>__<原工具名>`**,双下划线(和 MCP 客户端 `mcp__server__tool` 同形;
    单下划线分不清边界,`web_search` 自己就带一个;OpenAI 系函数名不许有点号)。
    **前缀从 manifest 的 `name` 派生,不手写**——一个 bundle 冒充不了别人的前缀。
  - **内置工具不加前缀**(它们是主控自己的,没有"服务"可言)。memory 是 bundle,**加**。
  - **原工具名不改**:`recipes__list_recipes` 有点啰嗦,但"加前缀"和"重新起名"是两件事,
    **别把一次机械改名变成一次命名讨论**。

  ### ★ 第零个坑,安全攸关——派任务之前才核出来,上面三个坑里没有它

  `loop.py` 的 `all_tools()` 靠 `getattr(t, "__name__", "") == "propose_fact"` 认出那个工具,
  给它套 **P0-1 防提示注入的守卫**(不可信轮里 `user_stated` 强制降成 `untrusted`,不让注入
  自动写进长期档案)。**改名成 `memory__propose_fact` 之后这个 `if` 永远不成立,守卫整个脱落
  ——不报错、不一定红,账本多一条后门。** 任务书要求:守卫改成按身份认、不靠字符串字面量;
  一条按真实组装顺序的端到端测试,**先在改名后、修守卫前确认它是红的**;全仓库逐处搜一遍
  还有没有别处按名字认 bundle 工具。
  **这正是 G8 的形状**:"它是 propose_fact"这件事写在两处(函数名、守卫里的字面量),
  只改一处就脱节——而脱节的症状是安全边界静默消失。

  ### ★ 三个坑,每一个都是改名改出来的

  1. **历史回放会把旧名字的往返整次丢掉**(`loop.py` 那段「回放的工具名必须是注册过的,
     认不出的整次往返丢掉」——那道闸是对的,L3 的封闭词表)。直接改名 = 部署那一刻
     **所有近期工具往返从 L0 一次性消失**(她刚列出来的 #id 整段忘掉)。
     **起居注不许改写**(不可协商第 3 条)。要执行方论证:回放时认一张**旧名 → 新名**的
     映射表,并且**这张表要有删除条件**(L0 里最老的一轮都晚于改名那天,它就没有读者了,G6)。
  2. **251 处文字引用**(回话里的「先用 list_recent 看一眼」、docstring、skill、manifest)
     全部要指向**能调得到的那个名字**,不然就是在把模型指向一个不存在的工具。
     **要一条机械检查**:扫 bundle 与主控源码里的字符串字面量、skill 与 manifest,
     **出现裸的 bundle 工具名就红**(G8:一个事实有几处表述就要在几处守)。
  3. **前缀重建一次**(工具 schema 全改名),而**改名当天是全量未命中**——这是一次性的,
     记进回报,不是缺陷。

- **M6-7 结构化的事务 / 待办**——用户点的;**这是话头承不起的那一半(M5-32)**

  真机证据:[120] 用户想录课程表,她答「这个功能我目前还弄不了」;
  [123] 她自己纠正「日程和待办是有的,我刚才说岔了」,然后说实话——
  「我现在的日程/待办,存的是**一件事、一句话**,它不认结构化的一张表」。
  **用户明知她可能做不了还是提了两次,被拒之后又追问了一次**,这是穿过筛子的需求。

  ### 范围里多了作业和考试——**用户明确要求装在这里**

  > 「作业考试也是待办啊,为什么不能用待办系统来装呢?」

  所以这一条要装得下:一次性的事(「周五交材料」)**和**学业上的到期项
  (作业、考试)。**它们没有边界,就是同一种东西**——另造一套就是第二份真相。
  课程表**不在这里**(它是周期性的,而且已经在 M6-6 变成一份笔记了)。
  **可以有一个可选的"哪门课"字段**,但别因此长出一套课程管理——课程在 M6-6。

  ### 动手前必须先答的,和 M5-32 列的是同一批

  **不要直接交实现。** 先把这三件写出来给复核方看:

  1. **待办和话头的边界在哪?** 两个都在记"没完的事"。**如果答不上来,就是加了第二份真相。**
     我的倾向:话头管"聊到一半的事"(没有到期、没有完成判据),
     待办管"有交付时间、能核对完成"的事——但**这要你自己论证,别照抄我这句。**
  2. **课程表是待办吗?** 它是周期性的(每周三 10:00 高数),而待办是一次性的。
     **如果课程表要另一套东西,说清楚**——宁可这一步只做待办、课程表另立,
     也别搞一个两头都不像的。
  3. **"完成"怎么表示?** 删掉?标状态位?(M5-20 已经为流水答过一次这个问题,
     结论是标状态位 + 能撤回,**照它**。)

  ### 明确不做

  不做提醒——**用户砍掉了整个推送方向**(见 M6-8),所以**到期时间是个查询字段,
  不是一个会响的闹钟**。「我这周要交什么」是你问她才答。
  **这一点要写进工具的 docstring**,免得下一个人看到「到期时间」就以为该有提醒。
  也不做优先级、不做子任务、不做标签。**先让它能存能查能勾掉。**

- **M6-8 夜间归拢自动跑**——原本是"调度器 + 提醒 + 晨报",**被用户砍成了一件事**

  ### 砍它的那句话,是这个项目到目前最值钱的一条产品判断

  > 「我发现我之前不喜欢记账并不是说连发条信息都不想,而是**点来点去还要分类太过复杂**。
  > 所以就不走推送了我觉得。」

  **价值在输入,不在输出。** 用户原来不记账不是因为懒得发消息——对话框把"点来点去 +
  选分类"那一步整个删掉了,而那才是原来的摩擦。所以该投的是「说一句就记上」,
  **不是「她来提醒你」**。

  第三方 review 说「我会把持续运行和一条主动事务闭环排在大规模扩领域之前」,
  前半句仍然对(常驻、重启不漏事),**后半句被用户否了**——而他有权否,
  因为那条建议是从"一个通用助手该有什么"推的,不是从他怎么用推的。

  ### 所以这一条只剩一件事:**把 `/sweep` 从手动变成自动**

  这不是推送,是后台作业。**而"手动"已经吃过一次亏**:M5-24 那个缺口
  (头两天的对话一次没归拢过)之所以攒到跨三天,就是因为没人每天跑它。

  - 每天跑一次,失败不重试到死(M5-30 立的形状:区分成功/失败/未完成);
  - **跑完提出待审时那一条通知保留**——一天最多一次,而且它是"有东西等你点头",
    不是"她来催你"。这是复核方的判断,用户确认过。

  ### 明确不做(用户砍掉的)

  - **晨报**:主动在早上说话,正是被否掉的那个形状。
  - **待办到期提醒**:所以 M6-7 的待办是"你问她才答",到期时间是个**查询字段**,
    不是一个会响的闹钟。**这一点要写进 M6-7 的 docstring**,免得下一个人看到
    "到期时间"就以为该有提醒。
  - **预算的定时那一半**:M6-4 那个"记账当场说"就是全部了,而它现在有了理由
    ——花钱那一刻说,恰好是"输入时顺手给反馈",和这条判断同向。

  ### 一条前提没了

  原来写着「M5-34 必须在这之前有答案」,理由是"主动推送会让系统在用户没开口的时候
  自己产生轮次"。**归拢本来就是这样的作业**(它现在也在跑),所以这条前提不是被这一步
  引入的。M5-34 仍然要做,但**它的硬截止线回到短信入账那里**,不在这一条。

- **M6-8a 被风控拒掉的一批卡死归拢**(M6-8 验收时记下,2026-09-13)

  > **同日撤销,不做(见 DESIGN D22)。** 这一条的前提「DeepSeek 内容风控拒的是 400」是 M6-8 执行方回报里写的,
  > **复核方没核实就转述、据此立了这一条**。用户:「以己度人是吧,你 claude 有审核没错,deepseek api
  > 什么时候有审核了」。核下来:**官方错误码文档里没有任何审核相关的码**,只有第三方报告(`400 Content
  > Exists Risk`,多数是 V2 时期、触发内容是政治敏感词),当前模型没量过;而用户不聊那类话题。
  > 按 G5 不为没有证据的假设预付。**下面原文留着,是记"为什么曾经以为要做"。**

  M6-8 让归拢每天自动跑。400(服务商拒了**这一次请求的内容**)当天判失败是对的——但"这次请求的内容"
  是**一批对话**,而游标停在这一批前面。第二天那一班喂的还是同一批,还是 400,**之后每天都失败,
  归拢实际停摆**;手动 `/sweep` 以前也有这个问题,可那时是人在跑、卡住看得见,**现在是自动跑、没人盯着**。

  按 E4 "内容被拒 → 这件东西判死",判死的应该是**那一批**,不该是它后面所有的对话。但直接跳过有风险:
  **400 也可能是我们自己的请求格式写坏了**(改代码改出来的),那样"遇到 400 就跳过"会一晚上把所有批次跳光。
  两条可选的路,任务书让执行方论证选哪条:
  - **隔天再跳**:同一个游标在两个不同的日子都 400,才跳过那一批,并在起居注里记下跳过了哪一段(seq 起止);
    跳过之后紧接着的那一批要是也 400,**停止跳过**——那多半是系统性的,不是内容;
  - **二分找出那条**:把被拒的一批对半拆开重试,找出是哪条消息让服务商拒的,只跳那一条。
  **跳过只推归拢游标**,起居注一个字节不动,那段对话照样搜得到。

- **M6-9 隔一阵问一嘴**——用户点的;**这是"助手在场",不是"任务系统戳你"**

  > 「你隔个一会,比如说一个小时这种,**随机也行,更有人味一点**,随机在某个区间内。
  > 你隔一段时间让他发个消息来,就可以给他没关的话头啊之类的。**就是别太生硬吧**,
  > 就是不一定要提到话头,就是跟助手一样问一嘴,或者提一下别的、我喜欢的也行。」

  ### 先把它和被砍掉的那批分清——**否则下一个人会拿这条把晨报复活**

  ```
  用户砍掉的(M6-8)  晨报、待办到期提醒、预算定时推送   —— **任务系统在戳你**
  用户要的(这一条)  隔一阵问一嘴                     —— **助手在场**
  ```

  砍掉那批的理由是「价值在输入不在输出」(对话框删掉了"点来点去 + 选分类"那一步)。
  **这一条不在那个轴上**:它不是替你办事,是有个人隔一阵搭一句话。
  **M6-8「明确不做」那一栏仍然有效**,这一条不是它的例外。

  ### 怎么触发:**信封里放一条指令,让模型自己去看、自己组织话**

  用户:「**为什么你推送的那条不进模型呢?要进啊,也是上下文的一部分啊。**
  你进模型不用另外开窗口啊,**你就让他看看推送什么就行了**。
  **你加一条带 system 的指令让他推送**,就只是不显示在我这里而已,对话记录有。」

  **这和现有那条路不是一回事,复核方原来写混了。** 查过代码:

  ```
  现在的 make_daily_notifier   文本是**代码写死的**(「夜间归拢提出 3 条待审提案」),
                               它只负责落痕 + 投出件箱,**压根不调模型**
  这一条要的                   信封里放**一条指令**,跑一遍正常的轮,
                               **模型自己看手里有什么、自己组织话**
  ```

  后者才可能"不生硬"——代码写死的文本永远是生硬的,而"随机挑一条事实来寒暄"
  只会换一种生硬。**判断和措辞都必须是模型的。**

  所以形状是:

  ```
  唤醒 → 造一个 source="nudge" 的信封,内容是**一条指令**(不是要推的正文)
       → 走正常的一轮(前缀 + 档案 + 话头 + L0 全在,工具也能用)
       → 模型的回复就是要推的那句;它判断没什么好说的 → **回空,不推**
       → 信封和回复**都落起居注、都进 L0**
       → 出件箱只投那句回复 —— **用户看不到那条指令,而对话记录里有**
  ```

  **两条现成的东西正好接住它:**

  1. **`(系统触发 · {source}/{channel})` 那一支已经在** `assembler` 里
     (M4-7 建的,原则是「**不伪装成用户**」)。这条指令必须走它
     ——否则历史里会变成"用户说过『现在主动说一句』",而**模型之后每一轮都会照做**。
     这一点是硬的,不是风格问题。
  2. **M4-7 的"推送落成一轮完整对话"** 那条口径照旧:用户切过来回一句时,
     模型看得到上下文。当初的理由是「早上推『这个月餐饮 1240』,用户说『太多了吧』,
     **模型没有任何上下文**——那不是体验问题,是**系统失忆**」。

  **不许另建发送路径**:出件箱、渠道、退避都走现有那一套。
  新增的只有"唤醒 + 造那条指令信封"。

  ### 最要紧的一件事:**她必须能决定不发**

  如果每次定时都吐一条,它就变成噪音,而噪音只会让用户关掉整个功能。
  所以这一条的核心不是"定时发",是**定时去看一眼,然后决定要不要开口**:

  ```
  醒来 → 看手里有什么(开着的话头 / 档案里的事实 / 最近聊过什么 / 现在几点)
       → 有值得说的 → 说一句
       → 没有       → **不发,而且不算失败**
  ```

  **"不发"是正常出口,要在 docstring 和验收里都写清楚。**
  这是 G6 的形状:先问这一条该不该存在——**一条没什么好说的问候,不如沉默。**

  ### 别太生硬:随机区间,而且那个区间要能调

  - **间隔在一个区间里随机**(用户原话),默认建议 45–90 分钟,`LARARIUM_NUDGE_MIN/MAX`;
  - **不许对齐整点**——整点发是"定时任务"的味道,而这一条要的正是没有那个味道;
  - **静默时段**要有,但**默认值要小心**:这个用户凌晨四点在聊天(起居注里好几次),
    所以「晚上不发」对他是错的。默认给一个窄的(比如 02:00–08:00),
    **并且写明它是拍的、该由用户自己调**。
  ...
  - **说过的要记得**:哪个话头/哪条事实被当过话题、什么时候。走起居注,别另建表;
  - **同一个话头在 N 天内不重复**(N 拍一个,写明是拍的);
  - **用户没回上一条的时候,不发下一条**——连着两条无人应答的问候,
    就是"这东西在自言自语"。**这条是硬的**:上一条主动消息之后没有任何入站,
    就不发新的。

  ### 24 小时窗口:**发不出去就别攒着**

  微信的推送窗口是 24 小时(官方原话,M5-1 已经定性),用户超过 24 小时没开口时
  推送发不出去。现有行为是"消息留着等用户开口,不丢"——**而这一条要反过来**:

  **过期的问候必须丢掉。** 三天前那句「在忙什么」现在发出去是荒谬的,
  比不发更坏。所以这一条投出件箱的消息要带**保质期**,过期直接扔,
  **并且这件事要能在起居注里查到**(扔了多少条),否则"她怎么不说话了"没处查。

  ### 必须能一句话关掉

  第三方 review 的原话:「**取消后不再提醒、没有进展时不打扰**」。
  所以:
  - 用户说"别发了"/"安静点"这类话 → **她自己就能关**(给她一个工具,
    或者一条斜杠命令,**二选一,别两个都做**);
  - **关掉之后不许"过几天自己又开了"**;
  - `LARARIUM_NUDGE=off` 默认**关**——和 `LARARIUM_VISION` 同一个判断:
    「复制一份 .env.example 不该顺带打开一个会主动找你说话的东西」。

  ### 说什么:**有材料才说,而材料已经都在**

  ```
  开着的话头       「课程表那事你还想录吗」——但受上面"不重复"那条管
  档案里的事实     看 F1 / 东方树叶 / 胃不好 —— 「今晚有比赛,记得别空腹」
  最近聊过的       接着上次那个话头往下
  什么都没有       **沉默**
  ```

  **不许为此新造数据源。** 话头、档案、起居注都已经在前缀或工具里了。
  **也不许把"随机挑一条事实来寒暄"做成机制**——那正好会变成生硬,
  而生硬是这一条唯一要避免的东西。判断交给模型,**我们只负责给它材料和"可以不说"的权利**。

  ### 验收口径

  - **"不发"走得通**:构造一个"没什么好说的"状态(无开着的话头、刚聊过、档案为空)
    → **不产生任何出件箱消息,且不记为失败**;
  - **间隔真的随机**:连续取 20 次间隔,落在区间内且**不全相等**
    (写死成常量的实现要红);
  - **不重复**:同一个话头两次唤醒 → 第二次不拿它当话题;
  - **无人应答不追加**:上一条主动消息之后没有入站 → 下一次唤醒不发;
  - **过期就丢**:造一条超过窗口的问候 → 扔掉,**起居注里查得到扔了**;
  - **关得掉**:关掉之后连跑 5 次唤醒,零条消息;
  - **落痕完整**:发出去的那一条**在起居注里是一轮完整对话**(信封 + 回复),
    用户接着回一句,模型**看得到上下文**(M4-7 立的口径,拿它当金样);
  - **那条指令不许伪装成用户**:拿一条测试钉住它在 L0 里渲染成
    `(系统触发 · nudge/...)`——**这一条最要紧**,伪装成用户的话模型之后每一轮
    都会以为用户要它主动说话;
  - **用户看不到那条指令**:出件箱里只有回复那一条,**指令一个字都不许出现**;
  - **模型能用工具**:唤醒那一轮它可以 `list_threads` / `current_time` / 读档案
    ——拿一条测试证明工具调用在这一轮是通的(不然它只能凭前缀里那点东西说话);
  - 静默时段生效;`LARARIUM_NUDGE` 默认 off。

  ### 明确不做

  - **不做晨报**(被砍过一次,见 M6-8);
  - **不做到期提醒**(同上);
  - **不做"连续几天没说话就找你"**——那是另一件事,而且它和"过期就丢"直接冲突,
    真要做得先想清楚(现在没有证据要它);
  - **不在这一条里加任何新的数据源**。

- **M6-10 消息带上"隔了多久"——她没有时间观念**(2026-09-13 用户点的)

  > 「他要知道每条信息是啥时候发,要不然他没什么时间观念😂。明明是昨天的信息,他觉得是刚才。」

  ### 查下来:**数据是有的,她不会算**

  真机上她实际收到的 L0 里,**用户的每条消息本来就带着发送时间**
  (`[2026-09-13T18:21:59+08:00] 已经不是明晚了 是今晚了`)。没时间的是她自己的回复和工具结果。
  而那个例子里,她昨天那条和今天这条的时间**都在手上**,还是顺着昨天说「明晚」
  ——一串 ISO 时间戳挂在每条开头,模型不会自己去和上一条比、再意识到"已经隔了一天"。
  **这是 L2 的形状:能算的不要问模型。**

  ### 缓存的判断(用户问的)

  给消息标时间**不破坏缓存**,前提是**一条消息渲染出来之后永远不变**——历史里每条的发送时间写下就固定,
  现在的标法缓存照样命中。**会破坏的只有两种**:把"现在几点"放进前缀(每轮变);
  给历史消息标**相对时间**(「3 小时前」下一轮就成了「4 小时前」)。

  ### 形状(复核方的默认)

  - **把间隔算好写给她**:「距上一条 18 小时,跨天了」。**必须只由起居注里存下的数据决定**
    (这一条的 ts 和它前一条的 ts),同一条消息无论在第几轮渲染都逐字节相同——不许拿"现在"去算。
  - **时间格式改成人读的**:「9月13日 周日 18:21」。**改格式那一次所有历史渲染都变**,
    部署当天缓存全量未命中一次(和改工具名那次一样,一次性的,不是缺陷)。
  - **她自己的回复不标时间**:标了她会学着在回复开头写 `[时间]`,然后发到用户微信里。

## 推后的(原 M6,一次都没被用户提起过)

- **短信/通知入账 provider**(原 M6-1/M6-2):**M5-34 是它的硬前提**
  ——那条路会从"一天走不了几次"变成"每天都走"。
- **健康 bundle**(原 M6-3):用户明确没选。**学习 bundle 后来他要了,现在是 M6-6。**
  契约的反复验证由做菜(M6-5)和学习(M6-6)两个 bundle 承担。
- **`python_sandbox`**(原 M6-5):**无网络**那条约束照旧(D12——有网络的话模型能自己
  POST 审批端点给自己盖章)。没有使用证据要它。
...
- **M5-24 归拢的光标会跳过没归拢过的历史**——静默丢事实,M5-23 验收时挖出来的

  **症状**:真机第一次 `/sweep`,journal 里有 147 条、跨三天;归拢只看了 seq 97..147,
  然后把光标直接推到 147。**seq 1..96——头两天的全部对话——一次都没归拢过,以后也不会。**
  用户提过五次的"有女朋友"、四次的"看 F1 但不是法拉利车迷",模型从头到尾没见过。

  **成因**在 `sweep.py:run()` 两行的配合:

  ```python
  window_events = self._journal.events_in_range(since, until)   # since = now - 24h
  new_events    = [e for e in window_events if e["seq"] > cursor]
  window_max    = max((e["seq"] for e in window_events), default=0)
  ...
  self._advance_cursor(window_max)      # ← 推到**窗口内**的最大 seq
  ```

  `window_max` 是"窗口里最大的",不是"处理到哪儿了"。于是 `cursor < seq < 窗口下界`
  那一段被跳过去,而 `_advance_cursor` 用的是 `MAX(cursor_seq, excluded)`,**只增不减**
  ——跳过去就回不来了。

  **不是只有首次会踩**。任何让归拢停一天以上的事都会造出这种缺口:服务停了、
  定时没跑、跑了但抛异常、机器关机过夜。而**失效是静默的**:少提的事实和
  "模型觉得不值得提"长得一模一样,`/pending` 里看不出区别,起居注里也看不出
  ——`sweep` 那条 input 事件只记了它**实际**喂了什么,没记它**跳过**了什么。

  **要改的是"扫哪一段"这件事本身**:窗口的下界应当是 `cursor`(上次归拢到哪儿),
  `since` 只该当**上界**那一侧的东西 / 或者干脆只做兜底。请在回报里说清楚你选了哪种。
  两个绕不过去的约束:

  - **一次别喂太多**:缺口攒了两周就是几千条,`_PROMPT_CONVO_MAX_CHARS=20000` 会把
    最早的砍掉——那等于换一种方式丢同样的东西。补历史要**分批**,每批推进一次光标,
    **光标只推进到这一批实际处理到的那条**。
  - **光标推进必须和"这批处理完了"绑死**,不能和窗口绑。这是这条 bug 的根。

  **验收口径**:
  - 造一个缺口(cursor=0,journal 里有跨三天的事件)→ 归拢**必须覆盖到 seq 1**,
    不许跳;分批的话,批数 × 批量要能盖满;
  - **光标推进 = 实际处理到的最大 seq**,拿一个"窗口内有更大 seq 但没被处理"的场景钉死它;
  - 缺口补完之后再跑一次 → no-op(P1-1 的内容幂等不能被这次改动破坏);
  - 现有 `test_sweep.py` 全绿。

  **另外两件小的,一起做**(都不值得单开一步):

  1. **补跑一次真机的缺口**:改完之后在服务器上把 `cursor_seq` 归零跑一遍,
     让 seq 1..96 真的被归拢一次。这不是测试,是把已经丢掉的东西捞回来。
  2. **小节路由**:三条提案全落进了「长期偏好」,而「在上学」明显该进「身份」。
     先查一遍是 `propose` 的 `section` 写死了、还是模型没被问过这件事,**再决定改哪儿**
     ——按 G6,先问"这个坏了该不该在",别上来就加一层分类。

- **M5-25 `test_finance_amend.py` 的裸数字断言撞时间戳**——会假红,也会假绿

  M5-15 留下的。`list_recent` / `amend_expense` 都渲染 `YYYY-MM-DD HH:MM`,
  而测试用 `"49"` `"22"` 这样的**裸数字**断言金额。两个方向都坏:

  ```
  记账时刻 23:49 → list_recent: "- #2 2026-09-08 23:49 餐饮 22.00 元"
                   assert "49" not in default   → False   假红
  记账时刻 22:22 → 记 71 元、amend 一次都没调
                   assert "22" in default       → True    假绿(金额是 71 也绿)
  ```

  上面两条是**实测**的(显式传 `occurred_at`,渲染路径和真机逐字相同),不是推演。
  今天 23:49 门禁真的红过一次。

  **受影响的行**:`tests/bundles/test_finance_amend.py:66`、`:145`、`:146`
  ——这三处的 `occurred_at` 缺省成"现在"。134 / 212 / 239 传了固定日期,不受影响,
  但**写法一样脆**,顺手一起改。

  **怎么改**:断金额就断金额,别断裸数字。`"22.00 元"` 这样的渲染片段就够,
  或者干脆断 `all_rows()` 里的 `amount_cents`——这个文件里已经有一半断言是那么写的。
  **不要靠"给测试传固定 occurred_at"了事**:那治的是这三条,治不了下一个人再写一条。

  **验收口径**:把系统时间(或显式 `occurred_at`)摆到 23:49 和 22:22 各跑一遍,
  两次都绿;并且把 `amend` 的实现改坏(比如金额不写库)时,这些断言**必须红**。

- **M5-26 `amend_expense` 改成就地编辑,拆掉整套作废行**(用户定的方向;吃掉 M5-20 補和 M5-25)

  **现在**:改一笔 = 插一条新行 + 把旧行标 `voided_by`。于是账本上有**两个**状态位,
  「这行还在账上」要同时问两个:`voided_by IS NULL AND deleted_at IS NULL`。

  **改成**:`UPDATE` 就地改。不插新行,`#id` 不变,`voided_by` 整列拿掉。

  ### 为什么(不是嫌它绕,是它没在给谁干活)

  按 G6 问作废行:**谁会读到它?什么时候读?读到之后能做什么?**

  真机两天、13 次工具调用,`include_voided=True` **一次都没有被调用过**。
  它写在 `list_recent` 的 docstring 里是为了「删错了恢复一下时找回那个 #id」
  ——那是 `deleted_at` 的活,和 `voided_by` 没关系。

  **留证据这件事已经有人在干了。** 每一轮对话、每一次工具调用的参数和返回,
  全都在起居注里,那是不可协商第 3 条管的地方。账本里再留一份是第二份,
  而**第二份从来没被读过**。

  代价已经收到两张账单:

  1. **删了的行还能被 amend 改活**(原 M5-20 補):`amend` 不看 `deleted_at`,
     一个改金额的动作副作用是撤销一次删除。就地编辑之后这个洞根本不存在
     ——改一条已删的行,它还是已删的。
  2. **真机账本里有一行自己作废自己**(`#3.voided_by = 3`),没有替代行。
     我拿当前代码并发跑 40 次复现不出来,**大概率是早期手动改库留下的**,不是代码写的;
     但库里允许存在这种行本身就没有任何约束挡着。列没了,这个形状就构造不出来。

  ### 要做的

  1. **`amend_expense` 改成 `UPDATE`**:只改传进来的字段,其余原样;`id` / `created_at`
     一个字节不动。回话去掉「旧的那条留着,标了作废」,改成说清楚改了什么
     ——`#id` 没变这件事要说出来,用户接下来可能还要拿这个号做别的事。
  2. **`voided_by` 整列拿掉**,连同它在两条聚合 SQL、`_RECENT_WHERE`、`list_recent`
     的「已作废(见 #N)」分支、`amend` 的「已经改过了」分支、`delete_expense` 的
     「已经被 #N 替代了」分支里的每一处。**一处都不许留**:留一处就是下一个人
     读到两套规则。
  3. **`include_voided` 改名 `include_deleted`**。它现在只管一件事了,名字得跟上
     ——工具 schema 改了,前缀会重建一次,`prefix_log` 会记,这个代价认。
  4. **已删的行仍然拒绝 amend**(原 M5-20 補,但理由变了):不再是"会复活",
     而是"改了你也看不见"。回一句人话,**指向 `record_expense` 不指向 `undo`**:

     > 「#1 已经删了,改不了。要记就直接记一笔新的。」

     ——「删了的账要改成别的数」不是撤销,是记一笔新的;绕去恢复只会在账本上
     多两条没意义的痕迹。`undo=True` 留着,只管「删错了,恢复一下」那一种。

  ### 迁移:真机上有 3 行已经作废了,**别让它们冒出来**

  列一拿掉,`voided_by IS NULL` 这个过滤条件就没了,**那 3 行会重新出现在账上**
  ——其中 2 行是被替代的旧版本(28 元交通、66 元没备注的那笔),1 行是用户明确说
  「作废」的测试账。冒出来就是账上凭空多三笔,而且用户不会知道为什么。

  ```
  #1  28.00 交通  voided_by=3   ← 被 #3 替代的旧版本
  #3  28.00 交通  voided_by=3   ← 用户说「作废」的那笔测试账(自己指自己)
  #5  66.00 餐饮  voided_by=6   ← 被 #6 替代的旧版本
  ```

  **补列机制里加一步**(和 `voided_by` / `deleted_at` 当初补列同一个位置、同一个形状):
  在删列之前,把 `voided_by IS NOT NULL AND deleted_at IS NULL` 的行统统标成已删,
  `deleted_reason` 写明是这次迁移标的。这样它们仍然查得到(`include_deleted=True`),
  但不进任何视图、不进任何合计——**和迁移前用户看到的一模一样**。

  **迁移前后用户看到的账必须逐字节一致**,这是这一条的硬口径:拿真机那份库跑一遍
  `list_recent()` 和 `query_spending(2026-09-01, 2026-09-30, category)`,
  改之前改之后两份输出 diff 为空。当前值:9 笔、合计 1055.03 元。

  ### 测试

  `test_finance_amend.py` 整个是围着"插新行 + 作废旧行"写的,得重写。重写时**顺手把
  M5-25 一起吃掉**:文件里那些 `assert "49" not in out` / `assert "22" in out` 的
  **裸数字断言撞渲染出来的时间戳**,23:49 会假红(9-8 门禁真红过一次),
  22:22 会假绿(记 71 元、amend 一次都不调,`assert "22" in out` 照样过——实测)。
  断金额就断金额:`"22.00 元"` 这样的渲染片段,或者干脆断 `amount_cents`。

  ### 验收口径

  - 改一笔 → **表里还是一行**,`id` / `created_at` 没变,只改的那个字段变了;
  - 只给要改的那一项 → 其余原样带过来(这条现在有,不许丢);
  - 改完 → `list_recent`、按类目、按天**三个出口一起断**(两条聚合 SQL 是分开写的,
    这一节栽过一次);
  - 已删的行 → 拒绝、账上一行不变、**回话里不出现 `undo`**;
  - 撤回后再改 → 正常;
  - **迁移**:拿一份带 `voided_by` 的老库(照真机那三行造)开库 → 那三行变成已删、
    `list_recent()` 和 `query_spending()` 的输出和迁移前逐字节一致;
  - `grep -rn "voided_by\|include_voided" src bundles tests` **零命中**;
  - 门禁全绿,且在 23:49 和 22:22 两个时刻各跑一遍 `test_finance_amend.py` 都绿。

  **做完先别推真机**,等验收——真机那份库要按迁移路径走一遍,不能手改。

- **M5-22 `web_fetch`(给她一个链接,她读得懂)**——走 Tavily `/extract`,**不自己抓**

  > **这一条被推翻重写过一次。** 原来的方案是自己抓 + 一整套 SSRF 防护(五步钉住 IP、
  > 绕过用例表、DNS rebinding、`getpeername` 复查、trafilatura 加七个依赖)。
  > 推翻它的是一次实测,记在下面——**留着这段,是因为那套防护看起来很专业,
  > 而"看起来很专业"正是它差点被写出来的原因。**

  ### 推翻它的那次实测

  当时替自己抓辩护的最后一条理由是:「我们的服务器在国内,Tavily 的不在,
  国内站从美国机房抓不到」。**拿真机打了一遍,这条是假的**
  (在墙内、带浏览器 UA、直接 GET):

  ```
  少数派 文章    200  HTML  2,679 字 → 正文     13 字   壳子
  知乎 专栏      HTTPError                              被挡
  微信公众号     200  HTML 31,613 字 → 正文     57 字   壳子
  澎湃 文章      HTTPError                              被挡
  36氪          200  HTML 17,501 字 → 正文     54 字   壳子
  CSDN          URLError                               被挡
  阮一峰 博客    200  HTML 102,460 字 → 正文 12,855 字   ★ 拿到
  ```

  **挡我们的是反爬、登录墙、JS 渲染,不是地理位置。** 在墙内一点忙都帮不上。
  能抓的(静态博客、文档站)`/extract` 一样能抓;抓不到的,自己抓也照样抓不到。
  而商业抽取服务多半还有住宅代理和渲染——**只会比我们自己抓强,不会更弱**。

  那么自己抓换来的是:一大堆安全攸关、写完就永远归我们维护的代码,**换来零**。
  额度这条也不成立:`/extract` 是 1 credit / 5 个 URL,免费档一个月 5000 次。

  **SSRF 面归零,不是"防住了"是"根本没有"**——我们从头到尾不发起模型可控的出站请求。
  唯一的出站目的地是 `api.tavily.com`,和 `web_search` 同一个,写死在代码里。

  ### 要做的

  1. **`web_fetch(url)` 内置工具**,和 `web_search` 一样是内置不是 bundle,
     **走 M5-21 已经建好的那条出网层**(`websearch.py` 里的适配盒),别另起一个。
     `extract_depth` 用 `basic`,`format="markdown"`。
  2. **URL 的合法性还是要查一道**,但目的不同了——不再是防 SSRF(没有出站了),
     而是**别把垃圾送出去**:只认 `http` / `https`,长度封顶。
     一句话就够,**不要把那套禁区网段搬过来**——它现在没有守卫的对象,
     留着只会让下一个人以为这里有出站。
  3. **不可信处理和渲染完全复用 M5-21 的出口**:`_on_untrusted()`、围栏、折行、中和、
     来源标注、`url` 也中和。**共用同一个函数,不是照抄一份。**
     两个出口两套规则这一节已经栽过两次(M4-4、M5-5)。
  4. **正文字数封顶**,超了截断并说清楚少了多少
     ——「静默截断读起来和"就这些"一模一样」(M5-5 `load_images` 的 docstring)。
  5. **没配 key** → 和 `web_search` 同一句人话,不崩(E2)。
     网络失败、超时、Tavily 报错、URL 抓不到 → 都回人话。

  ### 抽不出来的时候说什么——两岔,不许合并成一句

  `/extract` 对上面那批壳子页大概率也拿不到东西(**这一点在动手前先验**,见下)。
  返回空或极短时:

  - **不许回「这页没什么内容」**——那是把"我读不到"说成了"它没有",是**编的**,
    而用户会信。
  - 要明说:「这页我读不到(要登录/有反爬/正文要浏览器跑脚本)」。

  ### 兜底那一层:**是「渲染」不是「OCR」,别搞混**

  用户要一条最后手段,对的,该有。但**OCR 不是那条**:OCR 只在「有像素没文字」时才用,
  而页面一旦被渲染出来,手里就是 DOM——直接读文字,再 OCR 一遍是把清晰文本
  转成图片再猜回来,又慢又把链接丢干净。

  **兜底 = 换一个会渲染、且从别的 IP 出来的取法。** 具体就是 `extract_depth="advanced"`
  ——**一个参数,不是一个新系统**(2 credit / 5 个 URL,免费档一个月还能抓 2500 次)。

  什么时候升到 advanced:第一层 basic 抽出来为空或极短(判据和上面那两岔共用同一个门槛),
  **自动升一次,只升一次**,别做成可以反复重试的循环。

  ### OCR 唯一替代不了的那个位置:**正文本身就是图**

  公众号那种整篇一张长图、扫描件、图表——渲染出来也还是图,这时候文本抽取拿到的
  永远是空的,而**这不是"读不到",是"内容不是文字"**。

  这个能力**已经有了**:`look_at_image` + `LARARIUM_VISION`。所以别新写 OCR,
  要做的只是**让第三层能把图交出去**。

  **但先别建这条路。** 现在没有任何数据说这种页面占多大比例,而按 G6:
  建了它谁会读到?什么时候?——先让第二岔("这页我读不到")如实说话,
  **顺便把这一岔说的内容写得能分辨**:是"抓不到"还是"抓到了但正文是图"。
  攒几天真机数据再决定。**这一步只要求把两种情形在回话里分开,不要求处理第二种。**

  ### 实测:哪些页面真的需要兜底(2026-09-09,从真机打的)

  ```
  少数派 文章       200  正文  4,996 字   ★ 直接抓就有
  少数派 文章2      200  正文  2,611 字   ★ 直接抓就有
  阮一峰 博客       200  正文 12,855 字   ★ 直接抓就有
  知乎专栏          403                   真挡(要登录)
  澎湃              403                   真挡(IP 级)
  36氪 首页         200  正文     54 字   壳子,JS 渲染
  ```

  **注意 403 那两个:兜底也救不了。** 403 是在拿到任何东西之前就挡了,
  渲染器要先能打开页面才谈得上渲染;知乎要登录,澎湃是 IP 级封。
  能过这关的是住宅代理 + 真实浏览器指纹 + 过验证码,**那是一整个行业,不做**。
  ——`/extract` 走的是 Tavily 自己的出口,**有可能**过得去,这正是 key 到位后
  要打的那一发要看的东西。

  **一条纪律**:上面这张表是拿**真实 URL** 打的。第一版是拿编的 URL 打的,
  测出「反爬拦死一片」,据此差点得出「让用户自己截图」的结论——**编的 URL 会返回
  200 加一个错误页,长得和反爬一模一样**。以后测抓取,URL 必须是真的。

  ### 动手之前先打这一发

  key 到位后,**先用 curl 打 `/extract`,再决定这一条怎么写**:

  ```
  少数派 / 知乎 / 微信公众号 / 澎湃   ——上面表里"被挡"和"壳子"那几个
  阮一峰博客 / 一个英文文档站        ——对照组,这两个必须能拿到
  ```

  - 壳子页也能拿到正文 → 照上面写,两岔里第二岔很少走到;
  - 壳子页照样拿不到 → **也照上面写**,只是第二岔会常走
    ——这不改变方案,只是让"说实话"那一岔更重要。

  **无论哪种结果,都不要因此回头去写自己抓那一套。** 上面那张实测表已经证明
  自己抓在这批页面上是零,重新捡起来只是把同一个错误再犯一遍。

  ### 验收口径

  - 正常页面 → markdown 进围栏、带来源标注、`url` 被中和;
  - **同一轮里抓完再 `propose(user_stated)` → 必须降档成 pending**
    (M5-18 那把闩在第二个来源上的复验);
  - 抽不出正文 → 回「我读不到」那一句,**断言里要挡住「没什么内容」这类措辞**;
  - 正文超限 → 截断且说清楚少了多少;
  - 没 key / 网络失败 / URL 非 http(s) → 各自一句人话,不抛;
  - **渲染是共用的**:加一条测试,让 `web_search` 和 `web_fetch` 的结果过同一个断言
    ——两边都得在围栏里、都得带来源、都得中和;
  - `grep` 一下确认没有第二份中和实现。

- **M5-27 联网两条工具的参数一次选齐**——先把 Tavily 的全部能力过一遍,再决定用哪些

  **这一条是用户要求的形状**:「你可以先看看 tavily 到底有什么功能 然后我们从中选用
  而不是发现一个又发现一个又发现一个这样子」。M5-21/22 上线后我连着发现了三个该用
  没用的参数,**那是我没先看清全貌**。所以这一条把该选的一次选完,底下附上全部选项
  和**每一条选/不选的理由**,以后再想加就先来这里翻,别再一个一个撞。

  ### Tavily 的全部端点与价钱(2026-09-09 查的,免费 Researcher 档 1000 credits/月)

  ```
  /search    搜索          basic 1 / advanced 2 credit      ← 已用
  /extract   读网页        1 per 5 URL / advanced 2 per 5   ← 已用
  /map       爬整站地图    1 per 10 页
  /crawl     整站遍历+抽取 map + extract 叠加
  /research  多轮搜索+生成带引用的报告  mini 4–110 / pro 15–250,异步轮询
  /usage     查额度        免费
  ```
  ...
  1. **`/extract` 加 `query` + `chunks_per_source`**(`web_fetch` 多一个 `question` 参数)
     ...
     现在盲取开头 4000 字,而开头往往是导航栏。实测阮一峰周刊那篇:

     ```
     不给 query   25,354 字 → 截前 4000,开头是「阮一峰的网络日志 » 首页 » 档案 [![](…」
     给 query     1,992 字,开头直接是答案那几段,短到根本不用截
     ```

     **上下文少一半,内容更对题。** `question` 缺省时行为一个字不变——「帮我总结这篇」
     没有可挑的焦点,挑了反而丢东西;给不给由模型定,工具不替它判断。
     升 advanced 那一次要把 `question` 一起带过去,别升了级反而丢焦点。

     **注意**:挑出来的是**不连续片段**,Tavily 用 `[...]` 连接。渲染要折行,
     折完片段界限就没了,模型会把两段不相干的话读成一句——**保留那个标记别美化**。
  ...
  3. **`/search` 加 `topic` + `time_range`**(`web_search` 多两个可选参数)
     ...
     ```
     默认           AccuWeather 的常年天气页
     topic=news
     + time_range   「台风"科罗旺"外围环流开始影响上海!金山周末天气→」
       =week        「未来一周上海阵雨继续"营业"」
     ```

     **第三行才是「这周末天气」真正的答案**,而用户点名缺的就是资讯这一类。
     这条**要模型判断**(「上海天气」问的是此刻,「Python 装饰器怎么写」不是),
     所以做成可选参数,docstring 写清楚什么时候该给。价钱不变,还是 1 credit。

  ### 明确不做的,以及理由(**别再来问第二遍**)

  - **`/research`**:4–250 credit 一次,免费档十来次就烧光;而且它是**一个 LLM 写的报告**
    ——和 `include_answer` 同一个病:进上下文的是第三方模型的转述,「来源」那一行
    指过去的页面里找不到这些话。我们要的是原话。
  - **`include_answer`**:同上,M5-21 已经判过一次,不重开。
  - **`/crawl` / `/map`**:整站遍历不是个人助手的需求。真要读某一页,`web_fetch` 就够。
  - **`include_raw_content`(搜索时顺带带回整页)**:它把 fetch 合进 search,
    代价是每条结果都拖着一整页——**那正是我们刚封顶治掉的那个问题**。
  - **`/usage`(查额度)**:免费,但按 G6 问一遍——**谁会读到它?什么时候读?
    读到之后能做什么?** 现在没有任何人会读,加了就是又一个有生产者没有消费者的信号
    (M5-17 刚删掉三个)。**等到真撞上额度用完那天再说**,那时候它有消费者了。

  ### 顺带记一个观测,不用治

  `/search` 偶发返回 0 条结果(~20 次里 2 次,同一个查询连打 5 次全正常)。
  **E2 已经兜住了**:回一句「网上没搜到…换个说法再试」,不崩、不编。
  记在这里是为了下次看到"她说没搜到"时知道有这种可能,别去查一个不存在的 bug。
...
# M5 补做:第三方 review 打出来的四条 P1 + 话头

> 2026-09-09,一份只读的第三方 review(基于 `9e2b603`)。**四条 P1 我逐条独立复验,
> 四条全部坐实,一条都不用打折。** 这些代码我自己验收过,一条都没找出来
> ——记在这里,是因为「复核方也会漏,而漏的那些正好是跨模块的接缝」这件事
> 比这四个 bug 本身更值得记住。四条全在**衔接处**:门控×检索、重试×起居注、
> 压缩×前置步骤、并发×读改写。单看每个模块都对。

- **M5-28 [P1] 不可信标记过不了起居注,门控被静默绕过**

  **真机数据坐实**:

  ```
  envelope       payload 里有 meta:[True, True, True]
  tool_result    payload 里有 meta:[False, False, False]
  reply          payload 里有 meta:[False, False, False]
  ```

  `Journal.search` 读的是 `json_extract(payload, '$.meta.untrusted')`,
  而**只有信封带这个字段**。于是:

  ```
  第 N 轮    web_search 捞回攻击者的内容 → 落成一条没有 meta 的 tool_result
  第 N+5 轮  模型 search_history 命中它 → hit.untrusted 是 False → 闩不拉
             → propose(user_stated) 自动放行 → 进长期档案
  ```

  **M5-18 修的是轮内的闩,轮间靠起居注传递这条路从来没建过。** 提示词那层可能还挡得住,
  但**程序保障没了**,而门控的全部价值就在于它不依赖模型自觉。

  **修法方向**:让来源属性贯穿存储与检索——`tool_result` / `reply` 落起居注时带上
  当轮的 untrusted。**注意老记录没有这个字段**,补列/默认值怎么定要想清楚:
  默认成 False 是"老数据一律可信",默认成 True 是"老数据一律要审"。
  **在回报里说清楚你选了哪个、为什么。**

- **M5-29 [P1] 连续失败时第三次尝试会重复记账**

  `last_attempt_tool_results` 的 `start` 取的是**最后一个** `envelope` 事件之后:

  ```
  第 1 次  envelope → record_expense 执行了(tool_executed)→ 模型调用失败
  第 2 次  envelope → 还没调到工具就失败(这一段没有 tool_executed)
  第 3 次  回放队列只看第 2 次那一段 → 空 → 模型重新记一遍
  ```

  **第 1 次那笔账被第 2 次的空记录挡住了。** 而 M5-13 已经证过这个服务商会失败,
  一次连不上就能造出"第 2 次没调到工具"。**这条是钱,而且账上多一笔没人会发现。**

  **修法方向**:同一信封**累计**已确立的执行结果,不要依赖"每次重试恰好把旧结果
  重新回放一遍"。注意别把幂等做反——同一个 `tool_executed` 回放两次也是错的。

- **M5-30 [P1] 压缩的前置步骤失败了,历史照样退出 L0**

  `_cut` 模型调用失败时 `return []`(还落了一条起居注),而 `run()` 不看这个:

  ```python
  segments = await self._cut(...)      # 失败 → []
  ...
  for seg in segments:                 # 空循环,一条索引都没建
      self._journal.add_index(...)
  self._journal.mark_compressed(ids)   # ← 无条件执行
  ```

  结果是**索引一条没建,那批对话已经退出 L0**。正文没删,但自动可见性没了
  ——而"能翻到"和"会被翻到"是两回事。`self._sweeper.run()` 同理:它失败也是
  以结果返回,不抛。

  **修法方向**:明确区分成功 / 失败 / 未完成,再决定提不提交压缩标记。
  **别顺手改成"失败就抛"**——那会让一次模型抖动变成整轮崩掉,而现在这个吞法
  至少还留着起居注。

- **M5-31 [P1] 同一笔流水的并发修改会互相覆盖**

  `amend_expense` 先 `SELECT ... fetchone()` 读整行,再 `UPDATE` 把**四个字段一起写**,
  而值全来自那次读,**读和写之间没有事务**。M5-8 早就确认过:一条 assistant 消息里的
  多个工具调用是**并发**的。两个 amend 打同一行 → 都读到旧值 → 后写的把先写的盖回去
  → **两边都报成功**。

  M5-26 把「插新行 + 作废旧行」改成一条 UPDATE,窗口变小了但没关上。
  **我 M5-26 验收时只打了迁移和就地语义,没打并发。**

  **修法方向**:要么把整个读-改-写包进事务,要么**只原子更新用户指定的那几个字段**
  (后者更小,而且顺带治掉"没指定的字段被旧值盖回去"这个根)。
  **加一条并发探针测试**,形状照 M5-8 那条(锁里数人头 + `Barrier` 卡齐 + 阳性对照),
  别照着崩溃写——那是靠运气变绿的测试。

- **M5-32 话头承不起"长期事务",而且沉下去就上不来**

  用户问:「你确定模型真的会在没有提示的情况下自己开关话题吗?还有只能显示 5 个话题,
  那没有提起一段时间但是重要的话题不是永远被埋着了吗」。**两问都问中了,真机数据:**

  ```
  两天 31 轮对话,模型自己调的工具:
    10  record_expense      3  amend_expense      2  list_recent
     1  open_thread     ← 开了一次
     0  close_thread    ← 一次都没关过
  ```

  **第二问更要紧,那是个闭环**:`MAX_OPEN = 5` 按 `updated_at` 倒序;
  而一个话头要回到可见列表,唯一的办法是被更新;要被更新,得先被看见。
  **沉下去的再也上不来**——这不是概率问题,是结构性的。

  第三方 review 的话:「不分 session 可以是用户体验,内部仍然需要清楚的事务边界」。
  「下周交材料」「等某人回复」「周五提醒我确认」这些事,最终需要**可核对的状态、
  时间和完成条件**,不能只靠模型记得聊过。

  ### 这一条**先不要动手**

  它不是一个 bug,是一次设计选择该不该重来。动手之前要先答三个问题,
  而现在一个都没有数据:

  1. **模型不开话头,是它不会,还是我们没给它理由?** `open_thread` 的 docstring
     说了什么、前缀里话头那一层长什么样——先看它被告知了什么,再说它做得对不对。
     (**别急着加提示词逼它开**——M5-11 就是这么来的。)
  2. **五条这个上限是不是真的在承重?** 现在真机上一共只有 1 条话头。
     上限从没被触发过,而我们在讨论它的失效模式。
  3. **"可核对的状态、时间、完成条件"该长什么样?** 一个到期时间?一个完成判据?
     这是加一个字段还是加一个领域 bundle,差一个数量级。

  **先攒数据**:M5-28~31 修完、真机跑一段时间,看话头到底被开了几条、有没有真沉下去过。
  按 G6:先问这个「坏了」该不该在。

- **M5-33 让话头真的被用起来:告诉她什么时候开关,再给她一条"看"的路**

  M5-32 查清楚了两件事,这一步分别对着它们:

  ```
  两天 31 轮:open_thread 1 次、close_thread 0 次
  ```

  **① 不是模型不会,是我们从来没说过。** `prompts/discipline.md` 里提到话头只有一次,
  而且是反着说的——「派生结论、会变的东西不入档,**归话头**」,那是在说"什么不该进档案",
  不是"什么时候该开话头"。全项目找不到一句「有件事没聊完就开个话头」。
  对比 `record_expense` 被调 10 次,是因为**触发词在用户嘴里**(「记一下 45.5」);
  而没人会说「开个话头」。

  **② 埋掉那个闭环,是因为她没有"看"的路。** 工具清单里没有 `list_threads`,
  她能看到的只有信封里那 5 条(`MAX_OPEN=5`,按 `updated_at` 倒序)。
  第 6 条不在视野 → 她碰不到 → `updated_at` 不动 → 永远第 6。
  finance 那边 `list_recent` 存在的理由一模一样:**让模型找回一个它手里没有的东西。**

  ### 一、`list_threads`(带分页)

  **照 `search_history` 那套分页,别另写一份**:`_paged_search` + `_format_search_result`
  的形状已经在那儿了——页码钳到 `[1, 总页数]`(0 / 负数 / 超大都不报错),
  **总数要给出来**(它是给模型的信号:3 条 = 就这些,40 条 = 该关一批了)。

  - 默认只列**开着的**;关掉的另给参数才带上,并标出来
    (同 `list_recent` 的 `include_deleted`——**一个开关一件事,别复用**)。
  - **不受 `MAX_OPEN=5` 限制**,那个上限管的是每轮信封里塞几条,和"我要查一下"是两件事。
  - **note 要不要在这里截到 80 字?** `MAX_NOTE_LEN` 是**入库时**就截的
    (`open_thread` 里 `note.strip()[:MAX_NOTE_LEN]`),所以库里本来就不会更长
    ——**别再截一次**,两处截断迟早漂成两个数。这一条去核一眼再动手。
  - 单页条数封顶,理由同 `list_recent`:不封顶一次调用就能撑爆 L0。

  **前缀代价(A1)**:多一个工具 = schema 变 = 前缀重建**一次**,`prefix_log` 会记。
  **追加在末尾**(`web_fetch` 之后),不许插队。

  ### 二、`discipline.md` 加一句:什么时候开、什么时候关

  **这是说服不是机制,一行提示词,不行随时撤。** 和 M5-11 的区别要拎清:
  M5-11 是基于误诊(n=1)加了一道**会拦工具调用的闸**;这次诊断是硬的
  (grep 出来那句话根本不存在),加的是一句话。

  写的时候两头都要给,**只说"开"会攒出一堆永远不关的话头**:

  - 什么时候**开**:用户提到一件还没完的事(在等谁、在办什么、下周要交什么)。
  - 什么时候**关**:那件事有结论了。**关掉不是删掉**,是"不用再惦记"。
  - 同名再开 = 更新那句状态,不是另开一个(这条工具 docstring 里有,纪律里不用重复)。

  **别写成"每轮都检查一遍话头"** ——那是在给每一轮加一份负担,而 M5-11 的教训正是
  "一条规则写进纪律,就是每一轮都要为它付钱"。

  ### 三、明确**不做**的

  - **不加到期时间 / 完成判据 / 提醒**。third-party review 说话头承不起"长期事务",
    说得对,但**现在真机上一共 1 条话头**——在给一个还没被用起来的东西设计任务系统。
    先让它被用起来。
  - **不动 `MAX_OPEN=5`**。它从没被触发过(总共 1 条),而我们在讨论它的失效模式。
    `list_threads` 一上,闭环就开了;上限该不该动,等有数据再说。
  - **不加提示词逼模型每轮开话头**。见上,M5-11。

  ### 验收口径

  - `list_threads` 页码 0 / 负数 / 超大 → 都钳住,不报错;总数出现在输出里;
  - 默认不列关掉的;带参数才列且**标得出来**;
  - **note 没有被截第二次**(拿一条正好 80 字的 note 钉住,入库和列出来一样长);
  - 单页封顶生效;
  - **前缀指纹变一次**,工具顺序里 `list_threads` 在末尾(拿现有的前缀稳定性测试钉);
  - `discipline.md` 改动后**前缀指纹跟着变一次**,这是预期的,记进 `REVIEW.md`;
  - **真机冒烟**(验收方打):开三条话头 → `list_threads` 三条都在 → 关一条 →
    默认列表剩两条、带参数还看得见那条。另外**记一条基线数字**:改完之后跑几天,
    `open_thread` / `close_thread` 各被调了几次——**这一步值不值只有那个数说了算**。

- **M5-34 L0 那条路上的不可信继承**——M5-28 修的是检索,这是第二个入口

  同一条外部内容有两条进上下文的路,M5-28 只补了检索那条:

  ```
  检索路  第 N+5 轮 search_history 命中 → 判据顺着信封推导(M5-28 补上了)
  L0 路   第 N 轮的 tool_result 以协议原生形状回放进第 N+1 轮
          → L0 的"外部数据"包裹只看信封 meta
          → 模型读它一次工具都不用调,判据根本没机会被问到
  ```

  **这条是 M5-18 第五节就记下的形状**,两轮都没补,而两轮的理由是同一个,**成立**:
  最直白的扩法「L0 里有过脏轮就算」会让**一条短信之后系统永久不可信**
  ——那不是更安全,那是把门控变成一个所有人都会想办法绕过的东西。

  **所以这一条的题目不是"补上",是"想清楚判据"**,而那需要先回答:

  1. 脏应该**传染多远**?一轮?到那条内容退出 L0 为止?还是到用户下一次开口?
  2. **谁来解除**?现在没有任何解除路径,而只升不降在轮内是对的(M5-18),
     跨轮永久不降就是另一回事。
  3. 短信入账(M6)会让这条路**每天都走**,而现在它一天走不了几次。
     **在那之前必须有答案**——第三方 review 也点了这一条。
