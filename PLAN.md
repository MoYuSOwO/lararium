...
- **M5-22 `web_fetch`(取一个网页,转成 markdown)**——SSRF 是主体,不是附带

  用户明确要这条能力。我原来主张先只做 `web_search` 看缺不缺,用户否了,**按用户的决定做**。
  下面这一节我查过资料才写,不是凭印象——参考的是 2026 年那批 SSRF 绕过的修法
  (CrewAI 1.15.17 / CVE-2026-55391 都是同一个洞、同一种修法)。

  ### 先说一个要你判的选项:我替你判了,不用再选

  Tavily 有 `/extract`,由**它**去抓页面,我们永远不发起模型可控的出站请求,
  **SSRF 面直接归零**。我不选它,理由两条:一是每次抓都烧 Tavily 额度
  (和 `web_search` 共用 1000 次/月),二是用户想看什么页面全都过一遍第三方。
  **自己抓,把防护写对。** 但这意味着下面每一条都是硬要求,不是建议。
  ...
  ```
  127.0.0.1:13096  1panel-core     ← 服务器管理面板,SSRF 的头号目标
  127.0.0.1:8420   lararium 自己
  0.0.0.0:8000     docker-proxy
  0.0.0.0:80/443   openresty
  ```

  (云元数据 `169.254.169.254` 在这台机器上不响应,不是那类云主机——但**照样要挡**,
  换一台机器就有了,而这条防线不该跟着部署环境变。)

  **杀伤链是完整的**:搜索结果可被 SEO 投毒 → 模型抓了攻击者的 URL →
  跳转到 `127.0.0.1:13096` → 面板内容进上下文 → 下一次 `web_search` 的查询词
  把它带出去。所以这条和 M5-21 是**同一条链上的两环**,防护不能只做一半。

  ### 防护:五步,一步都不能省

  **关键认知:检查「解析出来的 IP」,不检查「URL 里的字符串」。** 十进制 IP
  (`http://2130706433/`)、八进制、十六进制、`0`、IPv4 映射的 IPv6
  (`::ffff:127.0.0.1`)——这些编码花招在**解析之后**全都归一了,所以只要检查点在
  解析之后,就一次挡掉一整类,不用去枚举写法。

  1. **协议白名单**:只允许 `http` / `https`。`file://` `ftp://` `gopher://` 一律拒。
     **端口白名单**:只允许 80 / 443。这一条单独就挡住了上面那四个内部服务。
  2. **解析一次**,`socket.getaddrinfo`。**返回的每一个地址都要过检查,不是只看第一个**
     ——一个域名解析出 `[公网IP, 127.0.0.1]`,只验第一个就等于没验。
     任一地址落在禁区,整个请求拒掉。
  3. **连到那个验过的 IP,不要把主机名交给 httpx 重新解析一遍**。这是 DNS rebinding
     的窗口:验的时候是公网 IP,连的时候变成内网 IP,中间只隔一条 TTL=0 的 DNS 记录。
     httpx 的做法是 URL 里写 IP 字面量,`Host` 头写原主机名,TLS SNI 用
     `extensions={"sni_hostname": 原主机名}`(httpcore 的连接层支持这个扩展,我确认过)。
  4. **连上之后再用 `getpeername()` 复查一次**对端地址。第 2 步和第 3 步之间还有一条
     缝,这一步把它焊死。复查不过就关掉 socket。
  5. **重定向自己走,不交给 httpx**:`follow_redirects=False`,拿到 3xx 自己读
     `Location`,**对每一跳从第 1 步重新走一遍全部检查**。跳数封顶(建议 3 跳)。
     2026 年那批 CVE 里最常见的绕过就是"只验第一个 URL,跳转之后不管了"。

  **禁区清单**(IPv4 + IPv6 都要,`ipaddress` 模块自带判断,别自己写网段表):
  loopback、private、link-local(`169.254.0.0/16` 云元数据在这里)、
  unique-local(`fc00::/7`)、multicast、reserved、unspecified(`0.0.0.0` / `::`)。
  `ipaddress.ip_address(x)` 的 `is_private` / `is_loopback` / `is_link_local` /
  `is_reserved` / `is_multicast` / `is_unspecified` 合起来就够,**逐个属性写清楚**,
  别图省事只写 `is_private`(它不含 link-local,而云元数据正在那儿)。

  **代理必须关掉**:`trust_env=False`。环境里一个 `HTTP_PROXY` 就能让"对端"变成代理,
  上面五步全部作废。

  ### 封顶(三个都要,理由和 `web_search` 一样)

  - **响应体大小**:边下边数,超了当场断开——不是下完再判(下完就已经在内存里了)。
    2C2G 的机器,建议 2 MB。
  - **超时**:连接 + 读取分别封顶。
  - **抽出来的正文字数**:进上下文的那份要封顶,超了截断并说清楚少了多少
    ——「静默截断读起来和"就这些"一模一样」(M5-5 的 `load_images` docstring 里那句)。

  ### 正文抽取:用 trafilatura,但**别让它自己去抓**

  `trafilatura` 2.2.0 直接输出 markdown(它内部拿 readability 和 jusText 兜底,
  所以一个依赖顶三个)。基础依赖 7 个,含 lxml(有 wheel,2C2G 装得动)。

  **致命细节**:trafilatura 自带 `fetch_url`,它用自己的 urllib3 出网。
  **一旦调了它,上面五步防护全部绕过。** 只准把**我们自己下载好的 HTML 字符串**
  喂给 `trafilatura.extract(html, output_format="markdown")`。
  这条要写成注释钉在调用点上——下一个人看到"它自己能抓"是会顺手改的。

  ### 抽不出来的时候说什么——三岔,不许合并成一句

  **这一条是实测逼出来的。** 我拿七类页面量过留存(正文对正文,不是对整页可见文字):

  ```
  阮一峰长文(中文)  12,631 → 12,927  102%     MDN 正文  2,290 → 2,433  106%
  PEP 8(长文档)    38,587 → 43,544  112%     uv 文档   5,944 → 5,245   88%
  ```

  **正文基本不丢**(超 100% 是表格和链接被补成了 markdown 语法)。但另外两类会翻车:

  ```
  知乎专栏  下载到的 HTML 里可见文字只有 33 字
  少数派    只有 13 字
  ```

  **这不是抽取洗过头,是页面要浏览器跑完 JS 正文才出来**,抓一百次也这样。
  而 `trafilatura.extract` 这时返回 `None` 或者一小截——**和"这页真的没什么内容"
  在返回值上一模一样**,模型会照着空结果回一句「这页没什么内容」,那是**编的**。

  所以出口分三岔,**每一岔说的话必须不一样**:

  1. **抽出来了** → 正常返回 markdown。
  2. **抽取为空/极短,但页面里的可见文字(去掉 script/style)是够多的**
     → 说明是抽取器没认出正文结构,**降级用可见文字**,并**在回话里说明这是没整理过的原文**。
     洗过头要降级成"没洗",不许降级成"空的"。
  3. **抽取为空,可见文字也没几个字** → 明确回「这页的正文要浏览器跑脚本才出得来,
     我这边取到的是空壳,读不到内容」。**不许回「这页没什么内容」**——
     那是把"我读不到"说成了"它没有",是这一层最容易长出来的一句谎。

  第 2、3 岔的分界拿可见文字字数定(建议 200 字),**门槛写成模块常量并写明依据**。

  **验收**:三岔各一条测试,用**存下来的真实 HTML 做样本**(一个正常文章页、
  一个抽取器认不出但有正文的页、一个 JS 壳子页),不要用手搓的假 HTML
  ——手搓的 HTML 抽取器一认一个准,那样三岔里有两岔永远走不到。

  ### 不可信处理:和 `web_search` **共用同一个出口**

  渲染、中和、来源标注、`_on_untrusted()` 全部和 M5-21 一致,**共用一份实现**。
  两个出口两套规则这一节已经栽过两次(M4-4、M5-5)。
  `url` 同样要中和(它是攻击者可控的文本,且会出现在围栏附近,P1-4)。

  ### 验收口径

  - **绕过用例表驱动**,每一条都要有:`http://127.0.0.1:13096`、`http://localhost:8420`、
    `http://2130706433/`(十进制)、`http://0177.0.0.1/`(八进制)、`http://[::1]/`、
    `http://[::ffff:127.0.0.1]/`、`http://169.254.169.254/`、`file:///etc/passwd`、
    `http://example.com:22/`(端口不在白名单)、`http://0.0.0.0/`
    ——**全部拒绝,且回的是人话不是异常**;
  - **DNS rebinding**:把 `getaddrinfo` 打桩成"第一次返回公网 IP、第二次返回 127.0.0.1",
    请求必须失败。**这条是这一步的命根子**——它是唯一能证明"IP 真的被钉住了"的测试,
    没有它,一个"验完就把主机名交给 httpx"的实现照样全绿;
  - **多地址**:`getaddrinfo` 返回 `[公网IP, 127.0.0.1]` → 拒绝(只验第一个的实现会放过);
  - **重定向**:公网 URL → 302 到 `127.0.0.1` → 拒绝;跳数超限 → 拒绝;
  - **`getpeername` 复查**:打桩成"连上之后对端是内网" → 断开且拒绝;
  - **封顶**:响应体超限当场断(用一个能无限吐字节的假服务器验"边下边断",
    不是下完再判)、超时、正文截断说清楚少了多少;
  - **不可信闩**:抓一次 → `untrusted_seen` 落一条;**同一轮里抓完再
    `propose(user_stated)` → 必须降档成 pending**;
  - **trafilatura 没被用来抓**:门禁里加一条机械检查,`fetch_url` / `fetch_response`
    这些名字在 `src` 里零命中;
  - 网络失败、超时、DNS 失败、非 HTML 内容 → 一句人话,不抛(E2)。

  **真机冒烟留给验收方**,你在 `REVIEW.md` 里写清楚该打哪几条、看什么。
  **本机自测时不要真去连 `127.0.0.1:13096` 那类地址**——用打桩,别在自己机器上开洞。

  **前缀影响(A1)**:多一个工具 = 工具 schema 变,前缀第 0 层重建一次。
