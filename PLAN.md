...
M6-3a 退款并进收入      用户:很难说全退,那个指针指不准,派生的净额比没有更坏        ⏳ 进行中
M6-4  预算提醒         和 M6-3 同一个 bundle,接着做                                      ⬜ 等 M6-3a
M6-5  做菜 bundle      用户自己设计的
...
- **M6-5 做菜 bundle**——**用户重新设计过:它是文档,不是表**
  ...
  **别在这一步发明新守卫**(M5-11)。
  ...
  - 同名再 `write` = 更新不是新增;名字首尾空白/内部多空格 → 和原名对上同一道菜;
  - `append_to_recipe` 三次 → 三段都在,顺序不变,**原有内容一字没动**;
  - `list_recipes` 页码 0/负数/超大都钳住,总数出现在输出里,`category` 能筛;
  - **非法名字那一组**(见上)分别当 `category` 和 `name` 传 → 全部拒绝且回人话;
    注释掉白名单那一道,落点兜底仍然挡住,反之亦然;
  - **工具签名里没有任何路径参数**——加一条机械检查,`category` / `name` 之外
    不许出现 path/dir/file 这类参数(它是这一条的设计约束,不是风格偏好);
  - `read_recipe` 不给 `category`:唯一 → 读到;**两个类里同名 → 拒绝且说出是哪几类**;
  - `list_categories` → 只有类名,不含任何菜;`list_recipes` 分页钳位 + 总数;
  - `replace_in_recipe`:`old` 出现一次 → 改;**零次 → 拒绝**;**多次 → 拒绝且说出现几次**;
    改完回话里带得出改动前后那一小段;**绝不许"改第一处"**(拿一份 `old` 出现三次的
    做法钉死它);
  - 不存在的菜名、空名字、超长名字 → 各自一句人话,不抛;
  - **前缀影响(A1)**:新 bundle = 目录行 + 4 个工具 schema,前缀重建一次。
