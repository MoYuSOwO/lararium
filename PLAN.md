...
```
M6-1  语音            缺口就是 ASR;但先打一发真包,看微信有没有把文字送过来
...
- **M6-1 语音听不懂**——**缺口就是 ASR**;但先确认微信有没有把文字送过来

  用户:「你的 asr 也没加 语音识别 我发语音她好像还搞不定」。

  ### 标题改过一次,而原来那个是错的

  复核方原来写的是「**不是缺 ASR**,是微信转好的文字我们没读」。用户当场否了:
  「什么意思?什么叫不是缺 asr,**就是缺 asr 啊**」。**他对,两处都对:**

  1. **缺口确实是 ASR**——她听不懂语音,这就是缺口。复核方想说的是"不用**自己建**
     一套",但写成了"不是缺 ASR",**那是在否认一个存在的缺口**;
  2. 更要紧的是:**万一那个字段实际是空的,这个标题已经先把 ASR 那扇门关上了。**

  ### ★ 而 `voice_item.text` 这件事**我没验过**,只是读了源码

  下面那段官方源码证明的是"**这个字段存在、而且官方实现在用它**",
  **不证明微信的服务器会给这个账号填它**。

  **这和今天早上那次一模一样**:复核方信了 Tavily 文档说 `/extract` 没有 `title` 字段,
  打了真包才发现有——而「**文档也是二手的,唯一的一手是真包**」这条纪律
  是复核方自己那天写下来的,转头又犯。

  **所以这一条的第一步不是写代码,是打一发真的:**

  ```
  ① 让用户发一条语音
  ② 把那条 getupdates 的**原始报文**整份 dump 出来(先落到日志或文件)
  ③ 看 item_list 里那一项:type 是不是 3?voice_item 里有没有 text?text 是空的吗?
  ```

  **三种结果,三条路,动手前必须先知道是哪一种:**

  ```
  text 有内容      → 取文字那一支加上语音;**音频照样存**(见下面第 2 条)
                     并顺手量一下转写质量(见"顺带量一个数"那节)
  text 是空的      → **ASR 回到桌面上**,而那是另一条任务(见本条末尾)
  压根没有 type=3  → ClawBot 不转发语音,那连 ASR 都救不了(只能让用户打字)
  ```
  ...
  **微信自己做了语音转文字,放在 `voice_item.text`。** 而我们取文字那一支只认 `TEXT`:
  ...
  2. **音频一律存下来,哪怕 `text` 有内容**——**复核方原来写反了,用户的怀疑推翻了它。**

     原来照官方那个 `&& !i.voice_item?.text` 写的是"有 text 就不下载",
     理由是"存的那份没人读(G6)"。**而用户问:「微信的 asr 如何?主要是感觉质量
     不太好……」——这个怀疑成立的话,那份音频就是唯一能补救的东西,而我们把它扔了。**

     微信 ASR 的已知弱项**正好是这个用户的用词**:中英混、技术名词、数字。
     起居注里那些 `check_same_thread`、`GPT Pro 5x`、`iRacing`、`Tifosi`
     ——手机 ASR 基本都会糊。

     **所以:转写当文字用,音频照样落盘。** 代价极小(一段语音几 KB 的 SILK),
     换来的是"转写不行的时候还有救"。**这不违反 G6**:消费者是明确的
     ——万一要上自己的 ASR,它就是输入;而这个"万一"现在有具体的怀疑撑着,不是假想。
  ...
  用户:「**微信的 asr 如何?主要是感觉质量不太好……**」——**现在没有任何数据**。
  这一发真包顺手就能量:

  ```
  让用户发 3~5 条语音,内容故意包含他平时真会说的东西:
    ① 纯中文一句      「明天下午两点有课」
    ② 中英混          「GPT Pro 那个 5x 的差价记一下」
    ③ 带数字和金额    「麦当劳四十五块五」
    ④ 技术名词        「check same thread 那个参数」
  然后逐条对:微信转出来的 vs 他实际说的。
  ```

  **微信自己那个转写的准确率,网上搜不到可信数据**(搜出来全是第三方转写 app 的软文)——所以**只能自己量**,没有别的办法。

  **这个数决定后面走哪条路**:转写够用 → 就这样;糊得厉害 → 自己上 ASR
  (那时音频已经存着了,见上)。**而在有这个数之前,两边都不要下结论。**

  ### 验收口径

  - 造一条 `type=3` + `voice_item.text="麦当劳 45.5"` 的报文 → `content` 里有这句话,
    **而且音频也落了盘**(`attachments` 里有那一条);
  - 造一条 `type=3` 有 `media` 无 `text` → 音频落盘,占位文本**说得出"没有文字"**;
  - 造一条 `type=3` 既无 `media` 也无 `text` → **打一条 warning**,不静默;
  - 文本 + 语音混在一条消息里 → 两段都在 `content` 里,**顺序和 `item_list` 一致**;
  - 现有图片那条路逐字不变(拿现有测试当金样)。

  ### 真机这一发是**第一步**,不是最后一步(验收方打不了,要用户配合)

  **让用户发一条语音**,然后看:`content` 里有没有转出来的文字、
  `data/media/` 有没有多一个文件(不该多)、日志里有没有 warning。
  **这一条只有用户能打**,写进 REVIEW 时注明。

  ### 如果真机打出来 `text` 是空的:**ASR 回到桌面上,而那是另一条**
  ...
  - **模型那条路走不走得通**:`deepseek-flash` 只吃 webp/png/jpeg/gif
    (M6-6 那三条实测),**所以音频肯定进不去模型**——这条不用再试了;
  - **频率**:先攒数据。一天两条语音和一天二十条,值不值完全不同。

  **而无论有没有 ASR,这一条都必须做对一件事**:没有文字的语音,
  她要**说实话**「这条我听不了,你打字说一下」——**不许对着一行占位符编内容**
  (M5-19 之后那次就是这么犯的)。
