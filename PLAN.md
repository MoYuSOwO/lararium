...
- **M6-1 做菜 bundle**——用户自己点的第一顺位,「这个可能还更频繁用到一点」

  记各种菜的做法,和每次做的尝试。第三个领域 bundle,**契约到这一步才算被反复验过**
  (原计划靠健康/学习来验,换成这个——同样的收益,而这个会真被用)。

  ### 为什么是 bundle,不是让她写进档案

  **决定性的理由是前缀**:档案是前缀第 2 层、逐字节稳定、**每一轮都在**。
  一份做法几百字,写进档案就是从此每轮都付它的钱,而你一个月做一次。
  做法属于"要用的时候去查"的东西,和流水同形。

  ### 存什么——**查过标准之后,故意不照标准做**

  查了 schema.org/Recipe 和几份 cooking app 的库设计。它们的字段是:
  `recipeIngredient`(带数量/单位)、`recipeInstructions`(有序步骤)、
  `cookTime` / `prepTime` / `recipeYield` / `recipeCuisine` / `nutrition` / `image`,
  食材还要规范化成独立表 + 单位换算表。

  **那套是为「发布菜谱」设计的**(给搜索引擎、给分享、算营养、生成购物清单)。
  用户要的是「做法**和尝试**」——而**「尝试」在任何标准 schema 里都不存在**,
  因为标准描述的是一份**做好的**菜谱,不是一个人反复试的过程。

  所以按 G6 逐项问「谁会读到它」:

  ```
  食材规范化 + 单位换算   没人算购物清单、没人算营养 → 不做,食材就是一段自由文本
  cookTime / prepTime     没有任何证据要它 → 不做
  recipeYield / cuisine   同上 → 不做
  nutrition / image       同上(图片:真要看图,发图 + look_at_image 已经有了)
  有序步骤                **要**,但 schema.org 自己说"用换行分隔的单个字符串"就行
  尝试记录                标准里没有,而这是用户点名要的 → **这一步的重点**
  ```

  两张表,形状照 `expenses`:

  - **做法**:菜名(归一化后当钥匙,同名再存 = 更新)、食材(自由文本)、步骤、备注。
  - **尝试**:哪道菜、哪天做的、结果怎样、下次改什么。**一道菜多次尝试**,
    所以是独立的行 + 外键,不是往做法里塞一个数组。

  ### ★ 一个真问题:步骤的换行是结构,而我们的渲染器会把换行折掉

  这是查标准时撞出来的,**任务书原来那一版没有**。`neutralize_model_text` /
  `_render_hit` 那一套**折行**,理由是攻击者能凭换行伪造列表项(M4-4、M5-21 各栽一次)。
  可是**做法的换行是有意义的**:折完「1. 水开下面 2. 打蛋 3. 撒紫菜」变成一坨。
  这和 M5-27 那个 `[...]` 片段界限被折掉是同一个形状。

  **要在回报里给出方案并说明取舍**,我不替你定。三条路:

  - 步骤在渲染时换成行内编号(`① … ② …`),换行本身不留——**结构保住了,注入面没扩**;
  - 给步骤单独一条渲染路径,允许换行但别的都中和——**多一个出口,而两个出口两套规则
    这一节栽过两次**;
  - 存的时候就要求一行一步、渲染时自己编号。

  **不许"就折掉算了"**——那等于这个 bundle 的主要内容不可读。

  ### 工具(顺序即冻结顺序)

  `save_recipe` / `list_recipes` / `read_recipe` / `record_attempt`。
  **先只做这四个**,`amend_*` / `delete_*` 等真撞上再加——finance 那两个是被真机逼出来的
  (M5-15、M5-20),不是设计出来的。

  ### 三条必须守的

  1. **`list_recipes` 硬封顶 + 分页**,理由同 `list_recent`:不封顶一次调用撑爆 L0。
     照 `_paged_search` 那套,别另写一份。
  2. **步骤和备注走同一个渲染器**(折行 + 中和),而且**只此一份**——finance 那边
     `record_expense` 和 `list_recent` 各写一套渲染器的账已经还过一次(M4-4)。
  3. **做法可能来自网页**(用户让她 `web_fetch` 一个菜谱然后存下来)。那一轮是脏的,
     而 `save_recipe` 写的是 bundle 自己的库、不经过门控——**这不是漏洞
     (门控管的是长期档案),但要在 docstring 里写明白:存进来的做法是用户让存的,
     不是"经过核实的"。** 别在这一步发明新守卫(M5-11)。

  ### 验收口径

  - 存一份 → 读回来逐字一致(步骤里的换行、`|`、围栏字符都不许吃掉);
  - 同名再存 = 更新不是新增;
  - 一道菜记三次尝试 → 三条都在、按时间正序、`read_recipe` 带得出来;
...
- **M6-6 账本只有支出,没有收入**——用户问出来的,而账上**已经卡着 600 元**
  ...
  **新一张表,不是往 `expenses` 加符号位。** 理由:
  ...
  600 元挂在话头里三天了,而做菜 bundle 是还没发生的需求。**已经卡住的先修。**
