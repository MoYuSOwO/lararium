...
- **M5-1 渠道定型**:实测限频与可用性(QQ 群 / 飞书 / TG 三选一)。这一步的产出是
  一份实测记录 + 一个决定,不是代码。**需要你先申请机器人账号。**
- **M5-2 用户主渠道路由**:配置 `LARARIUM_USER_CHANNEL`;通知(`make_daily_notifier`)
  与**数据面那一轮的回复**都投它。收掉第四轮审计那条(数据面回复投回来源渠道 = 没人能读)。
