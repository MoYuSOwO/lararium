...
- **M5-11 查 `test_live_finance_skill` 为什么红**:记账前该先读 finance 总览,现在
  **两个模型都不读**(DeepSeek 与 mimo 各跑一次都 FAILED,只调了 `record_expense`)。
  所以不是换模型带来的,是这条已经红了。M4 就记过「read_skill 的 reach 低」,又退了一步。
  先查清是模型行为退化,还是 skill 注册/描述被改动过。
