...
- **M6-3 结构化的事务 / 待办**——用户点的;**这是话头承不起的那一半(M5-32)**
  ...
  **不要直接交实现。** 先把这三件写出来给复核方看:
  ...
  不做提醒(那是 M6-4)、不做优先级、不做子任务、不做标签。**先让它能存能查能勾掉。**

- **M6-4 调度器(从 M7-5 提前)**——M6-2 和 M6-3 的"主动"那一半都靠它

  原来排在 M7-5(「到这一步才有常驻可依赖」),但真机已经常驻 6 天了,那个前提已经成立。
  第三方 review 也点了同一件事:「我会把持续运行和一条主动事务闭环,
  排在大规模扩领域之前」。

  - 夜间归拢自动跑(现在还是 `/sweep` 手动——**而手动这件事已经吃过一次亏**:
    M5-24 那个缺口就是因为没人每天跑);
  - 待办到期提醒、预算的定时那一半;
  - **必须能取消**:review 的原话是「取消后不再提醒、没有进展时不打扰」。
    **一个提醒不掉的提醒比没有提醒更糟。**

  ### 一条硬前提

  **M5-34(L0 那条路的不可信继承)必须在这之前有答案。** 主动推送会让系统
  在用户没开口的时候自己产生轮次,而那条路上的判据现在还没写。
