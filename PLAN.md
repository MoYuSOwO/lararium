  ...
  - **前缀影响(A1)**:新 bundle = 目录行 + 7 个工具 schema,前缀重建一次。
...
- **M6-4 做菜 bundle**——**用户重新设计过:它是文档,不是表**
  ...
  **三次都是同一个毛病**:我先想"怎么把它做对",而不是先问"用户会怎么用它"。
  ...
  - **不加 `delete_recipe`**:用户自己删那个 `.md` 就行。
...
- **M6-5 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**
  ...
  **不加 `delete_course`。** 退课、建错了想清掉,**用户自己删那个目录就行**
  ——那正是选文件的理由之一;而给模型一个能删掉一学期笔记的工具,
  收益远小于风险。真撞上"我想让她帮我清掉"再说。
