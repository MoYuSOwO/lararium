...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿
  ...
  **这不用设计,是两条已有的规矩叠出来的。** 但要写进 docstring
  ...
  ### 一页的文字:**渲染成图,交给我们自己那个模型转,并且给它指令**

  用户:「要不要 OCR 换成直接调……**就是调我们用的那个模型**?因为那样你还能给指令
  ——比如说,**文字呈现不了的改成一两句对图片的描述并标记图片内容**,然后**公式也能对**。」

  **这条赢在一个本地 OCR 结构上做不到的地方:OCR 只能转录字形,模型能描述。**
  幻灯片上的时序图、流程图,OCR 给你空的。
  ...
  ...
  \( T(n) = \sum_{i=1}^{n} w_i / t_i \)                 ← 文字层给 ■、OCR 丢了 Σ

  [图: 一个时序图,展示 Thread A、Thread B 和 Connection 三条水平线。Thread A 先向下
  执行到 Connection,然后 Thread B 执行到 Connection……Connection 线右侧标有红色文字
  "X cursor corrupted"。]                                ← 另两条路什么都给不出
  ```

  1.9 秒,prompt 1128 token、输出 263 token。

  ### 于是 OCR 整条出局,连带省掉一堆东西

  ```
  不要了:rapidocr(30 MB 模型)+ onnxruntime(76 MB)+ 773 MB 峰值 + 整个子进程设计 + pypdf
  只留:  pypdfium2 —— 把页面渲染成图,而图那一半本来就要它
  ```

  **复核方在这一节判错了三次,依次记下:**
  ① 第一版"有文字层就抽文字、没有才渲染成图"——用户:「你不要算经济账,效果不好你
  1000 token 能抵消??」;
  ② 拿 `rapidocr-onnxruntime`(默认 **v4**)打,报"读不如视觉模型准"——换 **v6-small**
  重打就全对了,**我测了 v4 就报成"RapidOCR 如何如何"**;
  ③ 改成"OCR 出文字层"——而用户指出更好的那条一直在那儿:**同一个模型,加上指令。**
  **纪律:报一个库或模型的结论之前,先打印它实际加载的是哪个版本的哪个文件。**

  ### 转换用的那段指令是这一条的产物,要原样固定下来

  实测有效的那五条(**别改词序、别"优化"措辞,改了要重新打一遍样本**):
  ...
  ### 这一步引入一个注入面,要按 M5-5 那套处理

  **课件是外部内容**(用户下载的 PDF 可能是任何东西),而现在我们**把它喂给模型、
  再把模型的输出存下来**,那份输出之后每次 `read_material` 都进上下文。
  所以一页里写着"忽略以上规则"能influence转换结果。

  - **机制那一半**:存下来的文字**读的时候照样过围栏 + 中和 + 来源标注**
    ——和 `web_fetch` 共用同一个出口,**不许另写一套**;
  - **说服那一半**:上面第 5 条("只输出转换结果")就是框定语,
    **而它是说服不是机制**(M5-5 立的区分),别把它当防线;
  - 这一轮**不需要拉不可信闩**:课件是用户自己给的材料,不是模型上网捞回来的。
    但**这个判断要写在 docstring 里**,免得下一个人以为漏了。

  ### 什么时候转:**入库时整份转完,不是读到哪页才转**

  懒转(读到第 3 页才转第 3 页)省钱,但**搜索会不完整,而不完整的搜索是静默错的**
  ——"搜不到"和"没写过"长得一模一样。这条论证和"OCR 不建索引"那次是同一条。

  - 入库时逐页渲染 + 转换,**页数上限要有**,并**告诉用户转了多少页、还剩多少**;
  - **必须能续**:一页失败不影响别页,重跑只补没转的那些
    (M5-30 立的形状:区分成功 / 失败 / 未完成);
  - 一份 50 页讲义约 50 次调用、每次 ~1.4k token,**一次性**,之后只读缓存。
