...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿
  ...
  **模型手里只有两个名字**,底下怎么落盘它不知道。
  ...
  ```
  list_courses()                              有哪些课
  list_notes(course, page=1)                  某门课下有哪些笔记
  read_note(course, name)                     读一份
  write_note(course, name, content)           写 / 整篇覆盖
  append_to_note(course, name, text)          追加
  replace_in_note(course, name, old, new)     定点改(匹配不上就拒绝,同 M6-4)
  search_notes(query, course=None, page=1)    搜笔记名 + 搜内容;不给 course 就全搜
  ```
  ...
  **名字校验、落点兜底、`replace` 的"匹配不上就拒绝"、不建索引直接扫文件
