...
- **M6-5 学习 bundle**——用户后来补的;**课程表在这里落地,不在待办里**

  用户一开始没选它,后来说「其实我发现学习 bundle 也要」。
  **而它正好解掉一个悬着的问题**:课程表算不算待办?

  真机证据(这是六天里**唯一穿过筛子**的那条需求——明知她可能做不了还提了两次、
  被拒之后又追问一次):
  ...
  **只有课程表这一条有证据。** 作业、考试、复习进度都是我推的,**一条证据都没有**
  ——而这一节刚因为"先想怎么做对、不先问怎么用"判错过三次(见 M6-4 做菜那一节)。
  所以先答:

  1. **课程表是什么形状?** 它是**周期性**的(每周三 10:00 高数,持续一学期),
     而待办是一次性的。周期这件事要么存展开后的每一次,要么存规则——**差别很大,
     选哪个要论证**。
  2. **作业/考试该不该在这一步?** 它们有到期时间,听起来像待办(M6-6)。
     **如果答不上来它们和待办的边界,就先只做课程表**——宁可少做,
     也别搞一个两头都不像的东西。
  3. **要不要像做菜那样用文件?** 课程表是表,不是散文;而作业清单更像待办。
     **两者可能不该用同一种存储**,说清楚。

  ### 一条硬约束

  **到期时间是查询字段,不是闹钟**(M6-7 那条:用户砍掉了整个推送方向)。
  「我这周有什么课」「哪门作业快到了」是**你问她才答**。
  这一点要写进工具的 docstring,免得下一个人看到"到期"就配个提醒。

  ### 明确不做

  不做成绩、不做 GPA、不做课程评价、不做选课。**先让课程表能存能查。**
