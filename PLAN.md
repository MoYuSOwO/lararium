...
- **M5-27 `web_fetch` 带上「要问什么」**——上下文少一半,内容还更对题

  用户看到一次读网页要几千字,问「感觉很占上下文」。**封顶本身是好的**
  (`MAX_FETCH_CHARS=4000`,25,354 字的文章进上下文 4,174 字,而且明说少了多少)。
  **问题在于取的是哪 4000 字:现在盲取开头,而开头往往是导航栏。**

  **实测**(真 key、真包,阮一峰周刊那篇):

  ```
  不给 query   raw_content 25,354 字 → 截前 4000 字
               开头是「阮一峰的网络日志 » [首页](…) » [档案](…) [![](…」——全是导航

  给 query「这期周刊提到了哪些 AI 工具」+ chunks_per_source=3
               raw_content 1,992 字,开头是
               「国内新出的"葫芦 AI",尝试解决这个问题。它在同一个界面,提供上面
                四个模型的访问…」——**直接是答案所在的那几段**
  ```

  **上下文降到一半以下,而且截断那一刀不再落在正文中间**(1992 < 4000,根本不用截)。

  ### 要做的

  1. **`web_fetch(url, question=None)` 加第二个参数**,原样传给 `/extract` 的 `query`,
     同时带上 `chunks_per_source`(默认 3,官方范围 1-5)。
     **docstring 要写清楚 `question` 是什么**:不是给用户看的话,是"这次想从这页里
     知道什么"——模型手里正好有这个,它就是用户刚问的那句。
  2. **`question` 缺省时行为不变**(整页 + 截前 4000 字)。有些场景本来就该要整页
     ——「这篇讲了什么」「帮我总结」没有可挑的焦点,挑了反而丢东西。
     **不要试图替模型判断该不该给**,给不给由它定,工具只管照办。
  3. **升 advanced 那一次要把 `question` 一起带过去**,别升了级反而丢了焦点。

  ### 一个要小心的地方

  `chunks_per_source` 挑出来的是**不连续的片段**,Tavily 用 `[...]` 连接
  (实测报文里就有)。**渲染时不要把它当成连续正文**——`_render_web` 现在会折行,
  折完片段之间的界限就没了,模型会把两段不相干的话读成一句。
  **最小的做法是保留那个 `[...]` 标记不动**(它不是围栏字符,中和不会碰它),
  别自作聪明去美化它。

  ### 验收口径

  - 给 `question` → 请求体里带 `query` 和 `chunks_per_source`;不给 → 两个键都不出现
    (**断请求体,不是断返回**——这条是发出去的形状对不对);
  - 不给 `question` 时,输出和 M5-27 之前**逐字一致**(拿现有测试当金样);
  - 升 advanced 那一次 `question` 也在;
  - 片段之间的 `[...]` 没被折行吃掉;
  - **真机冒烟**:同一个 url 打两次(给/不给 question),把两次进上下文的字数和开头
    记进 `REVIEW.md`——这一条的全部理由就是那个对比,没有数就没有理由。
