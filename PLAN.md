...
## 执行顺序(2026-09-12 用户重排,**编号不动**——改号会把 REVIEW/CHANGELOG 的引用全打断)

```
① M6-5  语音          最小,用户已经踩到,而且查下来不用做 ASR
② M6-6  收入/退款      600 元卡在话头里三天了
③ M6-1  做菜           用户重新设计过,见下
④ M6-2  预算提醒
⑤ M6-3  待办
⑥ M6-4  归拢自动跑
...
**排序依据:已经卡住的 > 已经踩到的 > 还没发生的。** 前两条都有真机证据,
第三条是用户点名要的新能力。

- **M6-1 做菜 bundle**——**用户重新设计过:它是文档,不是表**
...
- **M6-2 预算 / 超支提醒**——用户点的,**做成"记账当场说",不做成定时推送**

  用户选的是「想要预算 / 超支提醒」。**先做反应式的那一半**:记一笔的时候,
  如果这一笔把某个类目或总额推过了线,**就在那一笔的回话里说**。

  **为什么不做成定时推送**:① 不需要调度器,M6-4 之前就能上;
...
- **M6-3 结构化的事务 / 待办**——用户点的;**这是话头承不起的那一半(M5-32)**
...
- **M6-4 夜间归拢自动跑**——原本是"调度器 + 提醒 + 晨报",**被用户砍成了一件事**
  ...
  **不是「她来提醒你」**。
  ...
  - **待办到期提醒**:所以 M6-3 的待办是"你问她才答",到期时间是个**查询字段**,
    不是一个会响的闹钟。**这一点要写进 M6-3 的 docstring**,免得下一个人看到
    "到期时间"就以为该有提醒。
  - **预算的定时那一半**:M6-2 那个"记账当场说"就是全部了,而它现在有了理由
...
- **M6-6 账本只有支出,没有收入**——用户问出来的,而账上**已经卡着 600 元**
