...
- **M6-1 做菜 bundle**——**用户重新设计过:它是文档,不是表**
  ...
  **我原来那版(做法表 + 尝试表)是错的。** 我查了 schema.org/Recipe,砍掉了它九成的字段,
  ...
  ### 「不给文件系统」我按最强的形式做:**存在 SQLite 里**

  用户说的是"给几个工具,不给读目录"。**最强的做法不是校验路径,是让路径不存在**:
  文档存在库里,**名字是一个键,不是一条路径**。于是:

  ```
  ../../etc/passwd     不是"被挡住了",是这个字符串只会被当成一个菜名
  读目录               没有目录可读
  符号链接 / 绝对路径   没有文件系统可以指
  ```

  **和 M5-22 那条同一个道理**:「SSRF 面归零——不是防住了,是根本没有出站」。
  **不要**为了"像文件"去落盘,那等于把一类不存在的 bug 重新造出来。

  名字仍然要规范化 + 封顶(归一化空白、去首尾、长度上限),但那是为了
  **同名能对上**(「紫菜蛋花面」和「紫菜蛋花面 」是一道菜),不是为了安全。

  ### 工具:四个

  `write_recipe(name, content, category=None)` / `read_recipe(name)` /
  `list_recipes(page, category=None)` / `append_to_recipe(name, text)`

  **为什么 `append` 而不是通用的 `edit`**:用户说"得改做法那肯定就 edit"——对,
  但 `edit` 有两种形状,差别很大:

  - **整篇重写**(= `write` 同名覆盖):已经有了,不用第二个工具。
    **但它有个真风险**:模型读完 500 字的做法、"改一处"的时候整篇重打,
    **可能悄悄漏掉一步,而用户不会发现**。这和 M5-5 那条「照着 `str()` 回放等于把图
    悄悄换成一句话,而模型不会知道自己少看了一张」是同一个形状。
  - **追加**:做完一次菜写一句"这次咸了,酱油减半"——**这是最常发生的那个动作**,
    而且它**不可能弄丢任何东西**。

  所以 `append_to_recipe` 单独给一个工具,让最常见、最安全的那条路最短。
  整篇改仍然走 `write` 覆盖。

  **★ 一个要你判的**:`write` 覆盖时要不要留上一版?
  M5-26 刚删掉一整套"留痕但没人读"的机制(作废行),所以按 G6 先问**谁会读到它**
  ——「我上次那版是怎么写的」听起来是有读者的,但**现在没有任何证据**。
  我的倾向:**先不留**,`write` 的 docstring 里写明"同名覆盖,旧的那版不留";
  真撞上"改坏了想找回来"再加。**做不做都行,但要在回报里说清楚理由。**
  ...
  **别在这一步发明新守卫**(M5-11)。
  ...
  - `read_recipe("../../etc/passwd")` → 「没有这道菜」,**而且要有一条测试写明
    这不是路径校验,是根本没有路径这回事**;
