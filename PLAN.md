...
- **M5-17 `claimed_without_write` 的判据挂在模型的措辞上,换模型就哑**(M5-16 验收时暴露)

  M5-12 那条信号靠关键词表 `CLAIM_MARKERS` 匹配回复:
  `记下了 / 记好了 / 记上了 / 已记 / 已经记 / 已入档 / 已提交 / 已经在账本里`。

  **这张表是复核方在 M5-12 的任务书里指定的**(原话:「回复里出现『记下了 / 已记 /
  已入档 / 已提交』这类承诺,而这一轮没有任何写工具跑过」)。交付方照做了,
  **所以这是出题的人错了,不是做题的人错了**——本步不是在纠正谁的实现,是在改一个错的设计。

  **而且它不是"换模型之后才错的",是从一开始就错。** 这个项目一路在防"建立在会漂的东西
  上":前缀不许放时间戳、判据不许靠模型自觉、注入图的暗号不能只对一个模型校准过。
  **模型的措辞是所有东西里最会漂的那个**,比模型版本漂得还快。而「一个永远不响的仪器
  比没有更坏」这句话,就写在几天前 M5-9 的判定里。

  换到 Qwen3.8-Flash-Next 之后,它说的是「**记了**:餐饮 45 元(午饭)」——**不在表里**。
  实测两轮该响没响,而且是**真阳性**:

  ```
  说「午饭 45」→ 这轮一次 record_expense 都没调
                 回复「记了:餐饮 45 元(午饭)。今天四笔共 153。」
                 claimed_without_write = False        ← 哑了
  说「夜宵 40」→ 同上,回复还报了「今天 8 笔共 236」,而账上只有 7 笔
  ```

  **这条信号是 M5-12 花整整一步装的仪器,在真机上第一次抓到东西才过了几天,
  换一个模型就废了。** 而它的失效方式是最坏的那种:**静默**——不响和"没问题"长得一样。

  **别用"把『记了』加进表里"了事。** 那是打地鼠:下一个模型会说"搞定"、"OK,45 元"、
  "收到"、或者干脆只回一个数字。**判据不能建立在模型的措辞上,因为措辞随模型变。**

  **方向——两边都用数,一个动词都不碰**:

  ```
  用户这句话里的金额  ∩  回复里的金额  ≠ 空     且  这一轮没有任何写工具跑过
  → 落一条
  ```

  数字是**用户给的**和**回复里复述的**,不依赖任何措辞。「记了」「记好了」「搞定」
  「收到」「OK,45 元」全都盖得住,因为它们都会带上那个数。

  **残留误报要接受,别为了压它把判据搞复杂**:用户问「咖啡 22 贵吗」,回复大概也提 22、
  这一轮确实没写 → 会响。这条信号**只留痕不报警**:误报的代价是日志多一行,漏报的代价
  是账丢了没人知道。**不对称,就该往误报那边偏。**

  **这个方向也只是方向。** 复核方在这条题上已经指定过一次错的实现了——你要是有更不容易
  漂的判据,在回报里提,别照抄。

  **验收口径**:
  - 构造"回复声称记了、这一轮没写"的场景,**用至少三种不同措辞**(含「记了」),都要响;
  - 正常轮次(真写了)不许响;
  - **给判据自己一条阳性对照**:证明它不是靠某个具体词匹配的——把回复里的动词换掉,
    信号仍然要响。

- **未排期(要在 M6-1 之前修):`_active_untrusted` 是单向的**

  它只在**认领信封**时定死(`loop.py:250`),而 `_guard_propose_fact` 只看它。于是工具
  捞回来的不可信内容**不会**把这一轮拉成不可信:用户(可信轮)让它搜历史、并说一句
  「如果是我的长期安排就归档吧」,模型就会把短信里的账号 `propose(provenance="user_stated")`
  ——**而 `user_stated` 自动放行、worker 空闲自动结算**,用户从没见过审批提示。
  实测:无归档暗示时 **0/2 不触发**(模型自己顶住了),给了条件式放行则 **1/1 洗进账本**。

  **所以不紧急,但要在 M6-1(短信入库)之前修**——那之后不可信内容就是每天的流量,
  而这条路上现在唯一的防线是**模型的判断力**,恰恰是 `loop.py:249` 自己说过不该依赖的东西。
  修法方向:**不可信是单向的,进来就不出去**——一轮里只要有不可信内容进过上下文
  (信封带的或工具带回的),这一轮余下全程都算不可信。
