...
```
M6-1  语音          最小;查下来不用做 ASR,微信转好的文字我们没读
M6-2  收入 / 退款    600 元卡在话头里三天了
M6-3  预算提醒       和 M6-2 同一个 bundle、同一块上下文,接着做
M6-4  做菜 bundle    用户自己设计的,已定稿
M6-5  学习 bundle    课程表——六天里唯一穿过筛子的需求
M6-6  待办           放在学习之后:那时才知道它还剩什么活
M6-7  归拢自动跑      后台作业,最不急
M6-8  隔一阵问一嘴    「助手在场」,和被砍掉的晨报不是一件事——排最后,因为它是唯一会烦人的
```
...
- **M6-2 账本只有支出,没有收入**——用户问出来的,而账上**已经卡着 600 元**
...
- **M6-3 预算 / 超支提醒**——用户点的,**做成"记账当场说",不做成定时推送**

  用户选的是「想要预算 / 超支提醒」。**先做反应式的那一半**:记一笔的时候,
  如果这一笔把某个类目或总额推过了线,**就在那一笔的回话里说**。

  **为什么不做成定时推送**:① 不需要调度器,M6-7 之前就能上;
...
- **M6-4 做菜 bundle**——**用户重新设计过:它是文档,不是表**
...
- **M6-5 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**
  ...
  **路径不出现在接口上。** 模型手里只有课程名(和课件的文件名)。
  只有一个内部函数知道目录在哪——同 M6-4,同 M5-4 那条教训
  ...
  **他说的两件事都对:**
  ...
  2. **模型没有目录、没有路径**(这是 M6-4/M6-5 刻意定的),所以它"复制"不了。
  ...
  **不要为此加"待归档"的状态**——那是个需要清理的队列,而一句话就能问清。
  ...
  ——**全部照 M6-4,一个字都不许另写一套**(接缝见 M6-4 那一节)。
...
- **M6-6 结构化的事务 / 待办**——用户点的;**这是话头承不起的那一半(M5-32)**
  ...
  **用户明知她可能做不了还是提了两次,被拒之后又追问了一次**,这是穿过筛子的需求。
  ...
  课程表**不在这里**(它是周期性的,而且已经在 M6-5 变成一份笔记了)。
  **可以有一个可选的"哪门课"字段**,但别因此长出一套课程管理——课程在 M6-5。
...
- **M6-7 夜间归拢自动跑**——原本是"调度器 + 提醒 + 晨报",**被用户砍成了一件事**
  ...
  **不是「她来提醒你」**。
  ...
  - **待办到期提醒**:所以 M6-6 的待办是"你问她才答",到期时间是个**查询字段**,
    不是一个会响的闹钟。**这一点要写进 M6-6 的 docstring**,免得下一个人看到
    "到期时间"就以为该有提醒。
  - **预算的定时那一半**:M6-3 那个"记账当场说"就是全部了,而它现在有了理由
...
- **M6-8 隔一阵问一嘴**——用户点的;**这是"助手在场",不是"任务系统戳你"**
