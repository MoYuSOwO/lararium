...
- **M6-5 学习 bundle**——**一个课程一个知识库**;作业考试归待办,不在这儿
  ...
  **这不用设计,是两条已有的规矩叠出来的。** 但要写进 docstring
  ...
  ### 本地 OCR:**用,但只用来补文字层,而且跑在独立进程里**

  用户问「搞一个百度的本地 OCR 行不行?听说效果很好而且巨小」。
  **是百度的**——PaddleOCR 是百度的项目,PP-OCR 是它的模型;RapidOCR 只是第三方
  把它们转成 ONNX 重新打包,好处是不用装 PaddlePaddle 那套运行时。

  **复核方在这里判错一次,错法要记下来**:第一次拿 `rapidocr-onnxruntime`(默认带 **v4**)
  打,结果是 `lndexError丶InterfaceError`、`thestatement`、`Keynumberonpage2:274`
  ——据此报了"读不如视觉模型准,只配用来做搜索"。**换成新版(默认拉 PP-OCRv6-small)重打:**

  ```
  中文截图  真实症状有三种面孔:IndexError、InterfaceError、以及TypeError
            check_same_thread=False / 关键数字:无锁时并发二十轮挂十五次     全对
  英文讲义  Lecture 2: Concurrency in SQLite / the statement cache /
            Key number on page 2: 274                                    全对,空格也对
  ```

  **和视觉模型在这两个样本上打平。** 我测了 v4 就报成"RapidOCR 如何如何"
  ——**反复在测「某一个」然后报成「那个东西」**,这一天里第五次(前四次:
  编的 URL、文档里的 title 字段、单样本的 country、答不了的问题问 uv)。
  **纪律:报一个库/模型的结论之前,先打印它实际加载的是哪个版本的哪个文件。**
  ...
  ### 773 MB 决定了它怎么用:**独立进程、一次性、缓存结果**

  真机 3.9 GB 总量、约 2 GB 可用,而 Lararium 自己占 ~800 MB,
  `MemoryMax=2500M`。**常驻在 Steward 进程里不合适**;而**跑一个用完就退的子进程**,
  那 773 MB 是瞬时的,常驻脚印不涨。

  于是它不是一个"在这一轮里调的工具",是**入库时的一次性作业**:

  ```
  一份扫描 PDF 进来 → 逐页 OCR 一次(约 2s/页,50 页约 100 秒)→ 文字层缓存起来
  之后 read_material 和 search_materials 都只读那份缓存,不再跑 OCR
  ```

  **这也顺带解掉一件事**:没有文字层的课件,现在 `search_materials` 也搜得到了。

  ### 所以 `read_material` 那一页的"文字"有两个来源,**对模型是一样的**

  ```
  PDF 自带文字层  → 直接抽
  没有文字层      → 用 OCR 缓存
  两个都没有(还没跑 OCR / OCR 失败)→ **说清楚是哪种**,别让"没文字"和"还没处理"混成一句
  ```

  **但图仍然一直给**——OCR 给的是字,不是对图表的理解;公式、流程图、板书的版式
  只有看图才有。这一点不变(用户定的:一页给两样)。

  ### 明确不做

  - **不装 tesseract**(v6-small 更小更准,没有理由);
  - **不起 subagent 编排 OCR**(它是一条顺序的作业,不是一个需要决策的任务);
  - **不把 OCR 放进 Steward 进程常驻**(见上面那 773 MB);
  - **不用 OCR 替代看图**(只补文字层)。

  ### 这一条的前提:**先确认真有扫描件**

  数字导出的 PPT/PDF 都有文字层,压根用不上 OCR。**现在不知道用户有没有扫描版课件。**
  所以:`read_material` 先做"有文字层就抽、没有就说没有 + 给图"那一版;
  **OCR 这一段等第一次真的撞上没有文字层的课件再接**——那时候它有了明确的消费者。
