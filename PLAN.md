...
- **M5-17 `claimed_without_write` 的判据挂在模型的措辞上,换模型就哑**(M5-16 验收时暴露)
  ...
  **方向(自选,但要能解释为什么它不随模型漂)**:
  - 把判据从**措辞**换成**数**:用户这句话里出现的金额,在这一轮的写工具参数里找不到
    → 落一条。数字是用户给的,不是模型措辞,换模型不动。
  - 或者从**回复里出现的金额**入手:回复提到某个金额,而这一轮没有任何写工具用过它。
  - 两条都会有误报(用户问「咖啡 22 贵吗」),但这条信号**只留痕不报警**,
    误报的代价是日志里多一行,漏报的代价是账丢了没人知道——**不对称,该往误报那边偏**。
