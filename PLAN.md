...
- **M6-6 学习 bundle**——一门课一个笔记本 + 课件只读;**作业考试归待办,不在这儿**
  ...
  **路径不出现在接口上。** 模型手里只有课程名(和课件的文件名)。
  ...
  （读课件走内置的 `read_file(id, page)`——**那一份实现在 Steward 侧,见下**）
  ...
  **拷贝那条的死穴用户自己点出来了**:一旦 bundle 各自持有文件,
  ...
  ### `read_file(id, page=1)`:一份实现,**认不出就说清楚,绝不兜底成另一种类型**

  **`page` 对图片怎么办——图片就是 1 页,不给它开特例。** 用户问到这个:
  「read 所有 file 都是这样子,那如果图片的话怎么办?图片没有 page 啊。」

  ```
  ✗ page 被图片忽略   read_file(id, page=5) 悄悄返回那张图 → **模型以为自己读了第 5 页**
  ✓ 图片就是 1 页     read_file(id, page=5) → 「这是一张图片,只有 1 页」
  ```

  后者是这个项目一贯的纪律:**宁可响亮地失败,不要静默地错**。
  顺带它统一了"共几页"这个属性——**任何可读的东西都有页数**,图片是 1、PDF 是 N。

  - **图片** → 1 页;`read_file(id)` 给图部件(受 `MAX_IMAGES_PER_TURN` 管);
  - **PDF** → N 页;那一页的**文字 + 图**(见下面那两节);
  - **`page` 超范围** → 人话 + **说共几页**(别只说"没有第 5 页");
  - **语音 / 视频 / 认不出的** → **说清楚它是什么、以及有没有别的路**
    (语音:"微信没转出文字,这条我听不了");
  - **任何情况下不许兜底成另一种类型**——`look_at_image` 当初"认不出就按 jpeg 送",
    一段语音和一份 PDF 都被贴上 `image/jpeg` 交了出去,真机上一句
    「这份账单看一下」就 400 当场死掉(M5-5)。**这一条才是那次的教训。**
  - **转换缓存键是 `(media_id, page)`,住在 Steward 侧**——跟着池子,不跟着 bundle。
    两门课引用同一份 PDF 时**共用同一份缓存**,而这不是优化,是"一个事实一个出处"。
  ...
  **前三次是同一件事:先想"怎么把它做对",而不是先问"用户会怎么用它"。**
  ...
  - **课件**:`read_file(id, page)` 一页**同时**给文字和图;要 4 页以上 → 说清一次最多 4 页,
    **不许静默截断**;还没转完 / 转失败 → **说得出是哪种**,
    别让"没文字"和"还没处理"混成一句;
  - **图片是 1 页**:`read_file(图片id)` 给图;`read_file(图片id, page=5)` → 拒绝
    且**说出共几页**;`list_materials` 里图片报 1 页;
  - **图片一律不转**:`add_file` 一张图 → **模型调用计数为 0**;
    之后 `read_file` 那张图 → 只有图、没有文字,**计数仍然为 0**
    (拿计数钉死,别让它为了"对称"偷偷转);
  - **`read_file` 不触发转换**:读一页还没转过的 PDF → 给图 + 说"这页还没转完",
