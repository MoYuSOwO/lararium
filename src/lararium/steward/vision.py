...
——从前在到达轮数,现在到达轮不取字节,就得在唯一那条路上数。(M6-6b 加了第二条取图的路
`read_pdf`:一页 PDF 画成图,和 `read_image` 共用同一份额度、同一把闩、同一句框定。)
...
# **M6-6b 起 `read_pdf` 画出来的每一页也算一张,两个工具数的是同一个计数**
# (`BuiltinTools._images_left`)——一轮最多进这么多张图,不是每个工具各这么多。
