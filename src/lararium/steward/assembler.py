...
def assemble(
    *,
    ...
    ★ **这里挂不上图片,一个字节都挂不上**(M6-2)。M5-5 那会儿到达轮那条 current
    消息是图片的挂载点,于是"历史轮不带图"要靠"只有一个挂载点"来保证;现在那个挂载点
    **被拆掉了**——图只能由模型自己调 `read_image` 取,走的是工具返回那条路
    (`model._adapt`)。于是三件事同时变成结构事实:组装出来的上下文里没有字节、
    起居注的 `prompt` 事件里没有字节、历史轮不可能带图。
    `image_notes` 是**话**不是字节:哪几张图它根本读不了(见 `vision.unreadable_notes`)。
    """
    ...
    # M6-2:到达轮只有**话**——附件那几行报告已经在 `envelope.content` 里
    # (`Attachment.as_line()`),这里再补上"哪几张读不了"。字节的挂载点没了,见 docstring。
